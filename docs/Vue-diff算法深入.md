...
- **diff 本质**：用 O(n) 的同层比较替代 O(n³) 的完整树比对；不跨层级移动节点。
- **虚拟 DOM 作用**：JS 对象描述 UI → 减少直接操作 DOM 的次数；提供跨平台渲染中间层（ReactNative / Weex）。
- **patchVnode 5 种情况**：① 新旧均静态且 key 相同 → 复用 componentInstance；② 新旧均有子节点 → 调用 `updateChildren`（核心）；③ 只有新节点有子节点 → 清空旧文本，addVnodes；④ 只有旧节点有子节点 → removeVnodes；⑤ 均无子节点 → 文本替换。
- **Vue 2 `updateChildren` 双端比较**：维护 oldStart/oldEnd/newStart/newEnd 四个指针，依次尝试首首、尾尾、首尾、尾首四种命中；未命中则查 key-map，再决定移动或新建。时间复杂度 O(n)。
- **Vue 3 diff 核心优化**：① 预处理相同前缀/后缀，跳过不变节点；② 剩余乱序部分用 **最长递增子序列（LIS）** 找出可免移动的节点集合，最小化 DOM 移动次数；③ 创建 VNode 时通过位运算标记类型（ShapeFlag），patch 时直接走对应分支，省去运行时判断。
- **React diff**：以 key + tag 匹配 oldFiber 与新 ReactElement；多节点时两轮遍历：第一轮处理可复用的顺序节点，第二轮用 Map 处理乱序，以 `lastPlacedIndex` 决定节点是移动还是插入。
- **sameVnode 判断（Vue 2 vs snabbdom）**：snabbdom 只比 `key + sel`；Vue 2 额外要求 tag、isComment、data 是否定义，以及 input 的 type 均一致。
- **key 的作用**：帮助 diff 算法识别并复用节点，避免不必要的创建/销毁；列表渲染中禁用 index 作 key（位移后会导致大量错误复用）。
- **源头**：Vue 2 diff 来自 [snabbdom](https://github.com/snabbdom/snabbdom)；Vue 3 diff 借鉴 [inferno](https://github.com/infernojs/inferno) 的 LIS 思路。
...
optimize 函数（src/compiler/optimizer.js）:主要功能是**标记静态节点** 。后面 patch 过程中对比新旧 VNode
...
在创建 VNode 就确定类型，以及在 mount/patch 的过程中采用位运算来判断一个 VNode 的类型，在这个优化的基础上再配合 Diff
算法，性能得到提升。
...
> 补充(现代做法)：上方链接指向的是 Vue 2 的 patch.js，Vue 3 的 diff 已完全重写，核心位于 [`packages/runtime-core/src/renderer.ts`](https://github.com/vuejs/core/blob/main/packages/runtime-core/src/renderer.ts) 的 `patchKeyedChildren` 函数，采用预处理 + 最长递增子序列（LIS）算法，与 Vue 2 的双端比较有本质区别。
...
```js
     ...
     * @param b  对比节点
     * 对比两个节点是否相同
     * 需要组成的条件：key相同，tag相同，是否都为注释节点，是否同时定义了data，如果是input标签，那么type必须相同
     */
