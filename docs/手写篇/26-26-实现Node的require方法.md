...
> 补充(现代做法)：上面讲的是 CommonJS（`require`/`module.exports`，同步加载、运行时解析）。现在 Node 与浏览器都以 **ES Module**（`import`/`export`）为标准：静态解析、支持 Tree Shaking、`import()` 动态导入返回 Promise。两者关键区别可作为面试答点——CommonJS 是**值的拷贝**且同步，ESM 是**实时绑定(live binding)** 且异步加载；ESM 中没有 `require`/`module`/`__dirname`（需用 `import.meta.url` + `fileURLToPath` 还原）。Node 里以 `"type": "module"` 或 `.mjs` 启用 ESM。
