---
title: "React React16为什么要更改生命周期下"
---
...
![](/images/s_poetries_work_images_20210428184607.webp)

> 在React 16.4之后，React
> 生命周期在之前版本的基础上又经历了一次微调，其实就调在了更新过程的`getDerivedStateFromProps` 这个生命周期上

![](/images/s_poetries_work_images_20210428184635.webp)
...
![](/images/s_poetries_work_images_20210428172751.webp)

运行示例代码

![](/images/s_poetries_work_images_20210428172830.webp)
...
![](/images/s_poetries_work_images_20210428183642.webp)

**第二个重点，该方法可以接收两个参数：props 和 state** ，它们分别代表当前组件接收到的来自父组件的 `props` 和当前组件自身的
`state`。我们可以尝试在 Demo 中输出这两个参数看一看，输出效果如下图所示：

![](/images/s_poetries_work_images_20210428183744.webp)

可以看出，挂载阶段输出的 `props` 正是初始化阶段父组件传进来的 `this.props` 对象；而 `state` 是 `LifeCycle`
组件自身的 `state` 对象

**第三个重点，getDerivedStateFromProps 需要一个对象格式的返回值** 。如果你没有指定这个返回值，那么大概率会被 React
警告一番，警告内容如下图所示：

![](/images/s_poetries_work_images_20210428183909.webp)
...
![](/images/s_poetries_work_images_20210428184212.webp)

## Updating 阶段：组件的更新

React 15 与 React 16.3 的更新流程对比如下图所示：

![](/images/s_poetries_work_images_20210428185041.webp)

> `React 16.4` 对生命周期流程进行了“微调”，其实就调在了更新过程的 `getDerivedStateFromProps` 这个生命周期上

![](/images/s_poetries_work_images_20210428185206.webp)
...
![](/images/s_poetries_work_images_20210428190634.webp)

值得一提的是，这个生命周期的设计初衷，是为了“与 `componentDidUpdate` 一起，涵盖过时的 `componentWillUpdate`
...
![](/images/s_poetries_work_images_20210428191042.webp)
...
![](/images/s_poetries_work_images_20210428191428.webp)
...
![](/images/s_poetries_work_images_20210428191535.webp)
...
![](/images/s_poetries_work_images_20210428191629.webp)
