---
title: "React 揭秘 Redux 设计思想与工作原理"
---

## 简版速记

- **Redux = 可预测的 JavaScript 状态容器**：源于 Flux，核心特征是**严格单向数据流**，状态变化来源唯一可追溯。
- **Flux 四要素**：View → Action → Dispatcher → Store → View（单向闭环），解决前端 MVC View ↔ Model 双向通信带来的数据混乱问题。
- **Redux 三要素**：
  - **Store**：唯一数据源，只读；
  - **Action**：描述变化的普通对象，必须含 `type` 字段；
  - **Reducer**：纯函数，`(prevState, action) => newState`，不得有副作用。
- **dispatch 是核心枢纽**：串联 Action → Reducer → Store；内部用 `isDispatching` 标志"上锁"，防止 reducer 内嵌套调用 dispatch（死循环防护）。
- **subscribe 是发布订阅**：`store.subscribe(listener)` 注册监听，返回 `unsubscribe` 函数；dispatch 执行完 reducer 后逐一触发所有 listener。
- **currentListeners vs nextListeners 双数组设计**：dispatch 遍历的是 `currentListeners` 快照，subscribe/unsubscribe 只操作 `nextListeners`，两者通过 `ensureCanMutateNextListeners` 分离引用，保证迭代中途增删 listener 不破坏当次遍历。
- **createStore 核心对外 API**：`getState()`、`dispatch(action)`、`subscribe(listener)`、`replaceReducer(nextReducer)`。
...
![](/images/s_poetries_work_images_20210429151813.webp)

> 一个典型的 Flux 工作流是这样的：`用户与 View 之间产生交互，通过 View 发起一个 Action；Dispatcher 会把这个
...
![](/images/s_poetries_work_images_20210429152003.webp)

除了允许用户通过 View 层交互来触发流程以外，MVC 架构还有另外一种形式，即允许用户通过直接触发 Controller
逻辑来触发流程，这种模式下的架构关系如下图所示：

![](/images/s_poetries_work_images_20210429152037.webp)
...
![](/images/s_poetries_work_images_20210429152225.webp)

这就允许了双向数据流的存在。当业务复杂度较高时，数据流会变得非常混乱，出现类似下图这种情况：

![](/images/s_poetries_work_images_20210429152248.webp)
...
![](/images/s_poetries_work_images_20210429152338.webp)
...
![](/images/s_poetries_work_images_20210429152722.webp)
...
![](/images/s_poetries_work_images_20210429152824.webp)
...
> 补充（现代做法）：Redux 4.x 起官方将 `createStore` 标记为 **deprecated**，推荐使用 **Redux Toolkit** 的 `configureStore`。它内置了 `redux-thunk`、`immer`（支持直接"可变"写法的 reducer）以及 Redux DevTools，一行代码替代上面的模板：
> ```js
> import { configureStore } from '@reduxjs/toolkit';
> const store = configureStore({ reducer: rootReducer });
> ```
> 同时，React-Redux 的 `useSelector` / `useDispatch` hooks 已取代 `connect()` HOC，成为组件消费 store 的主流方式。
...
![](/images/s_poetries_work_images_20210429153418.webp)
...
![](/images/s_poetries_work_images_20210429153844.webp)
...
![](/images/s_poetries_work_images_20210429154009.webp)
...
> dispatch 中执行的 listeners 数组从订阅中来，而执行订阅需要调用 `subscribe`。在实际的开发中，`subscribe`
...
![](/images/s_poetries_work_images_20210429154613.webp)
...
当 for 循环执行到索引 i = 1 处，也就是对应的 listener 为 listenerB 时，问题就会出现：listenerB 中执行了
