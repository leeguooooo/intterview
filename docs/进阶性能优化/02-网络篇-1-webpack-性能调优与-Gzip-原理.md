---
title: "网络篇 webpack 性能调优与 Gzip 原理"
---
...
> 补充(现代做法)：本节的 `DllPlugin`、`HappyPack` 多为 webpack 4 时代方案，现已基本退场。webpack 5 内置持久化缓存（`cache: { type: 'filesystem' }`）可跨进程复用编译结果，多数场景已替代 DllPlugin；多进程构建改用维护活跃的 `thread-loader`，`HappyPack` 已停止维护。若追求更快的本地开发体验，可直接换用 Vite / esbuild / SWC / Rspack 等基于原生语言的工具链（esbuild、SWC 用 Go/Rust 重写转译，速度比 babel-loader 快一个量级；Rspack 兼容 webpack 生态而构建更快）。
...
> 补充(现代做法)：`uglifyjs-webpack-plugin` 不支持 ES6+ 语法，早已被淘汰。webpack 5 默认内置 `terser-webpack-plugin`（基于 Terser，能正确压缩 ES2015+ 代码），生产模式（`mode: 'production'`）下自动开启，无需手动引入；要自定义压缩，配置 `optimization.minimizer` 即可。Tree-Shaking 也更成熟，可在 `package.json` 中声明 `"sideEffects": false` 帮助打包器更激进地剔除无用代码。
