...
> 补充(现代做法)：手写 `scroll` + `getBoundingClientRect` 的懒加载会频繁触发回流（读取布局属性属于「即时计算」），现代首选 **IntersectionObserver**：它由浏览器在元素进出视口时异步回调，不阻塞主线程、无需手动节流。最简单的方案则是给 `<img>`/`<iframe>` 直接加原生属性 `loading="lazy"`，零 JS、浏览器原生支持。
>
> ```js
