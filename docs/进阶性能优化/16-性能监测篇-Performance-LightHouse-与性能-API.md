...
> 补充(现代做法)：`performance.timing`（Navigation Timing Level 1）已被废弃，请改用 `performance.getEntriesByType('navigation')[0]`（Navigation Timing Level 2，返回 `PerformanceNavigationTiming` 对象，含 DNS/TCP/TTFB 等）。更重要的是，业界已从「加载阶段耗时」转向以用户体验为中心的 **Core Web Vitals** 指标：**LCP**（最大内容绘制，衡量加载体感）、**INP**（交互到下次绘制，已于 2024 年取代 FID 衡量响应性）、**CLS**（累积布局偏移，衡量视觉稳定性）。这些指标用 `PerformanceObserver` 订阅，或直接接入 Google 官方的 `web-vitals` 库一行上报，比手算 timing 差值更贴近真实体验。
