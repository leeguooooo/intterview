---
title: "http 基础 13 HTTP传输大文件"
---

## 简版速记

| 手段 | 关键请求头 / 响应头 | 状态码 | 典型场景 |
|---|---|---|---|
| 数据压缩 | `Accept-Encoding` / `Content-Encoding: gzip\|br` | 200 | HTML/CSS/JS 等文本 |
| 分块传输 | `Transfer-Encoding: chunked` | 200 | 流式/动态内容，body 长度未知 |
| 范围请求（单段） | `Range: bytes=x-y` / `Content-Range: bytes x-y/total` | 206 | 视频拖拽、断点续传 |
| 范围请求（多段） | `Range: bytes=x1-y1,x2-y2` / `Content-Type: multipart/byteranges; boundary=xxx` | 206 | 一次请求多个不连续片段 |

**记忆要点：**
- `Transfer-Encoding: chunked` 与 `Content-Length` **互斥**，二者不可同时出现
- 范围越界 → **416 Range Not Satisfiable**；合法范围 → **206 Partial Content**
- 分块格式：`十六进制长度\r\n数据块\r\n`，以 `0\r\n\r\n` 终止
- 范围请求服务器需先声明 `Accept-Ranges: bytes`；不支持则发 `Accept-Ranges: none` 或省略该字段
- 四种方法**可叠加**：压缩 + 分块、范围请求 + 分块均合法
...
> 补充（现代做法）：**Brotli（`br`）已成为主流首选压缩算法**。与 gzip 相比，Brotli 对 HTML/CSS/JS 的压缩率通常高 15–25%，且已被所有现代浏览器支持。Nginx 需安装 `ngx_brotli` 模块并配置 `brotli on; brotli_comp_level 6;`；Cloudflare、Caddy 等 CDN/服务器已默认开启。实践中优先级：`br` > `zstd`（新兴）> `gzip`（兜底）。
...
![](/images/s_poetries_work_gitee_2019_12_18.webp)

> 补充（现代做法）：**HTTP/2 废除了 `Transfer-Encoding: chunked`**。HTTP/2 自带二进制分帧（DATA frame）机制，天然支持流式传输，chunked 编码在 HTTP/2 及以上版本中无效；服务端推流（Server-Sent Events、gRPC streaming）均依赖 HTTP/2 帧而非 chunked。如果你的服务运行在 HTTP/2，无需也不应再设置 `Transfer-Encoding: chunked`，该字段只在 HTTP/1.1 连接上生效。
...
![](/images/s_poetries_work_gitee_2019_12_19.webp)
...
![](/images/s_poetries_work_gitee_2019_12_101.webp)
