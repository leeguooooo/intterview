---
title: "移动端与多端开发面试题"
description: "移动端与多端开发面试题:响应式、rem/vw 适配、1px 问题、H5、混合开发与跨端方案要点。"
---
...
| 考点 | 核心结论 |
|---|---|
| **移动端适配** | rem（动态设置 html font-size）、vw/vh（现代首选）、百分比 + flex；rem = html font-size × 倍数 |
| **300ms 延迟** | 由双击缩放机制引起；现代方案：CSS `touch-action: manipulation`；传统方案：fastclick.js / `user-scalable=no` |
| **1px 问题** | dpr≥2 时 CSS 1px = 2 物理像素；通用解：伪元素 + `scaleY(0.5)`；也可用 `border-image` / `box-shadow` |
| **击穿（点透）** | touch 比 click 快约 300ms，遮罩 touchend 消失后 click 落到底层；推荐方案：`e.preventDefault()` 或统一用 click |
| **JSBridge** | Native→Web：`evaluateJavascript("window.fn()")` ；Web→Native：拦截 URL Schema 或注入 API（`addJavascriptInterface` / `WKScriptMessageHandler`） |
...
> 补充（现代做法）：主流项目已转向 **vw/vh** 单位方案，配合 `postcss-px-to-viewport` 插件可直接在代码中写设计稿 px 值，构建时自动转换为 vw；无需 JS 动态计算 font-size，也没有 12px 最小字号限制。
...
> 补充（现代做法）：Chrome 32+ 在设置了 `width=device-width` 的 viewport 后已自动消除 300ms 延迟；所有现代浏览器均支持 CSS `touch-action: manipulation`，可在不禁用缩放的前提下消除延迟，无需引入 fastclick.js：
> ```css
...
![](/images/s_poetries_work_images_20210319155725.webp)
...
![](/images/s_poetries_work_images_20210319160140.webp)
...
```js
       webView.setWebChromeClient(new WebChromeClient() {
            ...
            public boolean onJsAlert(WebView view, String url, String message, JsResult result) {
                ...
                if ("showToast".equals(urlschema.getMethodName())) {
...
![](/images/s_poetries_work_images_20210319160503.webp)
...
> 补充（现代做法）：iOS 已弃用 UIWebView，现统一使用 **WKWebView**。Web→Native 注入 API 改用 `WKScriptMessageHandler`，JS 侧通过 `window.webkit.messageHandlers.<name>.postMessage(data)` 调用；Native→Web 仍使用 `evaluateJavaScript(_:completionHandler:)`。Android 侧 `addJavascriptInterface` 注意在 API 17+ 需要加 `@JavascriptInterface` 注解，否则低版本存在任意代码执行漏洞。
