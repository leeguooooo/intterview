原文链接: [https://interview.poetries.top/principle-docs/comprehensive/13-%E6%89%93%E9%80%A0%E5%89%8D%E7%AB%AF%E7%9B%91%E6%8E%A7%E7%B3%BB%E7%BB%9F.html](https://interview.poetries.top/principle-docs/comprehensive/13-%E6%89%93%E9%80%A0%E5%89%8D%E7%AB%AF%E7%9B%91%E6%8E%A7%E7%B3%BB%E7%BB%9F.html)
...
| 指标 | 采集方式 | 关键 API |
|------|----------|----------|
| 首屏时间 (FMP) | MutationObserver 分数变化率 | `MutationObserver` + DOM 权重打分 |
| 白屏时间 (FP) | `domLoading - navigationStart` | `performance.timing` |
| 卡顿 | 连续 3 帧 FPS < 20 | `requestAnimationFrame` |
| 资源加载耗时 | 各资源响应时间 | `performance.getEntries()` |

### 首屏优化四重保障

1. **懒加载** — 先渲染可视区域，下滑再按需加载
2. **缓存** — 静态资源用 `Cache-Control`（强缓存）或 `Etag`（协商缓存）；端内接口走 Native 并行预请求
3. **离线化** — `prerender-spa-plugin` 构建期预渲染，首屏直出 HTML
4. **并行化** — HTTP/2 多路复用，废弃 JS/CSS 合并策略和域名散列

### 监控平台架构（自建）

- **数据流**：SDK 上报 → Node.js 入库 → Kafka 消息队列 → Spark/Flink 计算 → MongoDB → 可视化前台
- **错误监控**：`window.onerror`（JS 运行时）+ `window.addEventListener('error')`（资源加载）+ XHR 代理拦截（接口）
- **Source Map**：打包时生成 `.map`，服务端用 `source-map` 库反解行列号定位源码
- **预警策略**：首屏 > 2s 或连续卡顿 → Flink 实时计算 → 企业微信/邮件/短信三级报警

### SDK 设计要点

- 上报优先用 **image GET**（天然跨域，业界主流），复杂数据用 POST
- 支持**抽样上报**（日活大时防流量/带宽爆炸）
- 弱网（2G/3G）本地存储延迟到强网再上报；App 空闲时批量上报（默认 30 条）
- 数据过滤：去重、补全、剔除负值与 > 10s 的极端值

### 现代标准（Core Web Vitals — 2024+）

- **LCP**（最大内容绘制）目标 ≤ 2.5 s
- **INP**（交互到下一帧绘制，2024 年替代 FID）目标 ≤ 200 ms
- **CLS**（累积布局偏移）目标 ≤ 0.1

## 一、性能优化方法论

![](/images/s_poetries_work_images_20210502210714.webp)

> 首屏时间可以拆分为白屏时间、数据接口响应时间、图片加载资源等

![](/images/s_poetries_work_images_20210502214539.webp)

![](/images/s_poetries_work_images_20210503212926.webp)
...
![](/images/s_poetries_work_images_20210503151137.webp)
...
![](/images/s_poetries_work_images_20210503152718.webp)
...
![](/images/s_poetries_work_images_20210503152937.webp)
...
![](/images/s_poetries_work_images_20210503153920.webp)
...
![](/images/s_poetries_work_images_20210503154327.webp)

那么，我们该如何搭建这样一个性能平台呢？

![](/images/s_poetries_work_images_20210503154342.webp)
...
所以最好我们选择 Kafka，先让数据写进消息队列。Kafka
...
![](/images/s_poetries_work_images_20210503160942.webp)
...
![](/images/s_poetries_work_images_20210503161021.webp)
...
![](/images/s_poetries_work_images_20210503161251.webp)
...
![](/images/s_poetries_work_images_20210503161715.webp)
...
![](/images/s_poetries_work_images_20210503162414.webp)
...
![](/images/s_poetries_work_images_20210503163046.webp)

## 九、JS SDK 设计

![](/images/s_poetries_work_images_20210503170316.webp)
...
> 补充（现代做法）：`performance.timing` 在 Chrome 117+ 已被标记为废弃（deprecated）并将移除。现代替代方案是 Navigation Timing Level 2 的 `PerformanceNavigationTiming`，通过 `performance.getEntriesByType('navigation')[0]` 获取，字段名与 `performance.timing` 基本一致但返回的是相对于 `timeOrigin` 的 `DOMHighResTimeStamp`，精度更高且符合最新规范。此外，Google 官方推荐用 [`web-vitals`](https://github.com/GoogleChrome/web-vitals) 库直接采集 LCP/INP/CLS 等 Core Web Vitals，一行代码即可接入。
...
![](/images/s_poetries_work_images_20210503175341.webp)
...
![](/images/s_poetries_work_images_20210503175506.webp)

其实对于数据的展示，可以不用kibana或者其他开源的产品进行展示，也可以自己通过es的restful接口，来搭建数据展示

![](/images/s_poetries_work_images_20210503175531.webp)
...
![](/images/s_poetries_work_images_20210503175936.webp)

### 手动搭建一个前端监控系统

在搭建前端监控系统开始之前，我们先来了解下整个系统的架构是怎么样的。因为你只有了解到流程是怎样走的，才清楚每个步骤我们该做些什么事情。

![](/images/s_poetries_work_images_20210503180034.webp)
...
![](/images/s_poetries_work_images_20210503180112.webp)
...
![](/images/s_poetries_work_images_20210503180234.webp)
...
![](/images/s_poetries_work_images_20210503180304.webp)
...
![](/images/s_poetries_work_images_20210503180352.webp)
...
![](/images/s_poetries_work_images_20210503180431.webp)

**关键性能指标图**

![](/images/s_poetries_work_images_20210503180455.webp)
...
```js
    ...
    // get 上报
    report.getReport({xxx: 111}, (data) => {
        console.log('上报回调的数据：', data)
    
    })
    
    
    // post 上报
    report.postReport({xxx: 111}, (data) => {
        console.log('上报回调的数据：', data)
...
![](/images/s_poetries_work_images_20210503180707.webp)
...
![](/images/s_poetries_work_images_20210503180802.webp)
...
![](/images/s_poetries_work_images_20210503180831.webp)
...
![](/images/s_poetries_work_images_20210503181001.webp)

![](/images/s_poetries_work_images_20210503181010.webp)

![](/images/s_poetries_work_images_20210503181017.webp)

![](/images/s_poetries_work_images_20210503181025.webp)

最终以：首页 Dashboard为例

![](/images/s_poetries_work_images_20210503181035.webp)
