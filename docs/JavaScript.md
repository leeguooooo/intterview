...
![](/images/s_poetries_work_uploads_2022_07_dccf46fd74b0f457.png)
...
![](/images/s_poetries_work_images_20210309114827.png)
...
![](/images/s_poetries_work_uploads_2022_08_61952b031d507e8c.png)
...
![](/images/s_poetries_work_uploads_2022_08_79901f3c439b7aca.png)
...
![](/images/s_poetries_work_uploads_2022_08_d1451162abd9747e.png)

简单录制一下百度页面，看看我们能获得什么，如下动图所示：

![](/images/s_poetries_work_uploads_2022_08_e683158f36977b6b.gif)

> 从上图中我们可以看到，在页面从零到加载完成这个过程中`JS
> Heap`（js堆内存）、`documents`（文档）、`Nodes`（DOM节点）、`Listeners`（监听器）、`GPU
> memory`（`GPU`内存）的最低值、最高值以及随时间的走势曲线，这也是我们主要关注的点

看看开发者工具中的`Memory`一栏，其主要是用于记录页面堆内存的具体情况以及js堆内存随加载时间线动态的分配情况

![](/images/s_poetries_work_uploads_2022_08_ef508139131f6abc.png)

堆快照就像照相机一样，能记录你当前页面的堆内存情况，每快照一次就会产生一条快照记录

![](/images/s_poetries_work_uploads_2022_08_e2a1fe0d0aa4d4fd.png)
...
![](/images/s_poetries_work_uploads_2022_08_ee6a489b7e87ef0f.png)
...
![](/images/s_poetries_work_uploads_2022_08_bcb184db6f3912da.png)
...
![](/images/s_poetries_work_uploads_2022_08_70ebeb00bb65e063.gif)
...
![](/images/s_poetries_work_uploads_2022_08_3c5e8a655b107d85.gif)
...
![](/images/s_poetries_work_uploads_2022_08_89b88d985b2b101e.gif)
...
![](/images/s_poetries_work_uploads_2022_08_3ab5821936052270.gif)
...
![](/images/s_poetries_work_uploads_2022_08_1fa90928bddb4112.gif)
...
![](/images/s_poetries_work_uploads_2022_08_7295a733de9b6e6d.gif)

> 开始录制，先触发一次垃圾回收清除初始的内存，然后点击三次按钮，即执行了三次点击事件，最后再触发一次垃圾回收。查看录制结果发现`JS
> Heap`曲线成阶梯上升，并且最终保持的高度比初始基准线高很多，这说明每次执行点击事件创建的很大的数组对象`obj`都因为`console.log`被浏览器保存了下来并且无法被回收
...
![](/images/s_poetries_work_uploads_2022_08_b875ced23a8c96d7.gif)
...
![](/images/s_poetries_work_uploads_2022_08_da2f80dd5bad24b2.gif)

注释掉了`console.log`

![](/images/s_poetries_work_uploads_2022_08_a69e5d3e85c60164.gif)
...
![](/images/s_poetries_work_uploads_2022_08_a55f5e42812ae5a2.gif)

> 这次的录制结果就能看出，最后的曲线高度和初始基准线的高度一样，说明并没有内存泄漏的情况

  * `memory`

![](/images/s_poetries_work_uploads_2022_08_3c69cab609414411.gif)
...
> 简单总结一下：
> 大家在平时用到了定时器，如果在用不到定时器后一定要清除掉，否则就会出现本例中的情况。除了`setTimeout`和`setInterval`，其实浏览器还提供了一个`API`也可能就存在这样的问题，那就是`requestAnimationFrame`
