...
- **Tree-shaking 本质**：两步配合——`usedExports: true`（标记未用导出为 dead-code）+ `minimize: true`（压缩时移除）；生产模式（`mode: 'production'`）自动开启两者。
- **前提条件**：打包入口必须使用 **ES Modules**（`import/export`），CommonJS 模块无法被 Tree-shaking。
- **babel-loader 陷阱**：babel-loader 8.x+ 会自动保留 ES Modules（不转 CommonJS），无需额外配置；若需手动保险，将 `@babel/preset-env` 的 `modules` 设为 `false`，而非默认 `'auto'`。
- **Scope Hoisting**：`concatenateModules: true`，将多模块合并到一个函数中（又称”作用域提升”），既减小体积又提升运行效率，Webpack 3+ 支持，生产模式自动开启。
- **sideEffects 与 Tree-shaking 的区别**：Tree-shaking 移除**未使用的导出成员**；sideEffects 移除**整个未被引用的模块**（即使模块内有副作用代码）。
- **开启 sideEffects**：两处缺一不可——① `webpack.config.js` 中 `optimization.sideEffects: true`；② `package.json` 中 `”sideEffects”: false`（或白名单数组）。
- **副作用白名单**：CSS import、原型扩展（polyfill）等具有全局副作用的模块，必须在 `package.json` 的 `sideEffects` 数组中列出，否则会被误删。
...
此外，在 main.js 文件中只是导入了 components.js，具体代码如下：
```javascript
    // ./src/main.js
...
![](/images/s_poetries_work_images_20210503215843.webp)
...
![](/images/s_poetries_work_images_20210503215857.webp)

我们这里注意一下 components 对应的这个模块，虽然外部没有使用这里的 `Link` 函数和 `Heading`
函数，但是仍然导出了它们，具体如下图所示：

![](/images/s_poetries_work_images_20210503215912.webp)
...
![](/images/s_poetries_work_images_20210503215920.webp)

此时你会发现 components 模块所对应的函数，就不再导出 `Link` 和 `Heading`
...
![](/images/s_poetries_work_images_20210503215929.webp)
...
我们回到配置文件中，这里我们在 `optimization` 属性中开启 `concatenateModules`。同时，为了更好地看到效果，我们先关闭
`minimize`，具体配置如下：
```javascript
    // ./webpack.config.js
    /** @type {import('webpack').Configuration} */
...
![](/images/s_poetries_work_images_20210503215938.webp)
...
![](/images/s_poetries_work_images_20210503215948.webp)
...
![](/images/s_poetries_work_images_20210503215958.webp)
...
> 补充(现代做法)：babel-loader 9.x（对应 Babel 7.x/8.x）同样默认保留 ES Modules；Webpack 5 在此基础上新增了 `innerGraph` 优化（默认开启），可追踪模块内部变量间的引用关系，进一步扩大 Tree-shaking 范围。此外，Vite（基于 Rollup）和 Rollup 4 的 Tree-shaking 能力更强，对于纯库打包可直接使用 Rollup/Vite，无需额外配置。
...
![](/images/s_poetries_work_images_20210503220008.webp)

通过查阅 babel-loader 模块的源码，我们发现它已经在 `injectCaller` 函数中标识了当前环境支持 ES Modules。

然后再找到我们所使用的 @babel/preset-env 模块源码，部分核心代码如下：

![](/images/s_poetries_work_images_20210503220015.webp)

在这个模块中，根据环境标识自动禁用了对 ES Modules 的转换插件，所以**经过 babel-loader 处理后的代码默认仍然是 ES
...
![](/images/s_poetries_work_images_20210503220024.webp)
...
![](/images/s_poetries_work_images_20210503220033.webp)

根据打包结果发现，所有的组件模块都被打包进了 bundle.js。

此时如果我们开启 Tree-shaking 特性（只设置 usedExports），这里没有用到的导出成员其实最终也可以被移除，打包效果如下：

![](/images/s_poetries_work_images_20210503220041.webp)
...
![](/images/s_poetries_work_images_20210503220051.webp)
...
![](/images/s_poetries_work_images_20210503220100.webp)
