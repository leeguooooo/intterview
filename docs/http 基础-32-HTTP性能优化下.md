原文链接: [https://interview.poetries.top/fe-base-docs/http-protocol/extend/32-HTTP%E6%80%A7%E8%83%BD%E4%BC%98%E5%8C%96%E4%B8%8B.html](https://interview.poetries.top/fe-base-docs/http-protocol/extend/32-HTTP%E6%80%A7%E8%83%BD%E4%BC%98%E5%8C%96%E4%B8%8B.html)

## 简版速记

| 方向 | 核心手段 |
|---|---|
| **开源**（挖潜） | Nginx 动静分离；开启长连接（keepalive）；TCP Fast Open（0-RTT） |
| **节流**（减量） | gzip/br 压缩文本；图片用 JPEG（有损）/ WebP（无损）/ AVIF（现代最优）；删除无用 HTTP 头（Server、X-Powered-By）；缩减 Cookie；域名收缩；避免重定向 |
| **缓存**（复用） | ETag + Last-Modified 标记资源；Cache-Control max-age 静态资源设长期缓存；Redis/CDN 分层缓存；"没有请求的请求，才是最快的请求" |
| **协议升级** | HTTP/2：多路复用 + 头部压缩 + 服务器推送；HTTP/1 的资源合并、域名分片在 H2 是反模式 |

**面试高频考点：**
- HTTP/1 中资源合并减少请求次数；HTTP/2 中多路复用使合并失去意义，反而损害缓存粒度
- 域名收缩在 HTTP/2 中必须做（单 TCP 连接获最佳性能）；HTTP/1 中域名分片可并发更多请求
- Cache-Control 优先级 > Expires；ETag 比 Last-Modified 更精确（基于内容哈希而非修改时间）
- CDN 本质 = 缓存 + 就近访问，是"中间一公里"优化的最有效手段
...
特别要说的是，对于 HTTP 协议一定要启用长连接，TCP 和 SSL
...
> 补充（现代做法）：AVIF 格式（基于 AV1 编码）在有损和无损场景下的压缩率均优于 WebP，且主流浏览器（Chrome 85+、Firefox 93+、Safari 16+）已全面支持。现代图片优先选用 AVIF，再以 WebP 作为降级方案，最后回退到 JPEG/PNG。可使用 `<picture>` 标签或构建工具自动转换。
...
记录的数据量，总是使用 domain 和 path 属性限定 Cookie 的作用域，尽可能减少 Cookie 的传输。如果客户端是现代浏览器，还可以使用
...
> 补充（现代做法）：HTTP/3（基于 QUIC/UDP）已于 2022 年正式标准化（RFC 9114），彻底解决了 HTTP/2 中 TCP 层的队头阻塞问题，并内置 0-RTT 连接恢复。Cloudflare、Google 等 CDN 已广泛部署。Nginx 1.25+ 原生支持 HTTP/3，可通过 `listen 443 quic reuseport` 开启。对于延迟敏感的业务，升级到 HTTP/3 是继 HTTP/2 之后下一个值得关注的优化方向。
...
![](/images/s_poetries_work_gitee_2019_12_88.webp)
