---
title: "http 基础 21 数字签名与证书"
---

## 简版速记

- **摘要算法**：对任意数据生成固定长度"指纹"；单向不可逆；MD5/SHA-1 已在 TLS 中禁用，推荐 **SHA-256**（SHA-2 系列）。
- **完整性**：原文 + 摘要一起用会话密钥加密 → HMAC；明文传摘要无意义，黑客可一并篡改。
- **数字签名**：私钥加密摘要（签名），公钥解密验证（验签）；实现**身份认证 + 不可否认**。
- **CA 与证书**：证书 = 公钥 + 持有者信息 + CA 签名；信任链从操作系统/浏览器内置**根证书**出发，逐级验证。
- **证书等级**：DV（域名验证）< OV（组织验证）< EV（扩展验证）；可信度依次升高。
- **证书吊销**：CRL（吊销列表）/ OCSP（在线状态查询）；CA 被黑或作恶则直接将其列入黑名单。
...
![](/images/s_poetries_work_gitee_2019_12_43.webp)
...
![](/images/s_poetries_work_gitee_2019_12_44.webp)
...
![](/images/s_poetries_work_gitee_2019_12_45.webp)
...
> 补充（现代做法）：Chrome 77（2019-09）和 Firefox 70（2019-10）起已移除地址栏中 EV 证书的企业名称展示，主流浏览器目前仅在证书详情页可查。EV 证书依然提供更严格的身份核验，但"绿色公司名"的视觉标识已基本成为历史。
...
![](/images/s_poetries_work_gitee_2019_12_46.webp)
...
![](/images/s_poetries_work_gitee_2019_12_47.webp)
...
![](/images/s_poetries_work_gitee_2019_12_48.webp)
