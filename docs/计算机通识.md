原文链接: [https://interview.poetries.top/docs/excellent-docs/19-%E8%AE%A1%E7%AE%97%E6%9C%BA%E9%80%9A%E8%AF%86.html](https://interview.poetries.top/docs/excellent-docs/19-%E8%AE%A1%E7%AE%97%E6%9C%BA%E9%80%9A%E8%AF%86.html)

## 简版速记

**网络**
- **UDP vs TCP**：UDP 无连接、不可靠、头部仅 8 字节，适合实时场景（视频/音频）；TCP 有连接、可靠、有序、有流量/拥塞控制。
- **三次握手**：SYN → SYN+ACK → ACK，防止历史失效请求导致服务端空等。
- **四次挥手**：FIN → ACK → FIN → ACK，TIME-WAIT 持续 2MSL 保证对端能收到最终确认。
- **GET vs POST**：GET 可缓存、参数在 URL；POST 无长度限制、支持多编码、相对安全。
- **常见状态码**：200 成功；301 永久重定向；304 未修改；400 语法错误；401 未认证；403 无权限；404 未找到；500 服务端错误；503 不可用。
- **DNS 查询顺序**：本地缓存 → 系统 DNS 服务器 → 根服务器 → 一/二/三级域名服务器；迭代查询由客户端发请求，递归查询由 DNS 服务器代为查询。

**数据结构**
- **栈**：先进后出（LIFO），用于括号匹配、函数调用栈。
- **队列**：先进先出（FIFO），单链队列 deQueue 为 O(n)，循环队列均摊 O(1)。
- **链表**：非连续内存，插入/删除 O(1)，随机访问 O(n)；注意虚拟头节点技巧。
- **BST**：左 < 根 < 右；中序遍历得有序序列；删除有子节点时用后继节点（右子树最小值）替换。
- **堆（大/小根堆）**：shiftUp 插入、shiftDown 删除根节点；父节点索引 `(i-1)/2`，左子 `2i+1`，右子 `2i+2`。

**算法**
- **时间复杂度**：常用最差情况；O(1) < O(logN) < O(N) < O(N logN) < O(N²)。
- **排序**：冒泡/插入/选择 O(N²)；归并/快排 O(N logN)；快排额外空间 O(logN)，归并 O(N)。
- **位运算速记**：左移 `a<<b` ≈ a×2ᵇ；右移 `a>>b` ≈ a÷2ᵇ；异或不进位加法，`a^a=0`，`a^0=a`。
- **链表反转**：三指针 pre/current/next，原地 O(1) 空间。
- **树遍历**：先序（根左右）/ 中序（左根右）/ 后序（左右根）；非递归用栈模拟；广度优先用队列。
...
具体来说
  ...
  * 在接收端，网络层将数据传递给传输层，`UDP` 只去除 `IP` 报文头就传递给应用层，不会做任何拼接操作
...
![](/images/s_poetries_work_gitee_2020_09_226.webp)
...
![](/images/s_poetries_work_gitee_2020_09_227.webp)
...
![](/images/s_poetries_work_gitee_2020_09_228.webp)
...
![](/images/s_poetries_work_gitee_2020_09_229.webp)
...
![](/images/s_poetries_work_gitee_2020_09_230.webp)
...
> 补充(现代做法)：HTTP/1.1 之后出现了 HTTP/2（多路复用、头部压缩 HPACK、Server Push、二进制分帧）和 HTTP/3（基于 QUIC/UDP，解决 TCP 队头阻塞，0-RTT 握手）。面试常考 HTTP/1.1 vs HTTP/2 的区别：HTTP/2 通过单连接多 Stream 消除了 HTTP/1.1 的队头阻塞问题；HTTP/3 则把传输层换成 QUIC，彻底告别 TCP 层面的队头阻塞。
...
> 补充(现代做法)：传统 DNS 查询明文传输，存在劫持和隐私问题。现代方案：**DNS-over-HTTPS（DoH）** 和 **DNS-over-TLS（DoT）** 将 DNS 查询加密，Chrome/Firefox 已默认支持 DoH。此外 DNS 也支持 DNSSEC 签名防篡改。
...
![](/images/s_poetries_work_gitee_2020_09_231.webp)
...
![](/images/s_poetries_work_gitee_2020_09_232.webp)
...
![](/images/s_poetries_work_gitee_2020_09_233.webp)
...
![](/images/s_poetries_work_gitee_2020_09_234.webp)
...
![](/images/s_poetries_work_gitee_2020_09_235.webp)
...
![](/images/s_poetries_work_gitee_2020_09_236.webp)
```js
    class MaxHeap {
      constructor() {
        this.heap = []
...
![](/images/s_poetries_work_gitee_2020_09_237.webp)
...
![](/images/s_poetries_work_gitee_2020_09_238.webp)
