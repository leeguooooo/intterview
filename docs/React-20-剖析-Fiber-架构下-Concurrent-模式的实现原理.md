...
1. **双缓冲（Double Buffering）**：React 同时维护两棵 Fiber 树——`current` 树（当前渲染到屏幕的）和 `workInProgress` 树（内存中构建中的）。commit 完成后 `current` 指针指向 workInProgress 树完成切换。节点通过 `alternate` 属性互相引用，第二次更新起可全量复用节点，无需重新创建 FiberNode。

2. **更新触发统一入口**：`ReactDOM.render` / `setState` / `useState` 均通过创建 `update` 对象 → `enqueueUpdate` 入队 → `scheduleUpdateOnFiber` 调度，最终殊途同归。

3. **同步 vs 异步路径划分**：`scheduleUpdateOnFiber` 中，`lane === SyncLane` 走 `performSyncWorkOnRoot`；否则走 `ensureRootIsScheduled` → `scheduleCallback` → `performConcurrentWorkOnRoot`。

4. **时间切片实现**：
   - 同步：`workLoopSync` —— while 无条件循环，不可中断；
   - 异步：`workLoopConcurrent` —— while 每次先调用 `shouldYield()`，超过时间切片（约 5ms）则中断，让出主线程；
   - `shouldYield` 本体是 `Scheduler.unstable_shouldYield`，比较 `performance.now()` 与 `deadline`；
   - `deadline = currentTime + yieldInterval`，yieldInterval 根据浏览器帧率动态计算。

5. **Scheduler 优先级调度**（`unstable_scheduleCallback`）：
   - 按 `priorityLevel` 计算 `timeout`，进而得到 `expirationTime = startTime + timeout`（优先级越高，timeout 越小，expirationTime 越小）；
   - 未过期任务 → 推入 `timerQueue`（以 startTime 为索引的小顶堆，堆顶 = 最早需要执行的任务）；
   - 已过期任务 → 推入 `taskQueue`（以 expirationTime 为索引的小顶堆，堆顶 = 最高优先级任务）；
   - 通过 `MessageChannel`（不支持时降级 `setTimeout`）发起异步调度，`flushWork` → `workLoop` 逐一清空 taskQueue。

6. **面试高频问**：
   - Concurrent 模式为何能中断渲染？→ `workLoopConcurrent` 中 `shouldYield()` 检查时间切片是否到期，到期则 while 退出，主线程归还给浏览器。
   - 双缓冲的意义？→ 复用 Fiber 节点（alternate 指针）+ 保证用户始终看到完整视图，避免渲染中间态暴露。
   - 优先级如何影响调度顺序？→ `expirationTime` 越小优先级越高，taskQueue 小顶堆堆顶即为最高优先级待执行任务。
   - `MessageChannel` 为何优先于 `setTimeout`？→ setTimeout 在嵌套调用时有 4ms 最小延迟，MessageChannel 宏任务延迟更小，调度更精准。
...
![](/images/s_poetries_work_images_20210502092851.webp)
...
![](/images/s_poetries_work_images_20210502092910.webp)

待 commit 阶段完成后，右侧的 workInProgress 树对应的 DOM 树就被真正渲染到了页面上，此时 current 指针会指向
workInProgress 树：

![](/images/s_poetries_work_images_20210502092926.webp)
...
![](/images/s_poetries_work_images_20210502092947.webp)

这段复用的逻辑在 beginWork 调用链路中的 createWorkInProgress 方法里。这里我为你截取了
createWorkInProgress 方法里面一段非常关键的逻辑，请看下图：

![](/images/s_poetries_work_images_20210502093001.webp)

在 createWorkInProgress 方法中，会先取当前节点的 alternate 属性，将其记为 workInProgress 节点。对于
rootFiber 节点来说，它的 alternate 属性，其实就是上一棵 current 树的 rootFiber，如下图高亮部分所示：

![](/images/s_poetries_work_images_20210502093019.webp)
...
![](/images/s_poetries_work_images_20210502093033.webp)
...
![](/images/s_poetries_work_images_20210502093050.webp)
...
![](/images/s_poetries_work_images_20210502093120.webp)

你会发现还是熟悉的配方，还是原来的味道。其实，挂载可以理解为一种特殊的更新，ReactDOM.render 和 setState
...
![](/images/s_poetries_work_images_20210502093141.webp)

请你关注图中两处标红的函数调用，你会看到 dispatchAction 方法在 performSyncWorkOnRoot
的左边。也就是说整体的更新链路应该是这样的：

![](/images/s_poetries_work_images_20210502093156.webp)

`dispatchAction` 中，会完成 update 对象的创建，如下图标红处所示：

![](/images/s_poetries_work_images_20210502093208.webp)
...
![](/images/s_poetries_work_images_20210502093233.webp)
...
![](/images/s_poetries_work_images_20210502093304.webp)
...
![](/images/s_poetries_work_images_20210502093315.webp)
...
![](/images/s_poetries_work_images_20210502093328.webp)
...
![](/images/s_poetries_work_images_20210502093359.webp)
...
![](/images/s_poetries_work_images_20210502093441.webp)

通过前面的学习，我们已经知道 Fiber 架构下的异步渲染（即 Concurrent 模式）的核心特征分别是“时间切片”与“优先级调度”。而这两点，也正是
...
![](/images/s_poetries_work_images_20210502093521.webp)

当我使用 ReactDOM.render 来渲染这个长列表时，它的调用栈如下图所示：

![](/images/s_poetries_work_images_20210502093533.webp)
...
> 补充（现代做法）：React 18 起，`createRoot` 已成为正式稳定 API，`ReactDOM.render` 被标记为 deprecated。新项目应直接使用 `createRoot`，Concurrent 模式是默认行为而非可选项。`ReactDOM.render` 在 React 18 中仍可用，但会以 Legacy 模式运行（同步渲染，无时间切片），并在控制台输出警告。

![](/images/s_poetries_work_images_20210502093551.webp)
...
![](/images/s_poetries_work_images_20210502093618.webp)
...
![](/images/s_poetries_work_images_20210502093629.webp)

shouldYield 直译过来的话是“需要让出”。顾名思义，当 shouldYield() 调用返回为 true
时，就说明当前需要对主线程进行让出了，此时 while 循环的判断条件整体为 false，while 循环将不再继续。
...
![](/images/s_poetries_work_images_20210502093654.webp)

其中 unstable_now 这里实际取的就是 performance.now() 的值，即“当前时间”。那么 deadline
又是什么呢？它可以被理解为当前时间切片的到期时间，它的计算过程在 Scheduler 包中的 performWorkUntilDeadline
方法里可以找到，也就是下图的标红部分：

![](/images/s_poetries_work_images_20210502093706.webp)
...
从源码中我们可以看出，unstable_scheduleCallback 的主要工作是针对当前任务创建一个 task，然后结合 startTime
...
而通过前面的科普，大家已经知道了小顶堆是一个相对有序的数据结构。timerQueue 作为一个小顶堆，它的排序依据其实正是 sortIndex
...
![](/images/s_poetries_work_images_20210502093842.webp)
