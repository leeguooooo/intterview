...
![](/images/s_poetries_work_images_20210421163450.webp)
...
属性值来进行预加载，就能进一步提升渲染速度。
  ...
  * `dns-prefetch`。当 `link` 标签的 `rel` 属性值为“dns-prefetch”时，`浏览器会对某个域名预先进行 DNS 解析并缓存`。这样，当浏览器在请求同域名资源的时候，能省去从域名查询 IP 的过程，从而`减少时间损耗`。下图是淘宝网设置的 DNS 预解析 ![](/images/s_poetries_work_images_20210421163848.webp)
...
![](/images/s_poetries_work_images_20210421164758.webp)
