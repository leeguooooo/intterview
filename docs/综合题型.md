原文链接: [https://interview.poetries.top/docs/base/comprehensive.html](https://interview.poetries.top/docs/base/comprehensive.html)

## 简版速记

**创建对象**：字面量 `{}` / `new Object()` / `Object.create(proto)`，三种方式。`Object.create` 用原型链连接，构造函数是 `Object`。

**原型链三角**：构造函数 `.prototype` → 原型；原型 `.constructor` → 构造函数；实例 `.__proto__` → 原型。`instanceof` 沿 `__proto__` 链查找；精确判断直接用 `obj.__proto__.constructor === Constructor`。

**new 四步**：① 创建空对象 ② `__proto__` 指向构造函数 `prototype` ③ 执行构造函数（this 指向新对象） ④ 构造函数有对象返回值则用该值，否则返回新对象。

**继承三式**：① 构造函数继承（`Parent.call(this)`）——不能继承原型方法；② 原型链继承（`Child.prototype = new Parent()`）——引用类型属性被所有实例共享；③ 组合继承（两者结合）——最常用，缺点是父构造函数执行两次。ES6 `class` + `extends` + `super()` 是现代推荐写法。

**DOM 事件**：DOM0 `onclick=fn`；DOM2 `addEventListener(type, fn, useCapture)`，`true` 为捕获，`false`（默认）为冒泡。捕获顺序：`window → document → html → body → … → 目标`。`preventDefault` 阻止默认行为；`stopPropagation` 阻止冒泡；`stopImmediatePropagation` 阻止同元素后续监听器；`currentTarget` 绑定元素，`target` 实际触发元素。

**Event Loop**：宏任务（setTimeout / setInterval / 主代码块）执行完毕 → 清空所有微任务（Promise.then / MutationObserver）→ GUI 渲染 → 下一个宏任务。

**盒模型**：标准 `width = content`；IE `width = content + padding + border`；`box-sizing: border-box` 切为 IE 模型。BFC 触发：`overflow≠visible` / 浮动 / 绝对或固定定位 / `inline-block` 等。BFC 特性：内部垂直 margin 重叠；独立容器；不与 float 区重叠；计算高度含浮动子元素。

**三栏布局**：浮动 / 绝对定位 / flex（中间 `flex:1`）/ table（`display:table-cell`）/ grid（`grid-template-columns: 300px auto 300px`）。高度不固定时只有 flex 和 table 可正常工作。

**CSRF vs XSS**：CSRF 需已登录，利用已有 cookie 伪造请求，防御用 Token / Referer 验证；XSS 注入脚本到页面，防御用 HTML 编码 + 过滤 + 富文本白名单。

**跨域五式**：JSONP（`<script>` src 回调，仅 GET）/ WebSocket（不受同源限制）/ CORS（服务端响应头配置，浏览器自动处理）/ Hash（iframe 通信）/ postMessage（H5 跨窗口）。

**错误监控**：运行时错误用 `try/catch` 或 `window.onerror`；资源加载错误用元素 `onerror` / `performance.getEntries()` 差集 / 捕获阶段 `addEventListener('error', fn, true)`。上报推荐 `new Image().src = reportUrl`（无跨域限制）。

**HTTP 要点**：无连接、无状态。GET 可缓存、参数在 URL、回退不重发；POST 参数在 body，相对安全。常见状态码：200 成功 / 206 范围请求 / 301 永久重定向 / 304 缓存可用 / 403 拒绝 / 404 不存在 / 500 服务端错误。HTTP/1.1 默认长连接（`Connection: keep-alive`），管线化允许批量发送请求但需按序响应。
...
![](/images/s_poetries_work_gitee_2020_07_18.webp)

### 3 原型、构造函数、实例，以及原型链

![](/images/s_poetries_work_gitee_2020_07_19.webp)

> PS：任何一个函数，如果在前面加了`new`，那就是构造函数。

#### 3.1 原型、构造函数、实例三者之间的关系

![](/images/s_poetries_work_gitee_2020_07_20.webp)
...
![](/images/s_poetries_work_gitee_2020_07_21.webp)
...
![](/images/s_poetries_work_gitee_2020_07_22.webp)
...
![](/images/s_poetries_work_gitee_2020_07_41.webp)
...
![](/images/s_poetries_work_gitee_2020_07_43.webp)
...
![](/images/s_poetries_work_gitee_2020_07_44.webp)
...
![](/images/s_poetries_work_gitee_2020_07_45.webp)
...
![](/images/s_poetries_work_gitee_2020_07_46.webp)
...
> 补充（现代做法）：ES6 的 `class` + `extends` + `super()` 在语义上等价于组合继承，内部仍基于原型链，但写法更简洁、可读性更高，是当前推荐的继承方式。面试中可先答 ES5 三种方式再补充 ES6 语法加分。
...
![](/images/s_poetries_work_gitee_2020_07_58.webp)
![](/images/s_poetries_work_gitee_2020_07_59.webp)
...
![](/images/s_poetries_work_gitee_2020_07_60.webp)
...
![](/images/s_poetries_work_gitee_2020_07_69.webp)
...
![](/images/s_poetries_work_gitee_2020_07_70.webp)
![](/images/s_poetries_work_gitee_20190928_3.webp)
...
![](/images/s_poetries_work_gitee_20190928_4.webp)
...
![](/images/s_poetries_work_gitee_20190928_6.webp)
...
![](/images/s_poetries_work_gitee_20190928_8.webp)
...
![](/images/s_poetries_work_gitee_20190928_10.webp)
...
![](/images/s_poetries_work_gitee_20190928_11.webp)
...
![](/images/s_poetries_work_gitee_2020_07_47.webp)

`IE`盒子模型：

![](/images/s_poetries_work_gitee_2020_07_48.webp)
...
![](/images/s_poetries_work_gitee_2020_07_49.webp)
...
![](/images/s_poetries_work_gitee_2020_07_50.webp)

父亲这个盒子：

![](/images/s_poetries_work_gitee_2020_07_51.webp)
...
![](/images/s_poetries_work_gitee_2020_07_52.webp)

> 此时我们给父亲`div`加一个`border`属性，就正常了：

![](/images/s_poetries_work_gitee_2020_07_53.webp)
...
> 有以下几种方法：
  ...
  * 方法3：定位中：只要`position`的值不是 `static`或者是`relative`即可，可以是`absolute`或`fixed`，也就生成了一个`BFC`。
...
![](/images/s_poetries_work_gitee_2020_07_54.webp)
...
![](/images/s_poetries_work_gitee_2020_07_55.webp)
...
![](/images/s_poetries_work_gitee_2020_07_56.webp)
...
![](/images/s_poetries_work_gitee_2020_07_57.webp)
...
![](/images/s_poetries_work_gitee_2020_07_25.webp)
...
![](/images/s_poetries_work_gitee_2020_07_17.webp)
...
![](/images/s_poetries_work_gitee_2020_07_23.webp)

#### 4.5 事件的触发顺序

![](/images/s_poetries_work_gitee_2020_07_24.webp)
...
> 补充（现代做法）：JSONP 仅支持 GET 请求，且需要服务端配合，安全性较低。现代项目优先使用 **CORS**（服务端设置 `Access-Control-Allow-Origin`），或在同域反向代理下直接用 Fetch/Axios，JSONP 仅在需要兼容极旧浏览器时作为备选方案。
...
![](/images/s_poetries_work_gitee_2020_07_30.webp)
...
![](/images/s_poetries_work_gitee_2020_07_31.webp)
...
![](/images/s_poetries_work_gitee_2020_07_32.webp)

上图中，红色那一栏表明，我的请求已经发出去了。点进去看看：

![](/images/s_poetries_work_gitee_2020_07_33.webp)
...
![](/images/s_poetries_work_gitee_2020_07_34.webp)
...
![](/images/s_poetries_work_gitee_2020_07_35.webp)
...
![](/images/s_poetries_work_gitee_2020_07_36.webp)
...
![](/images/s_poetries_work_gitee_2020_07_37.webp)
...
> 我们大概要记住以下几点：
  ...
  4. `get`请求在`url`中传递的参数有大小限制，基本是`2kb`，不同的浏览器略有不同。而`post`没有此限制。
...
![](/images/s_poetries_work_gitee_2020_07_38.webp)

> 常见的`http`状态码：

![](/images/s_poetries_work_gitee_2020_07_39.webp)
...
![](/images/s_poetries_work_gitee_2020_07_40.webp)
...
> 补充（现代做法）：HTTP/1.1 管线化因队头阻塞问题实际浏览器支持有限。**HTTP/2** 通过多路复用（Multiplexing）在同一 TCP 连接上并行处理多个请求/响应，彻底解决队头阻塞，是当前主流方案。**HTTP/3** 进一步基于 QUIC（UDP）解决了传输层队头阻塞。
