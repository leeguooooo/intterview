原文链接: [https://interview.poetries.top/fe-base-docs/http-protocol/base/18-HTTP%E7%9A%84%E4%BB%A3%E7%90%86%E6%9C%8D%E5%8A%A1.html](https://interview.poetries.top/fe-base-docs/http-protocol/base/18-HTTP%E7%9A%84%E4%BB%A3%E7%90%86%E6%9C%8D%E5%8A%A1.html)

## 简版速记

- **代理本质**：插在客户端与服务器之间的中间服务器，具备双重身份——对下游客户端表现为服务器，对上游源服务器表现为客户端；本身不生产内容，只负责转发。
- **正向代理 vs 反向代理**：正向代理代表客户端（如科学上网、企业出口网关），反向代理代表服务器（如 Nginx/CDN 负载均衡）；HTTP 协议层面两者行为一致，区别在于"谁在控制"。
- **代理的主要功能**：负载均衡、健康检查、安全防护、加密卸载（SSL Termination）、数据过滤、内容缓存。
- **`Via` 头**：通用头字段，每经过一个代理节点追加自身主机名；请求方向与响应方向相反（响应链倒序）。
- **客户端真实 IP**：
  - `X-Forwarded-For`：每经过代理追加请求方 IP，最左侧为原始客户端 IP（可伪造，需只信任最后一个可信代理写入的值）。
  - `X-Real-IP`：仅记录原始客户端 IP，无完整代理链，是 `X-Forwarded-For` 的简化版。
  - 现代标准：RFC 7239（2012）定义了 `Forwarded` 头，语义更严格；`X-Forwarded-For` 因历史存量仍占主导。
- **PROXY Protocol**：HAProxy 提出的四层代理协议，在 TCP 握手后立即发送带有客户端真实 IP 的首行，无需修改 HTTP 报文；Nginx 1.5.12+ 及众多后端框架支持。

引入 HTTP
代理后，原来简单的双方通信就变复杂了一些，加入了一个或者多个中间人，但整体上来看，还是一个有顺序关系的链条，而且链条里相邻的两个角色仍然是简单的一对一通信，不会出现越级的情况。

![](/images/s_poetries_work_gitee_2019_12_30.webp)
...
![](/images/s_poetries_work_gitee_2019_12_31.webp)
...
![](/images/s_poetries_work_gitee_2019_12_32.webp)
...
> 补充（现代做法）：RFC 7239（2012）正式定义了标准头字段 `Forwarded`，格式为 `Forwarded: for=192.0.2.1; by=203.0.113.1; proto=https`，语义更精确且支持 IPv6 与端口。不过 `X-Forwarded-For` 的历史存量极大（Nginx、AWS ELB、Cloudflare 等默认仍输出此字段），实际项目中两者并存。注意：`X-Forwarded-For` 的最左侧 IP 可被客户端伪造，后端应只信任可信代理最后一次写入的值，或配合 `PROXY Protocol` 在 TCP 层传递真实 IP 以彻底规避伪造风险。
...
![](/images/s_poetries_work_gitee_2019_12_33.webp)
