---
title: "http 基础 26 HTTP2特性概览"
---

## 简版速记

- **版本号**：HTTP/2 起不再有小版本号（无 2.0/2.1），每个大版本代表本质跃进。
- **向下兼容**：语义层（方法、URI、状态码、头字段）与 HTTP/1 完全一致，无需改动上层应用；语法层全面重构。
- **头部压缩**：HPACK 算法——静态表 + 动态表（字典索引）+ 哈夫曼编码，压缩率 50%~90%。
- **二进制帧**：报文拆为 HEADERS 帧（头）+ DATA 帧（体），告别文本格式，解析无歧义。
- **多路复用**：一条 TCP 连接上多个 Stream 并发，每个 Stream 有唯一 ID，解决 HTTP 层队头阻塞。
- **服务器推送**：服务端可主动推送 JS/CSS 等资源（h2 push），减少等待延迟。
- **安全要求**：实践中 h2 = TLS 加密（TLS 1.2+，禁用 DES/RC4/CBC/SHA-1）；h2c = 明文，主流浏览器不支持。
- **注意盲点**：HTTP/2 只解决了 HTTP 层队头阻塞，TCP 层的队头阻塞依然存在；HTTP/3（QUIC）才彻底解决。
...
> **补充（现代做法）：** Server Push 在实践中因缓存浪费、优先级难以控制等问题效果不佳。Chrome 自 2022 年 10 月（Chrome 106）起已移除对 HTTP/2 Server Push 的支持。现代替代方案是 **103 Early Hints**（RFC 8297）——服务端在正式响应前发送 `103 Early Hints` 状态码 + `Link` 头，告知浏览器预加载关键资源，已在主流浏览器和 CDN（Cloudflare、Fastly 等）广泛支持。
...
> **补充（现代做法）：** TLS 1.3 已于 2018 年 8 月正式发布（RFC 8446），现已被主流服务器和浏览器广泛支持并默认启用。TLS 1.3 删除了大量不安全特性、将握手往返从 2-RTT 降至 1-RTT（支持 0-RTT 恢复），安全性和性能均优于”TLS1.25”的过渡方案。
