原文链接: [https://interview.poetries.top/fe-base-docs/http-protocol/base/15-HTTP%E7%9A%84%E9%87%8D%E5%AE%9A%E5%90%91.html](https://interview.poetries.top/fe-base-docs/http-protocol/base/15-HTTP%E7%9A%84%E9%87%8D%E5%AE%9A%E5%90%91.html)

## 简版速记

- **机制**：服务器返回 3xx 状态码 + `Location` 响应头，浏览器自动发起新请求，用户无感知（共两次请求-应答）。
- **301 永久重定向**：原 URI 不再使用，浏览器/搜索引擎更新书签和索引，下次直接用新 URI，SEO 权重随之转移。
- **302 临时重定向**：原 URI 仍有效，浏览器保留原地址，每次均走跳转，不更新书签/索引。
- **303 See Other**：重定向后强制改为 GET，常用于 POST 表单提交后跳转结果页（PRG 模式，防重复提交）。
- **307 Temporary Redirect**：同 302 语义，但**严格保留请求方法**（POST 不会被改为 GET）。
- **308 Permanent Redirect**：同 301 语义，但**严格保留请求方法**（POST 不会被改为 GET）。
- **Location 头**：站内跳转可用相对路径；跨站跳转**必须**使用完整绝对 URI（含 scheme）。
- **注意事项**：每次跳转多一次 RTT，应适度使用；务必避免循环跳转（A→B→C→A），浏览器会检测并报错。
...
> 补充（现代做法）：307 和 308 目前已被 Chrome、Firefox、Safari、Edge 等主流浏览器完全支持，可放心使用。需要保留 POST 请求方法时，推荐用 **308** 替代 301、用 **307** 替代 302，语义更精确，可避免旧版浏览器将 POST 静默降级为 GET 的历史兼容问题。
