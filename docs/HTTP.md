原文链接: [https://interview.poetries.top/docs/excellent-docs/15-HTTP%E6%A8%A1%E5%9D%97.html](https://interview.poetries.top/docs/excellent-docs/15-HTTP%E6%A8%A1%E5%9D%97.html)
...
- `3xx` 重定向：`301` 永久重定向；`302` 临时重定向；`304` 协商缓存命中；`307` 临时重定向（保留方法）；`308` 永久重定向（保留方法）
- `4xx` 客户端错误：`400` 语法错误；`401` 未授权；`403` 禁止访问；`404` 未找到
- `5xx` 服务端错误：`500` 内部错误；`504` 网关超时

**TCP 三次握手 / 四次挥手**
- 握手：SYN → SYN+ACK → ACK（目的：防止失效连接占用服务器资源）
- 挥手：FIN → ACK → FIN → ACK（双方各自确认数据发完才关闭）

**HTTP 版本演进**
- HTTP/1.0：每次请求独立 TCP 连接（短连接）
- HTTP/1.1：默认 `keep-alive` 长连接、支持管道化（但有队首阻塞）
- HTTP/2：多路复用（同一 TCP 并发多流）、二进制分帧、HPACK 头压缩、Server Push
- HTTP/3：基于 QUIC（UDP），彻底消除 TCP 层队首阻塞

**HTTPS 握手（TLS 1.2 ECDHE 简版）**
1. Client Hello：random + 支持的加密套件列表
2. Server Hello：random + server_params + 证书
3. 客户端验证证书，发送 client_params；双方各自用 ECDHE 算出 pre_random
4. 双方用 client_random + server_random + pre_random 派生 session key（对称密钥）
5. 收尾握手后全程对称加密通信

**HTTPS 核心知识点**
- HTTP 明文（80端口）vs HTTPS = HTTP + TLS（443端口）
- 证书验证链：浏览器内置根证书 → 中间 CA → 服务器证书（数字签名校验完整性）
- 为什么数据传输用对称加密：非对称加密效率低，握手阶段非对称传递对称密钥，后续对称加密
- TLS 1.2 握手 2-RTT；TLS 1.3 握手 1-RTT，复用时支持 0-RTT

**缓存策略**
- 强缓存（不发请求）：`Cache-Control: max-age=xxx`（优先）> `Expires`，命中返回 `200 from cache`
- 协商缓存（发请求验证）：`Etag`/`If-None-Match`（优先）> `Last-Modified`/`If-Modified-Since`，命中返回 `304`

**Cookie / Session / Token**
- Cookie 存客户端（4KB 限制），Session 存服务端（服务器压力大）
- Token（JWT）：服务端无状态，前端 header 手动携带，不受 CSRF 影响

**WebSocket 要点**
- 基于 HTTP 升级握手（`101 Switching Protocols`），之后走原生 TCP 全双工
- 与 HTTP 区别：双向实时通信、低开销、无同源限制

**CORS 要点**
- 简单请求（GET/HEAD/POST + 受限 Content-Type）：直接发，浏览器验证响应头
- 复杂请求：先发 OPTIONS 预检，服务端返回允许规则后再发实际请求
- `Access-Control-Max-Age` 可缓存预检结果，避免每次都发 OPTIONS

**GET vs POST**
- GET：幂等、参数在 URL、可缓存、不适合敏感数据
- POST：有副作用、参数在 body、支持多种编码、不自动缓存
...
![](/images/s_poetries_work_images_202202211654404.webp)
...
![](/images/static001_geekbang_org_resource_image_1b_6c_1b49976aca2c700883d48d927f48986c.webp)
...
**举例：已失效的连接请求报文段**
  ...
  * 本来这已经是一个失效的报文，但是`server`端接收到这个请求报文后，还是会向`client`发出确认的报文，表示同意连接。
  * 假如不采用三次握手，那么只要`server`发出确认，新的建立就连接了，但其实这个请求是失效的请求，`client`是不会理睬`server`的确认信息，也不会向服务端发送确认的请求
  * 但是`server`认为新的连接已经建立起来了，并一直等待`client`发来数据，这样，server的很多资源就白白浪费掉了
...
> 补充（现代做法）：HTTP/3（2022年正式标准化，RFC 9114）基于 QUIC 协议（UDP），彻底解决了 HTTP/2 中 TCP 层的队首阻塞问题，握手延迟更低（0-RTT 复用），适合弱网环境。主流浏览器和 CDN（Cloudflare、Google）已大规模支持。
...
![](/images/s_poetries_work_gitee_2019_11_113.webp)
...
>
> 废除了很多的加密算法，只保留了5个加密套件。其中最主要的是废弃了RSA，因为在2015年发现了FREAK攻击，即已经有人发现了RSA的漏洞能进行破解；而且RSA不具备向前安全性。

  2. 提高性能

同时利用会话复用节省了重新生成密钥的时间，利用 PSK 做到了`0-RTT`连接。

> 补充（现代做法）：TLS 1.3 已成为主流服务器和浏览器的默认协议（Nginx 1.13+、OpenSSL 1.1.1+ 均默认支持）。TLS 1.0/1.1 已被 IETF 正式废弃（RFC 8996，2021年）。生产环境应配置 `ssl_protocols TLSv1.2 TLSv1.3`，禁用旧版本。
...
![](/images/s_poetries_work_images_202203211356046.webp)
...
![](/images/s_poetries_work_images_202203211356643.webp)
...
![](/images/s_poetries_work_gitee_2020_09_114.webp)
...
![](/images/s_poetries_work_images_20210319105457.webp)
...
![](/images/s_poetries_work_gitee_2020_09_1.webp)
