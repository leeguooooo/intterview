...
**强缓存**（命中时不发请求，状态码 200，Size 显示 `from disk/memory cache`）
- `Expires`：HTTP/1.0，绝对时间，受客户端时钟影响，精度差
- `Cache-Control: max-age=N`：HTTP/1.1，相对秒数，优先级高于 Expires
- `s-maxage`：仅代理/CDN 生效，覆盖 max-age
- `no-cache`：可缓存，但每次必须向服务器验证；`no-store`：真正禁止缓存

**协商缓存**（发请求，服务器返回 304 = 用缓存，200 = 返回新资源）
- `Last-Modified / If-Modified-Since`：时间戳比对，精度 1 秒
- `ETag / If-None-Match`：内容指纹，精度更高，优先级高于 Last-Modified

**缓存读取优先级**：Memory Cache > Disk Cache > 网络请求

**启发式缓存**：响应头无 max-age/expires 时，浏览器自行计算：`新鲜度 = max(0, date - last-modified) × 10%`

**SPA 最佳实践**
- HTML：`Cache-Control: no-cache` —— 每次确认是否更新
- JS / CSS / 图片：强缓存 + **hash 文件名** —— 内容变则 hash 变，触发重新请求

**webpack hash 三种类型**

| 类型 | 粒度 | 推荐场景 |
|---|---|---|
| `hash` | 项目级，任意文件改变全部重建 | 不推荐 |
| `chunkhash` | 入口文件级 | JS 入口 |
| `contenthash` | 文件内容级 | CSS 文件（避免 JS 改动导致 CSS 重建） |

**浏览器存储对比**

| 类型 | 大小上限 | 生命周期 | 适用场景 |
|---|---|---|---|
| Cookie | ≤4 KB | 可设过期时间 | 身份认证、服务端自动植入 |
| SessionStorage | 2.5–10 MB | 标签页关闭后清除 | 临时页面状态 |
| LocalStorage | 2.5–10 MB | 永久（手动清除） | token、用户偏好 |
| IndexedDB | ≥250 MB | 永久 | 大量结构化/二进制数据 |

**Chrome 刷新模式速查**
- 正常刷新（Ctrl+R）：优先读缓存
- 硬性刷新（Ctrl+Shift+R）：禁用缓存但不清空，异步资源仍可命中
- 清空缓存并硬性刷新：清空全部本地缓存后重新请求

**Service Worker 要点**：注册 → install（预缓存）→ activate（清理旧缓存）→ fetch（拦截请求，缓存回退网络）；仅在 HTTPS / localhost 可用。
...
![20211007134008.jpg](/images/s_poetries_work_md_new_1649296591473.webp)
...
![HTTP 缓存.png](/images/s_poetries_work_md_new_1649296591832.webp)
...
![代理服务器.png](/images/s_poetries_work_md_new_1649296591939.webp)
...
![20210824215429.jpg](/images/s_poetries_work_md_new_1649296782182.webp)
...
![首次访问流程.png](/images/s_poetries_work_md_new_1649296782323.webp)
...
![20210825212127.jpg](/images/s_poetries_work_md_new_1649296781881.webp)
...
![强缓存生效流程.png](/images/s_poetries_work_md_new_1649296782686.webp)
...
![20210825225010.jpg](/images/s_poetries_work_md_new_1649296782439.webp)
...
![20210826212213.jpg](/images/s_poetries_work_md_new_1649296782044.webp)
...
![客户端.png](/images/s_poetries_work_md_new_1649296782586.webp)
...
![20210827212431.jpg](/images/s_poetries_work_md_new_1649296781518.webp)
...
![协商缓存生效流程.png](/images/s_poetries_work_md_new_1649297222610.webp)
...
![20210831212257.jpg](/images/s_poetries_work_md_new_1649297221595.webp)

eTag 的优先级要高于 last-modified，当两者同时出现时，只有 eTag
会生效。只要有这两个缓存标识之一，在强缓存失效后浏览器便会携带它们向服务器发起请求，携带方式如下图请求头所示：

![20210930213501.jpg](/images/s_poetries_work_md_new_1649297222477.webp)
...
![20210901214458.jpg](/images/s_poetries_work_md_new_1649297222775.webp)

图中当判断所要处理的内容是文件 stats 对象时，将会采用上述方法生成 eTag 值，最后返回的值是由文件大小和文件最后一次修改时间组成的字符串。

而当内容非文件 stats 对象时，将采用**第二种方式：使用文件内容的 hash 值和内容长度** 。

![20210901215128.jpg](/images/s_poetries_work_md_new_1649297222336.webp)
...
![20211016164419.jpg](/images/s_poetries_work_md_new_1649297290244.webp)
...
> 补充（现代做法）：`CommonsChunkPlugin` 已在 **webpack 4** 中废弃并移除。现代项目应改用内置的 `optimization.splitChunks` 配置拆分公共模块，以及 `optimization.runtimeChunk: 'single'` 提取运行时代码，无需额外安装插件。
...
![图片2.png](/images/s_poetries_work_md_new_1649297290467.webp)
...
![20210907141210.jpg](/images/s_poetries_work_md_new_1649297377324.webp)
...
![20210907143905.jpg](/images/s_poetries_work_md_new_1649297376905.webp)
...
![20210907152933.jpg](/images/s_poetries_work_md_new_1649297377444.webp)
...
![20210907172651.jpg](/images/s_poetries_work_md_new_1649297377901.webp)
...
![20211102144051.jpg](/images/s_poetries_work_md_new_1649297377807.webp)
...
![20210908214505.jpg](/images/s_poetries_work_md_new_1649297377686.webp)
...
![20210910204624.jpg](/images/s_poetries_work_md_new_1649297463121.webp)

以上是笔者在掘金首页随便找了一个接口进行本地调用后报的跨域问题，从响应报头可以看出服务端做了访问限制：

![20210910205141.jpg](/images/s_poetries_work_md_new_1649297462579.webp)
...
![20210910215922.jpg](/images/s_poetries_work_md_new_1649297462076.webp)
...
![1610264529229005910.jpg](/images/s_poetries_work_md_new_1649297462782.webp)
...
![20210911162520.jpg](/images/s_poetries_work_md_new_1649297462991.webp)
...
![20210914220039.jpg](/images/s_poetries_work_md_new_1649297542306.webp)
...
![20210915153932.jpg](/images/s_poetries_work_md_new_1649297542507.webp)
...
![Service Worker 中间人.png](/images/s_poetries_work_md_new_1649297614344.webp)
...
![20210924220113.jpg](/images/s_poetries_work_md_new_1649297614192.webp)

当然除了上述介绍的 `install`、`activate`、`fetch` 事件，Service Worker
还有其他几个主要的事件，通过按需集成这些事件我们便可以开发强大而体验友好的渐进式 web 应用程序。

> 补充（现代做法）：在生产项目中手写 sw.js 较为繁琐且容易出错。Google 推出的 **Workbox** 库封装了常见缓存策略（CacheFirst、NetworkFirst、StaleWhileRevalidate 等），并提供 webpack/Vite 插件（`workbox-webpack-plugin`、`vite-plugin-pwa`），可大幅降低 Service Worker 的开发门槛。

这里笔者用一张图汇总了 Service Worker 的一些主要事件，大家有兴趣可以继续探索。

![Service Worker 主要事件.png](/images/s_poetries_work_md_new_1649297613659.webp)
...
![20210920112302.jpg](/images/s_poetries_work_md_new_1649297613930.webp)
...
![20210927214521.jpg](/images/s_poetries_work_md_new_1649300799429.webp)
...
![20210927220024.jpg](/images/s_poetries_work_md_new_1649300798991.webp)

之后前端调用任何同域下的接口时，浏览器会自动将网站的 cookie 值附加在请求头中传给后端进行校验，前端则不需要关心 token 的存取问题。

![20210927222425.jpg](/images/s_poetries_work_md_new_1649300798504.webp)
...
![20210927221734.jpg](/images/s_poetries_work_md_new_1649300799109.webp)
...
![20211001220900.jpg](/images/s_poetries_work_md_new_1649300799266.webp)
