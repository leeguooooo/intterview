原文链接: [https://interview.poetries.top/qa/1-%E5%89%8D%E7%AB%AF100%E9%A2%98%E8%87%AA%E6%A3%80.html](https://interview.poetries.top/qa/1-%E5%89%8D%E7%AB%AF100%E9%A2%98%E8%87%AA%E6%A3%80.html)

## 简版速记

> 面试高频考点精炼版，快速回顾 100 题核心知识点。

### 变量与作用域
- `var` 提升并初始化为 `undefined`；`let`/`const` 也提升但有**暂时性死区(TDZ)**，声明前访问抛 `ReferenceError`（Q1）
- `var` 在 `for` 循环中是全局/函数作用域；`let` 每次迭代独立块作用域（Q2）
- `var` 可重复声明；`let`/`const` 不可（Q23）
- `let x = y = 10` 中 `y` 意外成为全局变量（Q54）

### this 与箭头函数
- 箭头函数无自己的 `this`，继承外层词法作用域（Q3、Q82）；无 `prototype` 属性（Q92）
- `call` 立即调用并绑定 `this`；`bind` 返回新函数，不立即执行（Q33）

### 类型与隐式转换
- 基本类型（ES2020 起共 8 种）：`null` `undefined` `boolean` `number` `string` `symbol` `bigint` + 对象类型 `object`（Q34、Q39）
- `==` 隐式转换后比较值；`===` 同时比较值和类型（Q7）
- `new Number(3)` 是对象，`===` 与基本类型 `3` 不等（Q7）
- 6 个 falsy 值：`undefined` `null` `NaN` `0` `''` `false`；`new Number(0)` 等包装对象是 truthy（Q35）
- `typeof typeof 1` → `"string"`（Q36）

### 原型与继承
- 基本对象无原型；其他所有对象都有原型链（Q14）
- 实例共享方法应加到 `prototype`，不要加在构造函数本身（Q11）
- 子类 `constructor` 中必须先调用 `super()` 才能使用 `this`（Q66）
- `delete` 可删除原型上的属性；删后调用抛 `TypeError`（Q55）

### 事件与异步
- 事件传播三阶段：**捕获 → 目标 → 冒泡**（Q13）；`event.target` 是最深层触发元素（Q31）
- 宏任务（`setTimeout`）在调用栈清空后才执行（Q2、Q30）
- `setInterval` 返回定时器 ID，可传给 `clearInterval` 取消（Q42）
- `async` 函数始终返回 Promise（Q73）；`Promise.race` 返回最先 settle 的值（Q45）
- ES Modules `import` 在编译阶段提升执行；导入的绑定只读，不可赋值（Q57、Q67）

### 对象与内置方法
- 对象赋值是引用复制；修改一个会影响另一个（Q6）；对象作为键自动转为 `"[object Object]"`（Q29）
- `Object.defineProperty` 添加的属性默认不可枚举（Q61）
- `Object.freeze` 冻结对象，不可增删改属性（Q75）
- `JSON.stringify(obj, ['key1','key2'])` 第二参数数组白名单过滤属性（Q62）
- `Symbol` 每次创建都唯一；不可枚举，`Object.keys` 不返回（Q68、Q97）

### 数组与集合
- `push()` 返回新长度，不返回数组（Q74、Q91）
- `Set` 自动去重，类型严格区分（`'1'` ≠ `1`）（Q24、Q56）
- `for-in` 迭代对象/数组的**键**（索引）；`for-of` 迭代**值**（Q79）
- `[...'Lydia']` → `['L','y','d','i','a']`（字符串可迭代）（Q43）

### 其他高频点
- `Generator` 遇 `yield` 暂停；`next(val)` 的参数替换上一个 `yield` 的返回值（Q44、Q71）
- `sessionStorage` 关闭 tab 即清除；`localStorage` 持久（Q22）
- 剩余参数 `...args` 只能作最后一个形参（Q94）
- `return` 后单独换行会自动插入分号（ASI）（Q95）
- `||` 返回第一个真值，全假则返回最后一个（Q101）
...
![](/images/s_poetries_work_uploads_2023_03_f2061f810cc48014.webp)
...
`new Number()` 是一个内建的函数构造器。虽然它看着像是一个 number，但它实际上并不是一个真实的
...
这才会使 `member.getFullName()` 起作用。为什么这么做有益的？假设我们将这个方法添加到构造函数本身里。也许不是每个 `Person`
...
![](/images/s_poetries_work_uploads_2023_03_8e32d4b091484771.webp)
...
![](/images/s_poetries_work_uploads_2023_03_b4cea40dfd40ce5a.webp)

现在，`foo` 被调用，打印 `"First"`。

![](/images/s_poetries_work_uploads_2023_03_6ce7c2b18831d93e.webp)

`foo` 从栈中弹出，`baz` 被调用. 打印 `"Third"`。

![](/images/s_poetries_work_uploads_2023_03_dabb86afc159d9aa.webp)

WebAPI 不能随时向栈内添加内容。相反，它将回调函数推到名为 _queue_ 的地方。

![](/images/s_poetries_work_uploads_2023_03_2931de3f53e8c76e.webp)

这就是事件循环开始工作的地方。一个**事件循环** 查看栈和任务队列。如果栈是空的，它接受队列上的第一个元素并将其推入栈。

![](/images/s_poetries_work_uploads_2023_03_52f022d93a0bdb74.webp)
...
> 补充（现代做法）：ES2020 新增了 `bigint` 原始类型，现在共有 **8 种**内置类型（7 种原始类型 + `object`）。`typeof 42n === 'bigint'`。
...
> 补充（现代做法）：ES2020 起 `0n`（BigInt 零）和 `-0` 也是 falsy，实际上 falsy 值超过 6 个。面试时若提到 BigInt，可补充 `0n` 是 falsy。
...
![](/images/s_poetries_work_uploads_2023_03_bf6a8633c0503b2e.webp)
...
![](/images/s_poetries_work_uploads_2023_03_6f74ee077a3a5239.webp)

接下来我们让`person`等于`null`。

![](/images/s_poetries_work_uploads_2023_03_6b88c4fd30c36700.webp)
...
而`person`是个对象。参数`member`引用与之 _相同的_
...
```javascript
    ;[a, b] = [1, 2]
```![](/images/s_poetries_work_uploads_2023_03_4e6e1d7772ac6f28.webp)

`a`的值现在是`1`，`b`的值现在是`2`.而在题目中，我们是这么做的:
```javascript
    ;[y] = [1, 2, 3, 4, 5]
```![](/images/s_poetries_work_uploads_2023_03_35889afd8ca0aafa.webp)
