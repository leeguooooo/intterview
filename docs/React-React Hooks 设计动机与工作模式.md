...
**设计动机（Why Hooks）**
- React 公式 `UI = f(data)`，函数组件天然契合，类组件因 `this` 可变会捕获错误的 props（见 ProfilePage 案例）
- 类组件三大痛点：① `this` 指向难以预测；② 生命周期逻辑分散（相关逻辑被强行拆进不同生命周期）；③ 状态逻辑复用只能靠 HOC / Render Props，导致"嵌套地狱"
- Hooks = 让函数组件具备 state + 副作用 + 逻辑复用能力的"零部件箱"

**核心 API 速查**

| Hook | 作用 | 典型签名 |
|---|---|---|
| `useState` | 为函数组件引入单个状态 | `const [val, setVal] = useState(init)` |
| `useEffect` | 引入副作用（替代 didMount/didUpdate/willUnmount） | `useEffect(cb, deps?)` |
...
|---|---|
| `useEffect(cb)` | 每次渲染后 |
| `useEffect(cb, [])` | 仅挂载后执行一次 |
| `useEffect(cb, [a, b])` | 挂载后 + `a`/`b` 变化后 |
| `useEffect(() => { return cleanup }, deps)` | `cleanup` 在卸载（或下次 effect 执行前）被调用 |

**Hooks 使用规则（必须牢记）**
- 只能在函数组件或自定义 Hook 的**顶层**调用，不能放在条件、循环、嵌套函数中
- 只能在 React 函数中调用，不能在普通 JS 函数中调用

**Hooks 的局限（面试加分点）**
- `getSnapshotBeforeUpdate`、`componentDidCatch` 目前仍需类组件
- 过于复杂的组件用函数式拆分时，耦合与内聚的边界难以把握
- 不遵守调用规则会引发难以排查的 bug
...
不难看出，React 类组件内部预置了相当多的“现成的东西”等着你去调度/定制，state
...
![](/images/s_poetries_work_images_20210501184158.webp)
...
如果你认真阅读了我前面说过的那些话，相信你现在一定也**不仅仅能够充分理解 Dan 所想要表达的“函数组件会捕获 render
...
![](/images/s_poetries_work_images_20210501190439.webp)
...
![](/images/s_poetries_work_images_20210501190505.webp)
...
这里我给出的一个示意数组是 [num1, num2, num3]。首先需要说明，数组中的变量一般都是来源于组件本身的数据（props 或者
state）。若数组不为空，那么 React
就会在新的一次渲染后去对比前后两次的渲染，查看数组内是否有变量发生了更新（只要有一个数组元素变了，就会被认为更新发生了），并在有更新的前提下去触发
useEffect 中定义的副作用逻辑。

> 补充（现代做法）：React 18 的 `<StrictMode>` 在**开发环境**下会对每个 `useEffect` 进行"挂载 → 卸载 → 重新挂载"的双调用，用来暴露未正确清理副作用的问题。生产环境不受影响。最佳实践：始终在 `useEffect` 中返回清除函数，以确保订阅、计时器、事件监听等在组件卸载时被正确清理。
