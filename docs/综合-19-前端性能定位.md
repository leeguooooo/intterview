...
**2-5-8 原则**：2s 内=快；2-5s=可接受；5-8s=慢但可忍；>8s=放弃。**目标：力争 1s，保住 2s。**

**RAIL 模型**（以用户为中心）：

| 维度 | 阈值 | 说明 |
|------|------|------|
| Response（响应） | <100ms | 用户操作到反馈 |
| Animation（动画） | 16ms/帧 | 保证 60fps 流畅 |
| Idle（空闲） | <50ms/任务 | 主线程任务切片，不阻塞交互 |
| Load（加载） | <1s | 完成加载并可交互 |
...
**其他常用指标**：FP（首次像素变化）、FCP（首次内容绘制，好 <2s）、TTI（可交互，好 <3.8s）、TBT（总阻塞时间，好 <300ms）、Long Task（>50ms 的任务）

**性能定位工具速查**：

1. **Lighthouse** — 综合评分 + 优化建议（DevTools / npm cli / 插件均可）
2. **Chrome Performance 面板** — 火焰图定位长任务和耗时函数；Main 区域标红 Task 是重点
3. **WebPageTest** — 多地点在线测试（`webpagetest.org`，仅已发布站点）
4. **Coverage 面板** — 发现未执行的冗余 JS/CSS（Ctrl+Shift+P 搜 `coverage`）
5. **Memory 面板** — 堆快照排查内存泄漏；JS Heap 曲线持续上升 = 疑似泄漏

**Performance API 常用公式**：
...
**数据上报**：页面离开前监听 `visibilitychange` / `beforeUnload`，用 `navigator.sendBeacon()` 上报（不阻塞页面关闭，不丢包）。
...
![](/images/s_poetries_work_uploads_2022_10_e9e86c2fb3c512b6.webp)
...
![](/images/s_poetries_work_uploads_2022_10_5f72a00c10594524.webp)
...
![](/images/s_poetries_work_uploads_2022_10_371585195be23969.webp)
...
![](/images/s_poetries_work_uploads_2022_10_bb0a7833216a6f87.webp)
...
![](/images/s_poetries_work_uploads_2022_10_b5ee7ee6fdc4c8c4.webp)
...
![](/images/s_poetries_work_uploads_2022_10_ac3d40c180143d67.webp)

> 补充（现代做法）：Google 于 2024 年 3 月正式以 **INP（Interaction to Next Paint）** 替换 **FID** 作为核心 Web Vital。INP 衡量整个页面生命周期内所有交互的最差延迟（阈值：好 <200ms，差 >500ms），更全面反映交互流畅度。`web-vitals` 库 v3+ 已支持 `getINP()`，Lighthouse 12+ 也已切换到 INP。
...
![](/images/s_poetries_work_uploads_2022_10_dcbb1760bd319f3b.webp)
...
![](/images/s_poetries_work_uploads_2022_10_12a9b269a9afb096.webp)
...
![](/images/s_poetries_work_uploads_2022_10_9c205a46e33f41b8.webp)
...
![](/images/s_poetries_work_uploads_2022_10_3d382c1bde031e54.webp)
...
![](/images/s_poetries_work_uploads_2022_10_99b44fd02f1a1caa.webp)

![](/images/s_poetries_work_uploads_2022_10_1f679dee4437827e.webp)

我们发现 lighthouse 和 performance **区别还挺大** 滴，为啥呢？

![](/images/s_poetries_work_uploads_2022_10_f3ebbda38c9239a2.webp)

原来 lighthouse 默认进行了**节流处理** 。我们可以不勾选节流同时也直接点击 view trace 生成对应 performance
面板的数据。

![](/images/s_poetries_work_uploads_2022_10_3f218a80cb528b0a.webp)
...
![](/images/s_poetries_work_uploads_2022_10_5b807139fc3f7175.webp)

同时也提供可**优化** 方案

![](/images/s_poetries_work_uploads_2022_10_b9942f777f5efe2f.webp)

#### Accessibility 可访问性

可访问性：指无障碍设计，也称为网站可达性。是指所创建的网站对所有用户都可用/可访问，不管用户的生理/身体能力如何、不管用户是以何种方式访问网站。

![](/images/s_poetries_work_uploads_2022_10_beca96ece3131c1c.webp)

#### Best Practice 最佳实践

实际应用中，网站的安全问题

![](/images/s_poetries_work_uploads_2022_10_d20c9a737bff052f.webp)

#### SEO 搜索引擎优化

搜索引擎优化，是一种利用搜索引擎的规则提高网站在有关搜索引擎内的自然排名

![](/images/s_poetries_work_uploads_2022_10_ec96cb38daa7275e.webp)
...
![](/images/s_poetries_work_uploads_2022_10_7e786b5f7737a786.webp)
...
![](/images/s_poetries_work_uploads_2022_10_5fb7a8345786d90f.webp)
...
![](/images/s_poetries_work_uploads_2022_10_5a1684a3e45d1f0e.webp)

可以看到所有进行的进程，可以看到内存占用网络消耗。

### Google performance 面板

![](/images/s_poetries_work_uploads_2022_10_bf3d19cf85587472.webp)
...
![](/images/s_poetries_work_uploads_2022_10_32aaa3c8a641d859.webp)
...
![](/images/s_poetries_work_uploads_2022_10_b064f72f814a44dd.webp)
...
![](/images/s_poetries_work_uploads_2022_10_bb8cec381391697a.webp)
...
![](/images/s_poetries_work_uploads_2022_10_449880989d27a5b9.webp)
...
![](/images/s_poetries_work_uploads_2022_10_6e8ddaf30fa4bb79.webp)
...
![](/images/s_poetries_work_uploads_2022_10_3ab54f6ac6bebab2.webp)
...
![](/images/s_poetries_work_uploads_2022_10_1a488a759db184a1.webp)
...
![](/images/s_poetries_work_uploads_2022_10_30538768460964c8.webp)

Call Tree 中的内容，在 Bottom-Up 中也能看到，无明显的区别。

##### Event Log：表示事件发生的顺序列表

![](/images/s_poetries_work_uploads_2022_10_82932f6e8d6854ac.webp)
...
![](/images/s_poetries_work_uploads_2022_10_3dfe29bf9e397962.webp)

#### 1\. 缓存测试 Disable cache

![](/images/s_poetries_work_uploads_2022_10_5855d04ad84eaec2.webp)

#### 2\. 吞吐测试，模拟网速

![](/images/s_poetries_work_uploads_2022_10_5b2e43b44abf1e9f.webp)
...
![](/images/s_poetries_work_uploads_2022_10_1e046cc764cf3d02.webp)

可以看到第一个文件有`58%`没有被使用到，第二个有`95.2%`没有使用到

![](/images/s_poetries_work_uploads_2022_10_1ce647270e67ec23.webp)
...
![](/images/s_poetries_work_uploads_2022_10_88587b2e98052c2f.webp)

结束后可以查看到内存占用大小，就可以对对应的模块进行优化。

![](/images/s_poetries_work_uploads_2022_10_eb05a432c3a1905f.webp)
...
![](/images/s_poetries_work_uploads_2022_10_1646e6a466d8c822.webp)

还可以使用性能监视器，这是一个事实的监视器。

`Ctrl + Shift + P` 输入 `monitor`

![](/images/s_poetries_work_uploads_2022_10_5a780b936aef5e84.webp)
