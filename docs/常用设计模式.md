...
> 第一个参数的意义，前文已经解释过。第二个参 数name，是我们修饰的目标属性属性名，也没啥好讲的。关键就在这个 `descriptor`
...
> 补充(现代做法)：本节示例基于 Babel 的「legacy」装饰器提案，方法装饰器签名是 `(target, name, descriptor)`。该提案已被废弃。2023 年随 TC39 进入 Stage 3、并在 TypeScript 5.0 落地的是**新版装饰器**，签名完全不同——方法装饰器现在收到 `(value, context)`，其中 `context` 携带 `kind`、`name`、`addInitializer` 等元信息，不再操作 `descriptor`；类装饰器也支持 `accessor` 字段装饰器。另外 React 生态早已转向函数组件 + Hooks，类组件的 HOC 写法虽仍有效，但「逻辑复用」更多用自定义 Hook 实现；Redux 也推荐 `react-redux` 的 `useSelector`/`useDispatch` 取代 `connect`。
...
![Vue 响应式系统：Observer 劫持数据 getter/setter，Dep 收集 Watcher 依赖，数据变化时通知 Watcher 更新视图](/images/diagrams/vue-reactivity.webp)
...
![](/images/s_poetries_work_images_20210327204351.png)
...
![](/images/s_poetries_work_images_20210327204920.png)
...
> 补充(现代做法)：本节的响应式实现基于 Vue 2 的 `Object.defineProperty`，它有两个已知局限——无法侦测对象属性的「新增/删除」（需 `Vue.set`/`Vue.delete`）和数组下标/`length` 的直接修改。Vue 3 改用 ES6 `Proxy` 重写了响应式系统（`reactive`/`ref`/`effect`），天然支持新增属性与数组索引，并做了惰性递归 + 依赖收集优化。设计模式的内核仍是观察者：`effect`(订阅者) 在读取时被 `track` 收集为依赖，写入时 `trigger` 通知重跑。

> 补充(现代做法)：Vue 3 移除了实例上的 `$on`/`$emit`/`$off`，官方不再推荐用根实例当 Event Bus。组件通信改用 props/emit、`provide`/`inject` 或 Pinia；确需全局事件总线时，自己写一个 `EventEmitter`（即下一节的实现）或引入 `mitt`、`tiny-emitter` 等轻量库。
...
![观察者模式 vs 发布-订阅模式：前者目标直接通知观察者、仅减少耦合；后者发布者与订阅者经事件中心通信、完全解耦互不感知](/images/diagrams/observer-pubsub.webp)
...
![](/images/s_poetries_work_images_20210327211020.png)

**观察者模式**

![](/images/s_poetries_work_images_20210327211122.png)
...
![](/images/s_poetries_work_images_20210327212411.png)
...
![](/images/s_poetries_work_images_20210327212836.png)
