---
title: "浏览器原理面试题精选"
description: "浏览器原理面试题:渲染流程、重排重绘、事件循环、跨域、缓存、存储、垃圾回收,深入理解浏览器工作机制。"
---

## 简版速记

**跨标签页通讯**：`postMessage`、`localStorage` + `onstorage` 事件、`BroadcastChannel`（现代首选）、`SharedWorker`、`cookie` 轮询、服务端 WebSocket

**浏览器架构**：多进程模型；每个标签页独立渲染进程（含渲染引擎 + JS 引擎 + 事件触发线程 + 网络/定时器线程）

**渲染流程**：HTML → DOM 树 → CSSOM 树 → 渲染树 → 布局(Layout) → 绘制(Paint) → 合成(Composite)

**回流 vs 重绘**：回流(reflow) = 布局改变，代价高；重绘(repaint) = 仅外观变化，代价低。回流必定重绘，反之不成立。优化：批量 DOM 操作、`transform/opacity` 走 GPU 合成层、避免读取强制同步布局属性（`offsetWidth` 等）

**跨域解决方案**：CORS（主流，需服务端设置 `Access-Control-Allow-Origin`）、Nginx 反向代理、Node 中间层、JSONP（仅 GET）、`postMessage`、WebSocket；`document.domain` 已在 Chrome 101+ 禁用

**HTTP 缓存**：强缓存（`Cache-Control: max-age` / `Expires`，命中不请求服务器，200 from cache）→ 协商缓存（`ETag/If-None-Match`、`Last-Modified/If-Modified-Since`，需请求服务器，命中返回 304）；`ETag` 优先级高于 `Last-Modified`

**存储对比**：`cookie`（4K，随请求发送）< `localStorage/sessionStorage`（5M，不随请求）< `IndexedDB`（无上限）

**事件循环**：执行一个宏任务 → 清空所有微任务 → 渲染 → 下一个宏任务。微任务：`Promise.then`、`MutationObserver`、`queueMicrotask`；宏任务：`setTimeout`、`setInterval`、`script`、`I/O`

**内存泄露常见原因**：意外全局变量、未清理定时器、未移除事件监听、闭包持有大对象、DOM 节点删除后 JS 仍持有引用

**hash vs history 路由**：hash 无需服务端配合，监听 `hashchange`；history 需服务端配置 fallback，用 `pushState/replaceState` + `popstate` 监听
...
> 补充(现代做法)：`BroadcastChannel` API（Chrome 54+）是同源多标签通讯的现代首选，用法极简：`const bc = new BroadcastChannel('ch'); bc.postMessage(data); bc.onmessage = e => ...`，无需轮询或借助 localStorage。`SharedWorker` 也可作为共享状态的中间层。
...
![](/images/s_poetries_work_gitee_2020_09_98.webp)
...
![](/images/s_poetries_work_uploads_2022_08_b0f21ec685ae839b.webp)
...
![](/images/s_poetries_work_uploads_2022_08_418cd4b22ded0606.webp)
...
![](/images/s_poetries_work_uploads_2022_08_4b918458db26d17b.webp)

> 服务端拿到 `if-Modified-Since` 之后拿这个时间去和服务端资源最后修改时间做比较，如果一致则返回 `304`
> ，不一致（也就是资源已经更新了）就返回 200 和新的资源及新的 `Last-Modified`

![](/images/s_poetries_work_uploads_2022_08_aeec07248588bf72.webp)
...
![](/images/s_poetries_work_uploads_2022_08_1d8b9e11dbaca97f.webp)
...
![](/images/s_poetries_work_uploads_2022_08_053ef73d5751267e.webp)
...
![](/images/s_poetries_work_images_20210329140157.webp)
