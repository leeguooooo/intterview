原文链接: [https://interview.poetries.top/docs/excellent-docs/17-%E6%A1%86%E6%9E%B6%E9%80%9A%E8%AF%86.html](https://interview.poetries.top/docs/excellent-docs/17-%E6%A1%86%E6%9E%B6%E9%80%9A%E8%AF%86.html)

## 简版速记

### MVVM 核心
- **M-V-VM**：Model(数据) ↔ ViewModel(双向绑定桥) ↔ View(界面)
- **Vue2 数据劫持**：`Object.defineProperty` setter/getter + Dep/Watcher 发布订阅；缺陷：无法检测数组索引变化、新增属性需 `Vue.set`
- **Vue3 升级**：改用 `Proxy` 拦截整个对象，原生支持数组及新增属性，无需深度递归初始化
- **Angular 脏检查**：`$digest` 循环遍历 watcher，至少 2 次至多 10 次

### 路由原理
| 模式 | 原理 | 服务端是否需要配置 |
|------|------|-----------------|
| hash | `location.hash` + `hashchange` 事件，`#` 后内容不发送给服务器 | 不需要 |
| history | `pushState/replaceState` + `popstate` 事件，URL 更美观 | 需配置 fallback 到 index.html，否则刷新 404 |

### Virtual DOM
- JS 对象模拟 DOM 树 → diff 算出最小变更 → patch 真实 DOM
- 完整树 diff 时间复杂度 O(n³)；React 限同层比较降为 **O(n)**
- 列表渲染必须加唯一 `key`，否则退化为全量替换

### Diff 算法三代对比（面试高频）
| 版本 | 算法 | 核心思路 |
|------|------|---------|
| React | 递增法 | 遍历新列表，记录其节点在旧列表的位置；不递增则移动；用 Map 将复杂度降至 O(n) |
| Vue2 | 双端比较 | 头头/尾尾/头尾/尾头四指针轮流对比，命中即移动指针，减少 DOM 操作次数 |
| Vue3 | 最长递增子序列 (LIS) | 前后置预处理跳过相同节点，剩余部分通过 LIS 确定最少需移动的节点集合 |

> 答题顺序建议：为什么需要 Virtual DOM → diff 复杂度优化 → 三代算法演进对比
...
> 补充(现代做法)：Vue 3（2020 年正式发布）已全面切换到 `Proxy`，Vue 2 的 `Object.defineProperty` 方案随之成为遗产代码。使用 Vue 3 时不再需要 `Vue.set` 来触发响应式，直接对属性赋值或向数组 push 均可被追踪。
```js
...
![](/images/s_poetries_work_gitee_2020_09_99.webp)

> `History`模式是 `HTML5` 新推出的功能，比之 `Hash URL` 更加美观

![](/images/s_poetries_work_gitee_2020_09_100.webp)
...
![](/images/s_poetries_work_images_202203211358420.webp)
...
![](/images/s_poetries_work_images_202203211358473.webp)
...
![](/images/s_poetries_work_images_202203211358728.webp)
...
![](/images/s_poetries_work_images_202203211358228.webp)
...
![](/images/s_poetries_work_images_202203211358441.webp)
```js
    function reactDiff(prevChildren, nextChildren, parent) {
        let lastIndex = 0
        for (let i = 0; i < nextChildren.length; i++) {
...
![](/images/s_poetries_work_images_202203211359786.webp)
...
![](/images/s_poetries_work_images_202203211359054.webp)
...
![](/images/s_poetries_work_images_202203211359533.webp)
...
![](/images/s_poetries_work_images_202203211359184.webp)
...
![](/images/s_poetries_work_images_202203211359531.webp)
...
![](/images/s_poetries_work_images_202203211359343.webp)
...
![](/images/s_poetries_work_images_202203211359968.webp)
```js
    function vue2Diff(prevChildren, nextChildren, parent) {
      //...
      while (oldStartIndex <= oldEndIndex && newStartIndex <= newEndIndex) {
...
![](/images/s_poetries_work_images_202203211359489.webp)
...
![](/images/s_poetries_work_images_202203211400801.webp)
```js
    function vue2Diff(prevChildren, nextChildren, parent) {
      //...
      while (oldStartIndex <= oldEndIndex && newStartIndex <= newEndIndex) {
...
最后当**旧列表** 遍历到`undefined`时就跳过当前节点。
```js
    function vue2Diff(prevChildren, nextChildren, parent) {
      //...
      while (oldStartIndex <= oldEndIndex && newStartIndex <= newEndIndex) {
...
![](/images/s_poetries_work_images_202203211400020.webp)

这个例子非常简单，几次循环都是尾节点相同，尾指针一直向前移动，直到循环结束，如下图

![](/images/s_poetries_work_images_202203211400324.webp)
...
剩余的节点删除即可。这里我们需要注意，**旧列表** 的`undefined`。在第二小节中我们提到过，当头尾节点都不相同时，我们会去**旧列表**
中找**新列表** 的第一个节点，移动完DOM节点后，将**旧列表**
的那个节点改为`undefined`。所以我们在最后的删除时，需要注意这些`undefined`，遇到的话跳过当前循环即可。
```js
    function vue2Diff(prevChildren, nextChildren, parent) {
      //...
      while (oldStartIndex <= oldEndIndex && newStartIndex <= newEndIndex) {
...
![](/images/s_poetries_work_images_202203211400687.webp)
...
![](/images/s_poetries_work_images_202203211400875.webp)
...
![](/images/s_poetries_work_images_202203211400253.webp)
...
![](/images/s_poetries_work_images_202203211400806.webp)
...
![](/images/s_poetries_work_images_202203211400840.webp)
...
![](/images/s_poetries_work_images_202203211401709.webp)

首先 `7 < 13` 那么`7`对应的长度就是`13`的长度再加1，`1 + 1 = 2`

![](/images/s_poetries_work_images_202203211401591.webp)

继续，我们对比`8`。我们首先和`7`比，发现不满足递增，但是没关系我们还可以继续和`13`比，`8 <
13`满足递增，那么`8`的长度也是`13`的长度在加一，长度为`2`

![](/images/s_poetries_work_images_202203211401764.webp)
...
![](/images/s_poetries_work_images_202203211401780.webp)

之后的计算依次类推，最后的结果是这样的

![image-20210220201751694](/images/s_poetries_work_images_image_20210220201751694.webp)
