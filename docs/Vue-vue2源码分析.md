...
**初始化流程**
- `new Vue()` → `_init()` → `$mount()` → `mountComponent()` → `new Watcher()` → `render()` → `_update()` → `patch()`

**数据响应式核心**
- `Object.defineProperty` 劫持 getter/setter；getter 收集依赖(Dep)，setter 触发通知
- `Observer`：递归遍历对象属性，调用 `defineReactive`
- `Dep`：依赖收集容器，维护 Watcher 列表，变更时 `notify()` 通知所有 Watcher
- `Watcher`：三类——渲染 Watcher、computed Watcher、user Watcher(`$watch`)

**数组响应式**
- 无法用 `defineProperty` 拦截下标赋值或 `length` 变更
- 重写7个变异方法：`push/pop/shift/unshift/splice/sort/reverse`
- 直接 `arr[index] = val` 或 `arr.length = 0` 不触发更新，需用 `Vue.set()`

**异步更新队列**
- 同一事件循环内多次修改同一 watcher，只入队一次(去重)
- `nextTick` 降级策略：`Promise.then` → `MutationObserver` → `setImmediate` → `setTimeout`

**虚拟 DOM & diff**
- `patch` 三步：新无旧 → 删；旧无新 → 增；都有 → diff(`patchVnode`)
- `updateChildren` 双端比较：oldStart/oldEnd 与 newStart/newEnd 四指针，减少移动次数
- `key` 的作用：加速 `sameVnode` 命中，避免原地复用导致的渲染错误

**模板编译三阶段**
1. `parse`：模板 → AST
2. `optimize`：标记静态子树，patch 时跳过，提升性能
3. `generate`：AST → render 函数字符串

**组件化**
- `Vue.component()` 内部调用 `Vue.extend()` 生成子类构造函数
- 组件 VNode 通过 `createComponent` 创建，patch 时调用 `init` hook 实例化并挂载

## 文件结构

![](/images/s_poetries_work_images_20210313165151.webp)

**源码目录**

![](/images/s_poetries_work_images_20210313165238.webp)
...
> 补充(现代做法): Vue 3 将 `Object.defineProperty` 替换为 `Proxy`，可直接拦截属性新增/删除及数组下标变更，不再需要 `Vue.set()/Vue.delete()`，也无需递归遍历全部属性，初始化性能更优。
...
> 补充(现代做法): Vue 3 基于 `Proxy`，数组下标赋值(`arr[0] = val`)和 `arr.length = 0` 均可被原生拦截，无需重写原型方法，`Vue.set()`/`Vue.delete()` 已从 Vue 3 API 中移除。
```js
    data: {
      arr: []
    }
...
![](/images/s_poetries_work_images_20210313171859.webp)
...
![](/images/s_poetries_work_images_20210313171946.webp)
...
![](/images/s_poetries_work_images_20210313172248.webp)
...
![](/images/s_poetries_work_images_20210313173014.webp)
...
![](/images/s_poetries_work_images_20210313173135.webp)
```javascript
    // patchVnode过程分解
    // 1.div#demo // 2.h1
    // 3.text
...
![](/images/s_poetries_work_images_20210313174344.webp)
...
![](/images/s_poetries_work_images_20210313174730.webp)

> 但是，当结束时`newStartIdx > newEndIdx`时，说明新的VNode节点已经遍历完了，但是老的节点还有 剩余，需要从文档中删
> 的节点删除。

![](/images/s_poetries_work_images_20210313174753.webp)

## 模板编译

模板编译的主要目标是将模板**(template)**转换为渲染函数**(render)**

![](/images/s_poetries_work_images_20210313174858.webp)
```javascript
    template => render()
```
...
![](/images/s_poetries_work_images_20210313175206.webp)
...
![](/images/s_poetries_work_images_20210313175323.webp)
...
![](/images/s_poetries_work_images_20210313175652.webp)
...
![](/images/s_poetries_work_images_20210313175757.webp)
