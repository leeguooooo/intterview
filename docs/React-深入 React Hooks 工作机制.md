...
**两条黄金规则**
- 只在 React **函数组件**或**自定义 Hook** 中调用 Hook
- 不在循环、条件、嵌套函数中调用 Hook

**为什么？底层是单向链表，不是数组**
- 首次渲染：`mountState` 按调用顺序把每个 Hook 对象追加到链表尾部（构建链表）
- 后续渲染：`updateState` 按固定顺序遍历链表取值（消费链表）
- Hook 没有"名字"，React 靠**位置（顺序）**区分不同 Hook
- 若某次渲染跳过了一个 Hook 调用，链表位置错位，后面所有 Hook 取到的都是错误状态

**面试一句话**
> Hooks 的本质是挂在 Fiber 节点上的单向链表；每次渲染必须按相同顺序调用相同数量的 Hook，否则 `updateState` 按位置取值时会读到错误的 `memoizedState`。

**常见追问**
- "Hooks 的本质是数组吗？" → 不是，是**链表**（行为类似数组按索引取值，但数据结构是 `{ memoizedState, next }` 的链表节点）
- "ESLint 怎么检测？" → `eslint-plugin-react-hooks` 的 `rules-of-hooks` 规则在静态分析阶段就能识别条件/循环内的 Hook 调用并报错
- `mountState` 返回 `[hook.memoizedState, dispatch]`，即常见的 `[state, setState]` 解构
...
> 补充（现代做法）：React 18 以上项目通常通过 `eslint-plugin-react-hooks`（`rules-of-hooks` 规则）在编译期强制执行这两条规则，违规直接报 Error，无需等到运行时才发现乱序问题。React 19 引入的 React Compiler（原名 React Forget）进一步在编译阶段静态分析 Hook 调用顺序，为符合规则的组件自动生成记忆化代码；若检测到违规，编译器会跳过该组件并发出警告。
...
![](/images/s_poetries_work_images_20210428195211.webp)

**注意** ，你在自己电脑上模仿这段代码的时候，千万不要漏掉 if 语句里面`// eslint-disable-next-
...
![](/images/s_poetries_work_images_20210428195323.webp)
...
![](/images/s_poetries_work_images_20210428195406.webp)
...
![](/images/s_poetries_work_images_20210428195419.webp)
...
![](/images/s_poetries_work_images_20210428195616.webp)
...
从这段源码中我们可以看出，`mountState` 的主要工作是初始化 `Hooks`。在整段源码中，最需要关注的是
...
![](/images/s_poetries_work_images_20210428195756.webp)
...
![](/images/s_poetries_work_images_20210428195911.webp)
...
![](/images/s_poetries_work_images_20210428195933.webp)
...
![](/images/s_poetries_work_images_20210428200029.webp)
