...
- **工作流程**：首次访问 → 服务器 `Set-Cookie` 响应头下发 → 浏览器保存 → 后续请求自动携带 `Cookie` 请求头。
- **多值发送**：服务器可设置多个 `Set-Cookie`；浏览器回传时用 `;` 分隔在同一行发送。
- **浏览器绑定**：Cookie 由浏览器存储，换浏览器/换设备即失效，需重新走 `Set-Cookie` 流程。
- **有效期**：`Expires`（绝对时间点）/ `Max-Age`（相对秒数）；两者同时存在时 **`Max-Age` 优先**；都不设则为会话 Cookie，关闭浏览器即失效。
- **作用域**：`Domain` 限定域名，`Path` 限定路径；不满足则不发送该 Cookie。
- **安全三件套**：
  - `HttpOnly`：禁止 JS 通过 `document.cookie` 读写，防 XSS 窃取。
  - `SameSite`：防 CSRF；`Strict` 完全禁止跨站，`Lax` 允许顶级导航 GET，`None` 需配合 `Secure`。
  - `Secure`：仅 HTTPS 传输，但 Cookie 本身明文存储在浏览器中。
- **典型用途**：会话状态（登录保持）、广告跟踪（第三方 Cookie）。
...
![](/images/s_poetries_work_gitee_2019_12_24.webp)
...
![](/images/s_poetries_work_gitee_2019_12_25.webp)
...
> 补充(现代做法)：Chrome 80（2020年2月）起将 `SameSite` 的**默认值从无限制改为 `Lax`**，无属性的 Cookie 不再随第三方请求发送。跨站场景确需携带 Cookie 时，需显式设置 `SameSite=None; Secure`（必须同时开启 HTTPS）。Safari 和 Firefox 也有类似默认策略。
...
> 补充(现代做法)：主流浏览器正在逐步淘汰第三方 Cookie。Safari 的 ITP（Intelligent Tracking Prevention）已默认阻止；Chrome 在隐私沙盒（Privacy Sandbox）框架下推进替代方案（如 Topics API）。第三方 Cookie 的广告跟踪能力正在被平台层面收紧，开发者应避免依赖第三方 Cookie 实现跨站身份识别。
...
![](/images/s_poetries_work_gitee_2019_12_104.webp)
