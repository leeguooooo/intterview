...
![](/images/s_poetries_work_gitee_2020_07_guide_25.png)
...
> 补充（现代做法）：上面示例里的 `webpack.optimize.UglifyJsPlugin` 在 webpack 4+ 已废弃，压缩交给内置的 `TerserPlugin`，开启 `mode: 'production'` 即默认压缩、Tree Shaking。如今新项目更常用 **Vite**（基于 esbuild/Rollup，启动毫秒级、按需编译）或 **esbuild/SWC/Rollup** 系工具链；脚手架直接 `npm create vite@latest`。面试时建议补一句：理解构建工具要点（模块化打包、语法编译、压缩、Tree Shaking、HMR）是通用的，具体工具从 Gulp/Grunt → webpack → Vite 一路演进，重点答清「解决了什么问题」即可。
...
![](/images/s_poetries_work_gitee_2020_07_guide_26.png)
...
> 补充（现代做法）：文中描述的是早期生态，如今格局已变。**Angular** 指的是基于 TypeScript 重写的 Angular 2+（旧的 AngularJS 1.x 已停止维护），其余主流变化：React 推出 Hooks（函数组件 + `useState/useEffect`）与并发渲染，配合 Next.js 做 SSR/SSG/RSC；Vue 进入 3.x，组合式 API（`<script setup>`）+ Vite，配套 Pinia（取代 Vuex）；跨端除 React Native 外，还有 Flutter、Taro、uni-app。面试核心仍是讲清「为什么需要框架」：声明式 UI、组件化、虚拟 DOM/响应式带来的数据驱动视图，以及它们解决的开发效率与可维护性问题。「纯展示页用框架反而更慢」的结论在 SSR/水合（hydration）和岛屿架构（Islands，如 Astro）普及后已不绝对，需结合具体场景判断。
