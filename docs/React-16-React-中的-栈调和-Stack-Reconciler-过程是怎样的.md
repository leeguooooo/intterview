...
- **调和 ≠ Diff**：调和（Reconciliation）是将虚拟 DOM 映射到真实 DOM 的完整过程；Diff 只是其中”找不同”的环节。
- **栈调和 = React 15 的 Diff 算法**，以同步递归方式运行，一旦开始无法中断，深树场景会阻塞主线程。
- **O(n³) → O(n) 的三大前提**：
  1. **分层对比**：只比较同层节点，放弃跨层级比较；
  2. **类型优先**：类型不同直接替换，不再往深处递归；
  3. **key 复用**：key 充当节点唯一标识，帮助同层节点稳定复用。
- **跨层级操作代价高**：React 无法识别”移动”操作，只能销毁 + 重建，官方建议保持 DOM 结构稳定。
- **key 必须唯一且稳定**：用数组 `index` 作 key，在节点插入/删除场景下会退化为无 key 的低效行为。
...
> new
> window)](https://github.com/facebook/react/tree/15-stable/src/renderers/shared/stack/reconciler)这个路径，`调和器所做的工作是一系列的，包括组件的挂载、卸载、更新等过程，其中更新过程涉及对
> Diff 算法的调用`。
...
> 补充（现代做法）：React 16 用 **Fiber 调和**彻底取代了栈调和。栈调和是同步递归、不可中断的，深树渲染会长时间占用主线程导致卡顿；Fiber 将渲染工作拆分为可中断的”工作单元”，支持优先级调度和时间切片（Time Slicing），这也是 React 18 并发模式（Concurrent Mode）的底层基础。面试时理解两者的本质差异——**同步不可中断 vs 异步可中断**——是加分点。
...
具体来说，若一张页面中有 100 个节点（这样的情况在实际开发中并不少见），100³ 算下来就有百万次操作了，这还只是一次 Diff
...
团队结合设计层面的一些推导，总结了以下两个规律， 为将 O (n3) 复杂度转换成 O (n) 复杂度确立了大前提：`
  ...
  * 处于同一层级的一组子节点，可以通过设置 `key` 作为唯一标识，从而维持各个节点在不同渲染过程中的稳定性。
...
![](/images/s_poetries_work_images_20210429221559.webp)
...
![](/images/s_poetries_work_images_20210429221957.webp)
...
![](/images/s_poetries_work_images_20210429222131.webp)
...
![](/images/s_poetries_work_images_20210429222302.webp)
...
![](/images/s_poetries_work_images_20210429222911.webp)
