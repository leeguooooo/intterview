...
- **核心问题**：传统 JSX 转换（`createElement` / automatic runtime）对每个元素都分配大量短暂对象，在 SSR 场景下产生大量 GC 压力。
- **解决方案**：`precompile` JSX 转换——静态部分直接编译为 HTML 字符串，动态部分通过类 Tagged Template Literal 的 `jsxTemplate(statics, ...exprs)` 形式传入。
- **安全保障**：动态内容自动包裹 `jsxEscape()`，动态属性使用 `jsxAttr()`，XSS 转义内置于转换中，框架必须提供实现。
- **组件回退**：自定义组件（大写开头）无法静态化，仍回退到 `jsx(Foo, props)` 的 automatic runtime 形式。
- **性能收益**：合成基准快 **7–20 倍**，真实网站实测约 **8 倍**，GC 时间减少约 **50%**；最坏情况（全动态）与原转换持平，绝不会更慢。
- **框架支持**：Preact、Hono JSX 已默认支持；Deno 通过 `deno.json` 中将 `"jsx": "react-jsx"` 改为 `"jsx": "precompile"` 一行启用。
- **面试要点**：这种思路并不新鲜——传统服务端模板语言（PHP/Jinja/ERB）一直如此工作；JSX precompile 只是将相同思想引入 JSX 生态，且框架无关。
...
> **补充（现代做法）**：React 19 + React Compiler（2024–2025 年稳定发布）从另一个角度解决相同问题——编译器在构建时静态分析组件，自动跳过不必要的 re-render 和对象分配，但 SSR 字符串化路径的优化思路与本文的 precompile 方案如出一辙。Next.js App Router 的 React Server Components 模式在服务端根本不生成 VDOM，直接流式输出 HTML；Remix / React Router v7 也走了类似路线。如果你今天在非 Deno 环境（Bun / Node.js）使用 Preact 或 Hono，可通过各自的构建工具（esbuild 插件 / tsc）配置 `jsxImportSource` 来启用 precompile，无需依赖 Deno 运行时。
