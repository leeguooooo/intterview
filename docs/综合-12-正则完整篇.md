...
> 正则 = 匹配字符 + 匹配位置，两者叠加就能解决绝大部分问题。

| 类别 | 常用语法 | 含义 |
|------|----------|------|
| **字符组** | `[abc]` `[a-z]` `[^abc]` | 任选其一 / 范围 / 排除 |
| **简写** | `\d\D` `\w\W` `\s\S` `.` | 数字/非数字、单词/非单词、空白/非空白、任意字符 |
| **量词（贪婪）** | `{m,n}` `?` `+` `*` | 重复次数；默认贪婪（尽量多匹配） |
| **量词（惰性）** | `{m,n}?` `??` `+?` `*?` | 量词后加 `?` 变惰性（尽量少匹配） |
| **锚点/位置** | `^` `$` `\b` `\B` `(?=p)` `(?!p)` | 开头/结尾/单词边界/非边界/正向先行/负向先行 |
| **分组** | `(p)` `(?:p)` `\1` | 捕获 / 非捕获 / 反向引用 |
| **分支** | `p1\|p2` | 分支惰性：前一项匹配成功即不再尝试后续 |

**高频面试考点：**
- 贪婪 vs 惰性：`.*` 贪婪，`.*?` 惰性
- 千位分隔符：`/(?!^)(?=(\d{3})+$)/g`
- 密码校验（至少含两种字符）：`/((?=.*[0-9])(?=.*[a-z])|(?=.*[0-9])(?=.*[A-Z])|(?=.*[a-z])(?=.*[A-Z]))^[0-9A-Za-z]{6,12}$/`
- 反向引用确保分隔符一致：`/\d{4}(-|\/|\.)\d{2}\1\d{2}/`
- 非捕获分组 `(?:p)`：只分组不捕获，不影响 `$1` 序号
- 回溯策略：贪婪→吐出；惰性→索取；分支→换家
...
![](/images/s_poetries_work_uploads_2023_12_ed05c12bd5dedc75.webp)
...
> 补充（现代做法）：ES2018 起 JS 正式支持**后行断言**（lookbehind）：`(?<=p)` 正向后行、`(?<!p)` 负向后行，表示某位置**前面**是/不是 p。例如 `(?<=\$)\d+` 匹配美元符号后面的数字。Chrome 62+、Node.js 9.11.2+ 均已支持，可安全使用。同版本还引入了**具名捕获组** `(?<name>...)` 及 `\k<name>` 反向引用，可读性大幅提升。
...
![](/images/s_poetries_work_uploads_2023_12_da1447013f155fa8.webp)

> 而当目标字符串是"abbbc"时，就没有所谓的“回溯”。其匹配过程是：

![](/images/s_poetries_work_uploads_2023_12_03c4b5f670eb7117.webp)
...
![](/images/s_poetries_work_uploads_2023_12_907643611caaa74c.webp)
...
![](/images/s_poetries_work_uploads_2023_12_241c70dd801d6291.webp)

  * 目标字符串是：`"acd"ef`，匹配过程是

![](/images/s_poetries_work_uploads_2023_12_881a9bbc0b51f582.webp)
...
![](/images/s_poetries_work_uploads_2023_12_ce3dd9321ba55a9c.webp)

> 目标字符串是"12345"，匹配过程是

![](/images/s_poetries_work_uploads_2023_12_d9509244020d7ce1.webp)
...
![](/images/s_poetries_work_uploads_2023_12_21cf73e992373694.webp)

目标字符串是"candy"，匹配过程

![](/images/s_poetries_work_uploads_2023_12_aad9216afc57f661.webp)
...
![](/images/s_poetries_work_uploads_2023_12_b2cbf9f6bd52e905.webp)
...
> 补充（现代做法）：ES2020 引入了 `String.prototype.matchAll(regex)`，regex 必须带 `g` 标志，返回一个迭代器，每次迭代都给出含 `index`、`groups`（具名捕获）的完整匹配对象，彻底替代手动循环 `exec` 的写法。示例：`for (const m of str.matchAll(/\b(\d+)\b/g)) console.log(m[1], m.index)`。Node.js 12+、Chrome 73+ 支持。
