...
**渲染链路三阶段**：`ReactDOM.render` 调用栈以 `scheduleUpdateOnFiber` 和 `commitRoot` 为界，分为 **初始化 → render → commit** 三段。

**初始化阶段**：
- `legacyRenderSubtreeIntoContainer` 创建两个关键实体：`fiberRoot`（FiberRootNode，关联真实 DOM 容器）和 `rootFiber`（FiberNode，虚拟 DOM 根节点）；两者通过 `fiberRoot.current === rootFiber` 相连。
- 最终通过 `unbatchedUpdates` → `updateContainer` → `scheduleUpdateOnFiber` 进入 render 阶段。

**render 阶段**（`performSyncWorkOnRoot` 开启，同步执行）：
- **双缓冲**：`createWorkInProgress` 以 current（rootFiber）为模板创建 workInProgress 树，两棵树通过 `alternate` 互指。
- **深度优先遍历**：`workLoopSync` 循环调用 `performUnitOfWork`，每个节点先走 `beginWork`（递，创建子 Fiber 节点），无子节点时走 `completeWork`（归，创建真实 DOM 节点并收集副作用）。
- **节点连接**：Fiber 节点通过 `child / return / sibling` 三个指针连接，数据结构本质是链表而非树。
- **effectList**：`completeUnitOfWork` 自底向上将有副作用的节点串成链表，最终汇聚在 rootFiber 上，供 commit 阶段直接消费。

**commit 阶段**（`commitRoot` 开启，**绝对同步**）：
- **before mutation**：DOM 未渲染，触发 `getSnapshotBeforeUpdate`，调度 `useEffect`。
- **mutation**：遍历 effectList，按 `flags`（旧称 effectTag）执行 DOM 增删改。
- **layout**：调用 `componentDidMount / componentDidUpdate`、`useLayoutEffect` 回调；将 `fiberRoot.current` 指针切换到 workInProgress 树（双缓冲交换）。

**同步 vs 异步的本质**：Fiber 节点的 `mode` 属性决定渲染模式。`ReactDOM.render` 对应 legacy 模式，`requestUpdateLane` 返回 `SyncLane`，走 `performSyncWorkOnRoot`（同步）；`ReactDOM.createRoot` 对应 concurrent 模式，返回异步 lane，走调度器（异步可中断）。Fiber 架构本身同时兼容两种模式。
...
![](/images/s_poetries_work_images_20210501195335.webp)

现在请你打开 Chrome 的 `Performance` 面板，点击下图红色圈圈所圈住的这个“记录”按钮：

![](/images/s_poetries_work_images_20210501195353.webp)

然后重新访问 Demo 页面对应的本地服务地址，待页面刷新后，终止记录，便能够得到如下图右下角所示的这样一个调用栈大图：

![](/images/s_poetries_work_images_20210501195414.webp)

放大该图，定位“`src/index.js`”这个文件路径，我们就可以找到 `ReactDOM.render` 方法对应的调用栈，如下图所示：

![](/images/s_poetries_work_images_20210501195521.webp)
...
![](/images/s_poetries_work_images_20210501195705.webp)
...
![](/images/s_poetries_work_images_20210501200309.webp)
...
![](/images/s_poetries_work_images_20210501201153.webp)

在这个流程中，你需要关注到 `fiberRoot` 这个对象。`fiberRoot` 到底是什么呢？这里我将运行时的 `root 和
fiberRoot`为你截取出来，其中 root 对象的结构如下图所示：

![](/images/s_poetries_work_images_20210501201231.webp)

可以看出，root 对象（`container._reactRootContainer`）上有一个 `_internalRoot` 属性，这个
`_internalRoot` 也就是 `fiberRoot`。fiberRoot 的本质是一个 FiberRootNode 对象，其中包含一个
current 属性，该属性同样需要划重点。这里我为你高亮出 current 属性的部分内容：

![](/images/s_poetries_work_images_20210501201327.webp)
...
![](/images/s_poetries_work_images_20210501201544.webp)
...
在 `unbatchedUpdates` 函数体里，当下你只需要 Get 到一个信息：`它直接调用了传入的回调 fn`。而在当前链路中，fn 是什么呢？fn
...
函数体中的 `scheduleWork` 其实就是 `scheduleUpdateOnFiber`，`scheduleUpdateOnFiber`
...
![](/images/s_poetries_work_images_20210501211335.webp)
...
> 而现在，我相信你心里更多的疑惑在于：`都说 Fiber 架构带来的异步渲染是 React 16 的亮点，为什么分析到现在，竟然发现
...
> 补充（现代做法）：React 18 正式发布后，模式已收敛为两种：**legacy 模式**（`ReactDOM.render`，已标记为 deprecated，React 18 仍保留但会打印警告）和 **concurrent 模式**（`ReactDOM.createRoot(rootNode).render(<App />)`，现为推荐的默认方式）。**blocking 模式已被完全移除**，`unstable_createRoot` 也已升级为稳定 API `ReactDOM.createRoot`。新项目应直接使用 `createRoot`。
...
![](/images/s_poetries_work_images_20210501212348.webp)

乍一看，好像和 `ReactDOM.render` 差别很大，其实不然。图中 `createRoot`
所触发的逻辑仍然是一些准备性质的初始化工作，此处不必太纠结。关键在于下面我给你框出来的这部分，如下图所示

![](/images/s_poetries_work_images_20210501212616.webp)

我们拉近一点来看，如下图所示：

![](/images/s_poetries_work_images_20210501212641.webp)
...
![](/images/s_poetries_work_images_20210501212753.webp)

在异步渲染模式下，由于请求到的 `lane` 不再是 `SyncLane`（同步优先级），故不会再走到 `performSyncWorkOnRoot`
...
![](/images/s_poetries_work_images_20210501214149.webp)

图中，`performSyncWorkOnRoot` 标志着 `render 阶段`的开始，`finishSyncRender` 标志着
...
![](/images/s_poetries_work_images_20210501214428.webp)
...
![](/images/s_poetries_work_images_20210501214533.webp)

紧随其后的是 `prepareFreshStack`，这里不卖关子，`prepareFreshStack`
...
![](/images/s_poetries_work_images_20210501214757.webp)
...
![](/images/s_poetries_work_images_20210501215239.webp)
...
`workLoopSync` 做的事情就是通过 `while` 循环反复判断 `workInProgress` 是否为空，并在不为空的情况下针对它执行
`performUnitOfWork` 函数。

> 而 `performUnitOfWork` 函数将触发对 `beginWork` 的调用，进而实现对新 `Fiber` 节点的创建。若
> `beginWork` 所创建的 `Fiber` 节点不为空，则 `performUnitOfWork` 会用这个新的 `Fiber` 节点来更新
...
![](/images/s_poetries_work_images_20210501215505.webp)
...
![](/images/s_poetries_work_images_20210501215910.webp)
...
`Placement` 这个 `effectTag` 的意义，`是在渲染器执行时，也就是真实 DOM 渲染时，告诉渲染器：我这里需要新增 DOM 节点`。
...
![](/images/s_poetries_work_images_20210501220545.webp)
...
![](/images/s_poetries_work_images_20210501220749.webp)
...
![](/images/s_poetries_work_images_20210501220845.webp)

`reconcileSingleElement` 将基于 `rootFiber` 子节点的 `ReactElement` 对象信息，创建其对应的
FiberNode。这个过程中涉及的函数调用如下图高亮处所示：

![](/images/s_poetries_work_images_20210501220908.webp)
...
![](/images/s_poetries_work_images_20210501221024.webp)
...
![](/images/s_poetries_work_images_20210501221222.webp)
...
![](/images/s_poetries_work_images_20210501221423.webp)
...
![](/images/s_poetries_work_images_20210501221451.webp)

class 为 `container` 的 DOM 元素对应的节点，其内容如下图所示

![](/images/s_poetries_work_images_20210501221508.webp)

h1 标签对应的节点

第 1 个 p 标签对应的 FiberNode，内容为“我是第一段话”，如下图所示

![](/images/s_poetries_work_images_20210501221520.webp)

第 2 个 p 标签对应的 FiberNode，内容为“我是第二段话”，如下图所示

![](/images/s_poetries_work_images_20210501221536.webp)
...
![](/images/s_poetries_work_images_20210501221636.webp)
...
这里我以 h1 这个元素对应的 Fiber 节点为例，给你展示下它是如何与其他节点相连接的。展开这个 Fiber 节点，对它的 child、
return、sibling 3 个属性作截取，如下图所示：

child 属性为 null，说明 h1 节点没有子 Fiber 节点：

![](/images/s_poetries_work_images_20210501221716.webp)

return 属性局部截图：

![](/images/s_poetries_work_images_20210501221727.webp)

sibling 属性局部截图：

![](/images/s_poetries_work_images_20210501221737.webp)
...
![](/images/s_poetries_work_images_20210501221811.webp)
...
![](/images/s_poetries_work_images_20210502090820.webp)

从图上我们需要把握住的一个信息是，从 `performUnitOfWork` 到 `completeWork`，中间会经过一个这样的调用链路：

![](/images/s_poetries_work_images_20210502090847.webp)

其中 `completeUnitOfWork` 的工作也非常关键，但眼下我们先拿 `completeWork` 开刀，你可以暂时将
...
![](/images/s_poetries_work_images_20210502091049.webp)

显然，第一个进入 `completeWork` 的节点是 `h1`，这也符合我们上一讲所构建出来的 `Fiber` 树中的节点关系，如下图所示：

![](/images/s_poetries_work_images_20210502091118.webp)
...
![](/images/s_poetries_work_images_20210502091323.webp)
...
捋顺思路后，我们直接来提取知识点。关于 completeWork，你需要明白以下几件事。
  ...
  * 创建好的 DOM 节点会被赋值给 workInProgress 节点的 stateNode 属性。也就是说当我们想要定位一个 Fiber 对应的 DOM 节点时，访问它的 stateNode 属性就可以了。这里我们可以尝试访问运行时的 h1 节点的 stateNode 属性，结果如下图所示： ![](/images/s_poetries_work_images_20210502091538.webp)
  * 将 DOM 节点插入到 DOM 树的操作是通过 `appendAllChildren` 函数来完成的

说是将 DOM 节点插入到 DOM 树里去，实际上是将子 Fiber 节点所对应的 DOM 节点挂载到其父 Fiber 节点所对应的 DOM
...
![](/images/s_poetries_work_images_20210502091827.webp)
...
![](/images/s_poetries_work_images_20210502091913.webp)
...
![](/images/s_poetries_work_images_20210502092000.webp)
...
![](/images/s_poetries_work_images_20210502092206.webp)
...
![](/images/s_poetries_work_images_20210502092259.webp)
...
> 补充（现代做法）：React 18 中 **effectList 链表机制已被移除**，改为在每个 Fiber 节点上用 `subtreeFlags` 位掩码标记子树是否含有副作用。commit 阶段通过递归检查 `subtreeFlags` 来定位需要处理的节点，避免了维护链表的额外开销。老版本资料中对 `firstEffect / lastEffect / nextEffect` 的引用在 React 18 源码中已不存在。
...
![](/images/s_poetries_work_images_20210502092337.webp)

这里的入参 root 并不是 rootFiber，而是 fiberRoot（FiberRootNode）实例。fiberRoot 的 current
