...
- **三阶段**：parsing（babylon/`@babel/parser` 生成 AST）→ transforming（plugins 遍历 AST）→ generating（`babel-generator` 输出代码）
- **babel 只转语法，不转 API**：箭头函数、class 等语法靠 plugins/presets 转；`Promise`、`Symbol`、`Array.from` 等新 API 靠 polyfill 补
- **polyfill vs runtime**：
  - `babel-polyfill`：整体注入，污染全局；适合业务应用
  - `babel-runtime` + `transform-runtime`：按需引用，不污染全局；适合库/插件
- **presets 执行顺序**：逆序（从数组末尾到开头）；plugins 执行顺序：正序
- **plugins 与 presets 同时存在**：plugins 先执行，presets 后执行
- **core-js**：polyfill 和 runtime 的底层实现，可按需引入单个特性
...
> 补充（现代做法）：Babel 7 将所有官方包迁移到 `@babel/` scoped 命名空间。对应关系：`babel-core` → `@babel/core`、`babylon` → `@babel/parser`、`babel-traverse` → `@babel/traverse`、`babel-generator` → `@babel/generator`、`babel-polyfill` → `@babel/polyfill`（Babel 7.4+ 已废弃，直接用 `core-js/stable` + `regenerator-runtime/runtime`）、`babel-preset-env` → `@babel/preset-env`、`babel-plugin-transform-runtime` → `@babel/plugin-transform-runtime`。升级时须同步更新 `.babelrc` 中的 preset/plugin 名称。
...
> 补充（现代做法）：`babel-polyfill` / `@babel/polyfill` 在 Babel 7.4+ 已正式废弃。现代推荐方式是在 `@babel/preset-env` 中配置 `useBuiltIns: 'usage'`（或 `'entry'`）与 `corejs: 3`，由 Babel 根据目标环境和实际用到的 API 自动按需注入 polyfill，无需手动 `import`：
> ```js
> // babel.config.js
> module.exports = {
...
babel-plugin-transform-runtime插件依赖babel-runtime，babel-
...
从上面这个例子可见，原本代码中使用的ES6新原生对象Symbol被transform-runtime插件转换成了babel-
