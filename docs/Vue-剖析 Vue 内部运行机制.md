...
![](/images/s_poetries_work_gitee_2020_07_vue_1.webp)

大家第一次看到这个图一定是一头雾水的，没有关系，我们来逐个讲一下这些模块的作用以及调用关系。相信讲完之后大家对Vue.js内部运行机制会有一个大概的认识。

### 初始化及挂载

![](/images/s_poetries_work_gitee_2020_07_vue_2.webp)
...
![](/images/s_poetries_work_gitee_2020_07_vue_3.webp)
...
![](/images/s_poetries_work_gitee_2020_07_vue_4.webp)
...
![](/images/s_poetries_work_gitee_2020_07_vue_5.webp)
...
![](/images/s_poetries_work_gitee_2020_07_vue_6.webp)
...
![](/images/s_poetries_work_gitee_2020_07_vue_7.webp)
...
![](/images/s_poetries_work_gitee_2020_09_129.webp)
...
> 然后我们定义一个 `defineReactive` ，这个方法通过 `Object.defineProperty` 来实现对对象的「**响应式**
...
![](/images/s_poetries_work_gitee_2020_07_vue_8.webp)
...
![](/images/s_poetries_work_gitee_2020_07_vue_9.webp)
...
> 我们知道，render function 会被转化成 VNode 节点。Virtual DOM 其实就是一棵以 JavaScript 对象（VNode
...
> 总的来说，`VNode` 就是一个 JavaScript 对象，用 JavaScript 对象的属性来描述当前节点的一些状态，用 VNode
...
![](/images/s_poetries_work_gitee_2020_07_vue_10.webp)
...
![](/images/s_poetries_work_gitee_2020_07_vue_11.webp)
...
![](/images/s_poetries_work_gitee_2020_07_vue_12.webp)
...
![](/images/s_poetries_work_gitee_2020_07_vue_13.webp)
...
> 用 `parseEndTag` 来解析尾标签，它会从 stack 栈中取出最近的跟自己标签名一致的那个元素，将 `currentParent`
...
> 首先我们需要定义一个 `getAndRemoveAttr` 函数，用来从 `el` 的 `attrsMap` 属性或是 `attrsList`
> 属性中取出 `name` 对应值。
```js
...
> 首先实现一个 `isStatic` 函数，传入一个 node 判断该 node 是否是静态节点。判断的标准是当 type 为
...
![](/images/s_poetries_work_gitee_2020_07_vue_14.webp)

> `diff` 算法是通过同层的树节点进行比较而非对树进行逐层搜索遍历的方式，所以时间复杂度只有 `O(n)`，是一种相当高效的算法，如下图。

![](/images/s_poetries_work_gitee_2020_07_vue_15.webp)
...
> 最后一种情况，当 `oldVNode` 与 `vnode` 都存在的时候，需要判断它们是否属于
...
![](/images/s_poetries_work_gitee_2020_07_vue_16.webp)
...
![](/images/s_poetries_work_gitee_2020_07_vue_17.webp)

> 首先当 `oldStartVnode` 或者 `oldEndVnode` 不存在的时候，`oldStartIdx` 与 `oldEndIdx`
...
![](/images/s_poetries_work_gitee_2020_07_vue_18.webp)

> 其次是 `oldEndVnode` 与 `newEndVnode` 符合 `sameVnode`，也就是两个 VNode 的结尾是相同的
> VNode，同样进行 `patchVnode` 操作并将 `oldEndVnode` 与 `newEndVnode` 向前移动一位。

![](/images/s_poetries_work_gitee_2020_07_vue_19.webp)
...
![](/images/s_poetries_work_gitee_2020_07_vue_20.webp)
...
![](/images/s_poetries_work_gitee_2020_07_vue_21.webp)
...
![](/images/s_poetries_work_gitee_2020_07_vue_22.webp)
```js
    else {
        elmToMove = oldCh[idxInOld];
        if (sameVnode(elmToMove, newStartVnode)) {
...
![](/images/s_poetries_work_gitee_2020_07_vue_23.webp)
```js
    else {
        createElm(newStartVnode, parentElm);
        newStartVnode = newCh[++newStartIdx];
...
![](/images/s_poetries_work_gitee_2020_07_vue_24.webp)

> 同理，如果满足 `newStartIdx > newEndIdx` 条件，说明新节点比对完了，老节点还有多，将这些无用的老节点通过
> `removeVnodes` 批量删除即可。

![](/images/s_poetries_work_gitee_2020_07_vue_25.webp)
```js
    if (oldStartIdx > oldEndIdx) {
        refElm = (newCh[newEndIdx + 1]) ? newCh[newEndIdx + 1].elm : null;
        addVnodes(parentElm, refElm, newCh, newStartIdx, newEndIdx);
...
![](/images/s_poetries_work_gitee_2020_07_vue_26.webp)
...
![](/images/s_poetries_work_gitee_2020_07_vue_27.webp)

> 这个时候，我们就需要用到我们的状态管理工具 Vuex 了。Vuex 是一个专门为 Vue.js 框架设计的、专门用来对于 Vue.js
