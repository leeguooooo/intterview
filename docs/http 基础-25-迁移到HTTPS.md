原文链接: [https://interview.poetries.top/fe-base-docs/http-protocol/advance/25-%E8%BF%81%E7%A7%BB%E5%88%B0HTTPS.html](https://interview.poetries.top/fe-base-docs/http-protocol/advance/25-%E8%BF%81%E7%A7%BB%E5%88%B0HTTPS.html)

## 简版速记

- **为什么迁移**：Apple/Android 强制要求 HTTPS；Chrome 将 HTTP 标记为"不安全"；搜索引擎给 HTTPS 更高权重。
- **三大顾虑**：慢（CPU 开销 <1%，网络开销 <2%）、贵（Let's Encrypt 免费）、难（抓住关键配置即可）。
- **申请证书**：推荐 Let's Encrypt + Certbot/acme.sh；同时申请 RSA（≥2048 位，SHA-2 摘要）和 ECDSA 双证书；有效期 90 天，需配置自动续期。
- **Nginx 配置要点**：`listen 443 ssl`；只开 TLS 1.2/1.3；启用 Session Ticket 会话复用；密码套件服务器优先，禁用 MD5/SHA1。
- **SNI（Server Name Indication）**：TLS 扩展，允许单 IP 托管多 HTTPS 域名；Client Hello 时携带域名，服务器据此选择证书。
- **重定向 + HSTS**：HTTP → HTTPS 用 `return 301`；响应头加 `Strict-Transport-Security: max-age=...` 防中间人劫持，浏览器后续直接走 HTTPS。
- **验证工具**：[SSLLabs](https://www.ssllabs.com/) 可对 TLS 配置进行综合评分。
...
> 补充（现代做法）：CA/Browser Forum 于 2025 年正式通过决议，将 TLS 证书最大有效期逐步压缩至 47 天（2029 年目标），部分场景未来甚至会推进到 6 天短期证书。这意味着手动续期将彻底不可行，**自动化续期（ACME + certbot/acme.sh 定时任务）已是强制要求**，而非可选项。
...
如果你的服务器上使用了 OpenSSL 的分支 BoringSSL，那么还可以使用一个特殊的“等价密码组”（Equal preference cipher
...
![](/images/s_poetries_work_gitee_2019_12_61.webp)
...
Nginx 很早就基于 SNI 特性支持了 HTTPS 的虚拟主机，但在 OpenResty 里还可以编写 Lua 脚本，利用 Redis、MySQL
等数据库更灵活快速地加载证书。
...
> 补充（现代做法）：HSTS 仍有"首次访问"漏洞——第一次请求走 HTTP 时仍可被劫持。解决方案是将域名加入 **HSTS Preload List**（[hstspreload.org](https://hstspreload.org/)），浏览器出厂时就内置该域名只走 HTTPS，彻底消除首次漏洞。加入条件：必须设置 `max-age≥31536000`、`includeSubDomains` 以及 `preload` 三个指令，且全站（含子域）均已部署 HTTPS。
