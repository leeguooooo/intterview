---
title: "小程序面试题精选"
description: "小程序面试题:双线程架构、生命周期、通信、分包、性能优化与常见坑,微信小程序开发速记。"
---
...
| 考点 | 核心要点 |
|---|---|
| OpenId vs UnionId | OpenId 单小程序/公众号唯一；UnionId 同开放平台下所有应用唯一，需绑定同主体 |
| 登录流程 | `wx.login` 拿 code → 服务端换 openid + session_key → 生成自定义 token → 前端缓存 |
| session_key 陷阱 | 先授权后 login 会刷新 session_key，导致解密失败；应先 checkSession 再获取用户信息 |
| 双线程架构 | View 层 (WebView) 与 AppService 层 (JSCore/x5) 通过 JSBridge 通信，相互隔离 |
| setData 优化 | 只传变化的字段、合并调用、后台页面延迟 setData、频繁更新抽成自定义组件 |
| 分包加载 | 主包 ≤ 2M；所有分包总计 ≤ 8M；主包放 TabBar/启动页；分包按需下载 |
| 图片导出 | canvas 绘制 → `canvasToTempFilePath` → `saveImageToPhotosAlbum`；网络图片需先 `downloadFile` |
| 埋点方案 | 包装 `Page()` 函数拦截生命周期和 click 事件，混入埋点逻辑，不侵入业务代码 |
| rpx 单位 | 屏幕宽固定 750rpx，`px = screenWidth / 750 * rpx` |
| 工程化 | 原生已支持 npm（2019+）；主流框架：Taro（React风格）、uni-app（Vue风格）已取代 mpvue/wepy |
...
> 补充（现代做法）：`wx.getUserInfo` 自 2021 年起不再弹授权框（直接返回匿名数据），官方推荐改用 `wx.getUserProfile`（每次调用需用户主动触发）；2022 年底 `wx.getUserProfile` 也已不再返回真实头像昵称，微信要求通过「头像昵称填写能力」（`open-type="chooseAvatar"` 按钮 + `bindchooseavatar`）获取用户信息。
...
> 补充（现代做法）：微信开发者工具自 2019 年起已原生支持 npm（需在项目设置中开启并执行「构建 npm」）；`async/await` 通过开发者工具勾选「增强编译」即可使用，无需额外配置 babel。主流工程化框架现以 **Taro**（React 风格，跨多端）和 **uni-app**（Vue 风格，跨多端）为主，wepy/mpvue 已基本停止维护。
...
![](/images/user_images_githubusercontent_com_2350193_44563914_ff97c380_a792_11e8_8e77_6d0970891e24.webp)
...
![](/images/user_images_githubusercontent_com_2350193_44186238_db146980_a14a_11e8_8096_bcb8fa6d28b2.webp)
...
> 补充（现代做法）：npm 与 ES7+ 限制均已解除（2019+）。`formId` 推送机制已被废弃，现统一使用「订阅消息」（`wx.requestSubscribeMessage`）替代服务通知。
  * 不支持使用自己的字体（未来官方计划支持）。
...
![](/images/user_images_githubusercontent_com_35895755_44379940_fa403c00_a53a_11e8_9165_21b217496aad.webp)
...
![](/images/user_images_githubusercontent_com_35895755_44244965_268d4d00_a209_11e8_8ef4_b80cc7a78af7.webp)

![](/images/user_images_githubusercontent_com_35895755_44379952_0af0b200_a53b_11e8_86be_640bf651bc9e.webp)
...
![](/images/user_images_githubusercontent_com_2350193_44184904_d8624600_a143_11e8_8ab9_c932573bd243.webp)

> 代码包的大小是最直接影响小程序加载启动速度的因素。代码包越大不仅下载速度时间长，业务代码注入时间也会变长。所以最好的优化方式就是减少代码包的大小

小程序加载的三个阶段的表示

![](/images/user_images_githubusercontent_com_2350193_44184987_4c9ce980_a144_11e8_9f28_764209b37341.webp)
...
![](/images/user_images_githubusercontent_com_2350193_44185607_2298f680_a147_11e8_8440_24ca42033623.webp)
...
![](/images/s_poetries_work_uploads_2022_07_e2328f87745f6433.webp)

独立分包

![](/images/s_poetries_work_uploads_2022_07_596fb9802c00df92.webp)

**渲染性能优化**

![](/images/user_images_githubusercontent_com_2350193_44185879_af907f80_a148_11e8_8dcb_22aadd4e49a6.webp)
