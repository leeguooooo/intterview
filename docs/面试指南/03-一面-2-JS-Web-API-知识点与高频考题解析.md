---
title: "一面 JS Web API 知识点与高频考题解析"
---
...
> 补充（现代做法）：现在跨域几乎只用 **CORS**，JSONP 因只能 GET、易受 XSS/CSRF 影响已基本淘汰。开发期更常用 **dev server 代理**（Vite `server.proxy` / webpack-dev-server `proxy`）绕开浏览器同源限制。带 cookie 跨域时 `Access-Control-Allow-Origin` 不能为 `*` 且需配合 `Access-Control-Allow-Credentials: true`，同时注意跨站 cookie 需设置 `SameSite=None; Secure`。非简单请求（自定义头、`PUT/DELETE`、`application/json` 等）浏览器会先发 **OPTIONS 预检**，服务端要正确响应预检。
