...
**React Compiler 是什么**：编译时工具，自动为组件、props 和 hook 依赖项注入 `memo`/`useMemo`/`useCallback`，无需手写。

**简单场景**：3/3 全中——无 props、带函数 props、带 children 均能正确 memoize。

**真实项目实测（3 个应用，每个找 8-10 个不必要重渲染）**：
- 大应用（150k 行）：10 个只修复 2 个
- 中型应用（30k 行，ESLint 0 违规）：10 个只修复 2 个
- 小应用（几屏 CRUD，100% 可编译）：8 个只修复 1 个

**编译器失效的根本原因**：
1. **第三方 hook 返回非 memoized 对象**：`useMutation()` 返回的是每次重渲染都新建的对象，编译器无法判断其稳定性。解法：只提取其中已 memoized 的 `mutate` 函数使用。
2. **内联 map 中的 callback 无法拆分 memoize**：列表渲染内的行需提取为独立组件（如 `CountryRow`），编译器才能对其单独处理。
3. **`key` 用 index 而非稳定 ID**：导致列表重排时强制重渲染，绕过所有 memoization。

**让编译器生效的最小改动**：
```ts
// 1. 提取 mutate，不依赖整个 mutation 对象
...
**面试结论**：
- 编译器"开箱即用"是真的（不会破坏现有代码），但"不再需要手动 memoize"是假的
- 真实项目修复率约 10-20%，剩余问题仍需手动调优
- 使用编译器反而需要**更深**理解 memoize 原理，才能调试它为什么没生效
- 组合技巧（状态下移、children 传递、Context 拆分）比 memoize 更根本
...
![](/images/www_developerway_com__next_image_url__2Fassets_2Fi_tried_react_compiler_2Fwelcome.webp)
...
![re-renders-
example-20220802-132550.png](/images/www_developerway_com_assets_i_tried_react_compiler_1_re_renders_example.webp)
...
> 补充（现代做法）：本文写于 2024 年 6 月，当时 React 19 和 React Compiler 均处于 canary/实验阶段。截至 2025 年，**React 19 已正式发布，React Compiler 已进入 RC**，安装方式已更新——不再需要 `next@canary`，直接用 `next@latest` 并安装 `babel-plugin-react-compiler` 即可。官方文档：<https://react.dev/learn/react-compiler>
...
![memo-in-dev-
tools-20240605-060936.png](/images/www_developerway_com_assets_i_tried_react_compiler_2_memo_in_dev_tools.webp)
...
有了编译器，这就像魔法一样：我们可以抛弃 `React.memo`，并且在开发者工具中还能看到 `VerySlowComponent`
...
但有了编译器，我们仍然不需要这样做！`VerySlowComponent` 在 React 调试工具中仍然显示为已记忆化，其中的 “control”
console.log 也不会被触发。
...
这是我老旧的临床天性所预料到的，但绝对不是我天真的内心孩子所期望的。也许我只是写 React
...
![third-app-ui-
screenshot-20240605-063231.png](/images/www_developerway_com_assets_i_tried_react_compiler_3_third_app_ui_screenshot.webp)
...
![4-everything-
memoized-20240605-092740.png](/images/www_developerway_com_assets_i_tried_react_compiler_4_everything_memoized.webp)

然而，尝试启用 “组件渲染时高亮显示更新” 设置，享受灯光秀吧。

![5-everything-re-
renders-20240605-093649.gif](/images/www_developerway_com_assets_i_tried_react_compiler_5_everything_re_renders.webp)
...
![6-no-more-re-
renders-20240605-095209.gif](/images/www_developerway_com_assets_i_tried_react_compiler_6_no_more_re_renders.webp)
...
对于那些来自未来的访客，我现在相当确信他们是来自平行宇宙。一个奇妙的地方，在那里 React 竟然用比臭名昭著的灵活的 JavaScript
