...
- **核心定位**：Rollup 是纯 ES Modules 打包器，Tree-shaking 概念由它首创；Webpack 是功能完备的前端工程化工具。
- **一句话对比**：**Webpack 大而全，Rollup 小而美**。
- **Rollup 优点**：输出结果扁平（无引导代码）、默认开启 Tree-shaking、打包产物完全可读、天然支持多种输出格式（es/cjs/umd/iife 等）。
- **Rollup 缺点**：加载非 ESM 第三方模块需额外插件；不支持 HMR；代码拆分在低版本浏览器需借助 AMD 库。
- **选型原则**：开发**应用程序**用 Webpack（需要 HMR、大量第三方模块、复杂分包）；开发 **JS 库/框架**用 Rollup（React、Vue 均采用 Rollup）。
- **插件体系**：Rollup 只有 plugins 一种扩展方式；Webpack 有 Loader / Plugin / Minimizer 三种。
- **配置文件**：`rollup.config.js` 原生支持 ES Modules 语法，`--config` 参数指定配置文件。
...
![](/images/s_poetries_work_images_20210503214552.webp)
...
![](/images/s_poetries_work_images_20210503214605.webp)
...
![](/images/s_poetries_work_images_20210503214615.webp)

在这个文件中我们的第一印象就是，Rollup 打包结果惊人的简洁，基本上就跟我们手写的代码一样。相比于 Webpack
...
在这个配置当中我们导出了一个数组，数组中的每个成员都是一个单独的打包配置，这样 Rollup 就会分别按照每个配置单独打包。这一点与 Webpack
非常相似。

配置完成之后，我们回到命令行终端，再次运行 Rollup 打包。那这次打包过后，dist 目录下就会生成不同格式的输出结果，如下图所示：

![](/images/s_poetries_work_images_20210503214627.webp)
...
@rollup/plugin-json 模块的默认导出就是一个插件函数。我们可以将这个函数的调用结果添加到配置对象的 `plugins`
...
![](/images/s_poetries_work_images_20210503214638.webp)
...
![](/images/s_poetries_work_images_20210503214651.webp)
...
![](/images/s_poetries_work_images_20210503214659.webp)
...
![](/images/s_poetries_work_images_20210503214707.webp)
...
![](/images/s_poetries_work_images_20210503214718.webp)
...
> 补充(现代做法)：主流浏览器已原生支持 ES Modules 动态导入（`import()`），将 `format` 保持为 `'es'` 即可在现代浏览器中直接使用代码拆分，无需引入 Require.js。另外，基于 Rollup 构建的 **Vite** 在生产环境同样以 Rollup 打包，并通过 `<link rel="modulepreload">` 等手段消除了原生 ESM 分包的瀑布请求问题，是当前库/框架及中小型应用的主流选择。
