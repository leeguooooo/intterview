原文链接: [https://interview.poetries.top/fe-base-docs/http-protocol/base/17-HTTP%E7%9A%84%E7%BC%93%E5%AD%98%E6%8E%A7%E5%88%B6.html](https://interview.poetries.top/fe-base-docs/http-protocol/base/17-HTTP%E7%9A%84%E7%BC%93%E5%AD%98%E6%8E%A7%E5%88%B6.html)

## 简版速记

| 要点 | 核心内容 |
|------|---------|
| Cache-Control 方向 | 响应头（服务器→浏览器）和请求头（浏览器→服务器）均可携带 |
| max-age | 缓存有效秒数；从**响应报文创建时刻**（Date 字段）起算，不是客户端收到时刻 |
| no-store | 完全不缓存，每次都从服务器重新获取 |
| no-cache | 可以缓存，但每次使用前必须去服务器验证是否仍有效（不是"不缓存"） |
| must-revalidate | 未过期可直接用；过期后必须重新验证，不能直接使用 |
| 条件请求 | `If-Modified-Since` + `Last-Modified`（时间比对）；`If-None-Match` + `ETag`（精确比对） |
| 304 Not Modified | 服务器确认缓存仍有效，浏览器复用本地缓存，响应体不重传 |
| 强/弱 ETag | 强 ETag：字节级完全匹配；弱 ETag（`W/` 前缀）：语义相同即可 |
| 刷新行为 | 普通刷新 → 请求头 `max-age=0`；强制刷新(Ctrl+F5) → 请求头 `no-cache` |
...
![](/images/s_poetries_work_gitee_2019_12_26.webp)
...
> 补充（现代做法）：现代前端构建工具（Vite、Webpack）对静态资源普遍采用"哈希文件名 + `Cache-Control: public, max-age=31536000, immutable`"策略。文件内容变化时哈希随之改变，URL 不同即为新资源，因此可以永久缓存；`immutable`（RFC 8246）告知浏览器在 max-age 期间内完全跳过条件请求，消除不必要的 304 往返。此外，`stale-while-revalidate=<秒>` 允许先以过期缓存响应再在后台异步刷新，兼顾低延迟与内容新鲜度。
...
![](/images/s_poetries_work_gitee_2019_12_27.webp)
...
![](/images/s_poetries_work_gitee_2019_12_28.webp)
...
## 小结
  ...
  * 浏览器也可以发送”`Cache-Control`”字段，使用”`max-age=0`”或”`no-cache`”刷新数据

> HTTP 缓存看上去很复杂，但基本原理说白了就是一句话：“没有消息就是好消息”，“没有请求的请求，才是最快的请求。”

![](/images/s_poetries_work_gitee_2019_12_29.webp)
