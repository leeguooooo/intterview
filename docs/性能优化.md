---
title: "前端性能优化面试题精选"
description: "前端性能优化面试题:关键渲染路径、懒加载、缓存、CDN、代码分割、防抖节流、Core Web Vitals,实战总结。"
---
...
- **缓存**：强缓存（`Cache-Control: max-age` / `Expires`，state code 200）→ 协商缓存（`ETag/If-None-Match`、`Last-Modified/If-Modified-Since`，304）；`ETag` 优先级高于 `Last-Modified`
- **资源加载**：`dns-prefetch`（DNS 预解析）、`preload`（当前页关键资源，提升优先级）、`prefetch`（未来页低优先级，空闲时加载）、`preconnect`（预建 TCP/TLS 连接）
- **图片优化**：懒加载（IntersectionObserver）、WebP 格式（现已全面支持主流浏览器）、雪碧图合并、小图 base64 内嵌、CDN 分发
- **代码优化**：Tree Shaking（ES Module 静态分析）、按路由拆包（动态 `import()`）、gzip/Brotli 压缩、文件名加 hash 配合强缓存
- **渲染优化**：减少重排重绘、事件委托代替大量绑定、`DocumentFragment` 批量 DOM 操作、CSS 动画优于 JS 动画（GPU 加速 `will-change/transform`）
- **长列表**：虚拟滚动（只渲染可视区 DOM，如 vue-virtual-scroll-list）、`Object.freeze` 冻结大数据跳过 Vue 响应式转换
- **监控**：`PerformanceObserver` / `performance.timing` 采集指标、`window.onerror` 拦截错误、sourceMap 便于 debug、img 标签打点上报
- **卡顿治理**：`requestAnimationFrame` 做动画（对齐屏幕刷新）、`requestIdleCallback` 处理低优任务、Web Worker 处理耗时计算、节流/防抖
- **Vue 编码**：`v-if` 优于 `v-show`（初始不渲染）、`keep-alive` 缓存组件、路由懒加载、减少 `data` 中的响应式数据量、长列表用事件代理
- **Vue3 亮点**：Proxy 响应式 + 编译期静态标记（PatchFlag）+ 按需 diff，update 性能约 1.3–2×，SSR 约 2–3×
...
> 补充（现代做法）：**HTTP/3（QUIC）** 已在主流 CDN（Cloudflare、阿里云、腾讯云等）上广泛部署。QUIC 基于 UDP，彻底解决了 HTTP/2 在弱网下的队头阻塞问题，握手延迟也更低（0-RTT 重连）。对延迟敏感的场景可优先评估 HTTP/3 支持情况。
...
> 补充（现代做法）：WebP 目前已被 Chrome、Firefox、Safari 14+、Edge 全面支持，兼容性问题基本消除。可直接使用 `<picture>` + `<source type="image/webp">` 提供 WebP，旧浏览器自动降级到 JPEG/PNG。更新一代的 **AVIF** 格式压缩率更高（比 WebP 再小 20–50%），Chrome 85+/Firefox 93+/Safari 16+ 均已支持，可作为优先选项。
...
> 补充（现代做法）：新项目推荐使用 **Vite**（基于 ESM 原生 + Rollup），开发环境无需打包、冷启动极快，生产构建同样支持 Tree Shaking、动态 import 拆包、文件 hash。Webpack 5 引入了**持久化缓存**（`cache: { type: 'filesystem' }`）和**模块联邦**（Module Federation），二次构建速度大幅提升，仍是大型存量项目的主流选择。
...
![image-20210307184052955](/images/s_poetries_work_images_image_20210307184052955.webp)
```js
...
> 补充（现代做法）：`performance.timing` 已被标记为**废弃**（deprecated），推荐改用 `PerformanceNavigationTiming`（通过 `performance.getEntriesByType('navigation')[0]` 获取）和 `PerformanceObserver` 监听 LCP/FID/CLS 等 **Web Vitals** 核心指标。Google 的 [web-vitals](https://github.com/GoogleChrome/web-vitals) 库可一行代码完成采集。
...
![image-20210307183730134](/images/s_poetries_work_images_image_20210307183730134.webp)
...
![image-20210307183810844](/images/s_poetries_work_images_image_20210307183810844.webp)
...
![image-20210307183845241](/images/s_poetries_work_images_image_20210307183845241.webp)
