...
> 补充（现代做法）：`HappyPack` 已停止维护，多进程改用官方推荐的 **`thread-loader`**；`ParallelUglifyPlugin` / `UglifyJS` 不支持 ES6+，webpack5 默认用 **`terser-webpack-plugin`**（内置并行 + 缓存）压缩，无需手动配置。`babel-loader` 的提速核心仍是 `cacheDirectory` + `exclude: /node_modules/`，但 webpack5 自带 **持久化文件缓存**（`cache.type='filesystem'`），二次构建提速比 DllPlugin 更省心，`DllPlugin` 在 webpack5 已基本不需要。
