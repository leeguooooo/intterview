...
对于 `Fragment` 类型的 **`VNode`** ，它的 `tag` 属性值为 `null`，但是纯文本类型的 `VNode` 其 `tag`
...
如上， `normalizeVNodes` 函数接收 `children` 数组作为参数，并遍历该数组，对于数组中没有 `key` 的 `VNode`
...
大家观察如上 `VNode` 对象，其中 `tag` 属性的值就是组件函数本身，另外 `functionalComponentVNode.children`
