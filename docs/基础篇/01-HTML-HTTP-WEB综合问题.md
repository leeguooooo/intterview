...
> 补充（现代做法）：涉及登录态的 `cookie` 应加上安全属性——`HttpOnly`（禁止 JS 读取，防 XSS 窃取）、`Secure`（仅 HTTPS 传输）、`SameSite=Lax/Strict`（防 CSRF，现代浏览器默认 `Lax`）。前端纯客户端存储更推荐 `localStorage` / `sessionStorage`，结构化大数据用 `IndexedDB`；敏感凭证不要放 `localStorage`（同样会被 XSS 读到），交给 `HttpOnly` cookie 或内存保存。
...
![](/images/s_poetries_work_cos_202204032131039.webp)
