---
title: "React 从 Redux 中间件实现原理切入 理解 面向切面编程"
---

## 简版速记

- **中间件执行时机**：`dispatch(action)` 之后、`reducer` 触发之前，属于 AOP（面向切面编程）的典型应用——在不修改核心流程的前提下注入横切逻辑（日志、异步、错误上报等）。
- **中间件签名**（三层柯里化）：`store => next => action => { ... }`。外层拿到 `dispatch/getState`，中层拿到 `next`（下一个中间件或原始 dispatch），内层处理 `action`。
- **applyMiddleware 做了什么**：① 用 `middlewareAPI` 调用每个中间件，得到"内层函数"数组 `chain`；② 用 `compose` 将 chain 从右到左串联；③ 以原始 `store.dispatch` 为初始值启动链，返回改写后的 `dispatch`。
- **compose 原理**：`funcs.reduce((a, b) => (...args) => a(b(...args)))` — `compose(f1, f2, f3)` 等价于 `(...args) => f1(f2(f3(...args)))`，中间件按**从左到右**的顺序依次拦截 action。
- **redux-thunk 核心逻辑**：若 `action` 是函数则执行它并传入 `(dispatch, getState)`；否则直接 `next(action)` 放行，从而支持异步 action creator。
- **createStore enhancer 机制**：第二参数为函数时被识别为 enhancer；存在 enhancer 时执行 `enhancer(createStore)(reducer, preloadedState)`，将原始 createStore 的控制权交给中间件层。
- **面试高频考点**：① dispatch 是被"改写"而非"绕过"；② middlewareAPI 中 dispatch 用箭头函数包裹，保证指向改写后的 dispatch（闭包延迟求值）；③ compose 是函数式编程中的通用工具，与 Redux 无关。
...
这段代码告诉我们，在只传入两个参数的情况下，createStore 会去检查第二个参数是否是 function
...
![](/images/s_poetries_work_images_20210429162806.webp)

若有多个中间件，那么 Redux 会结合它们被“安装”的先后顺序，依序调用这些中间件，这个过程如下图所示：

![](/images/s_poetries_work_images_20210429162825.webp)
...
> 补充（现代做法）：Redux Toolkit（RTK）已将 redux-thunk 内置为默认中间件，`configureStore` 开箱即用无需手动 `applyMiddleware(thunk)`。异步逻辑推荐用 `createAsyncThunk`，它会自动 dispatch `pending / fulfilled / rejected` 三个生命周期 action，配合 `createSlice` 的 `extraReducers` 处理状态，比手写 thunk 更简洁且类型安全。数据请求场景可进一步使用 RTK Query，彻底省去手写 thunk 的必要。
...
这个代码片段做了两件事：首先以 `middlewareAPI` 作为入参，逐个调用传入的 `middleware`，获取一个由“内层函数”组成的数组
