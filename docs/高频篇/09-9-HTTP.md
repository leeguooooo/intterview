...
> 补充(现代做法)：现在主流大站已普及 **HTTP/3**。它把传输层从 `TCP` 换成基于 `UDP` 的 **QUIC** 协议——HTTP/2 的多路复用仍跑在单条 `TCP` 连接上，一旦某个分片丢包会触发 TCP 层的「队头阻塞(Head-of-Line Blocking)」，拖慢所有流；QUIC 在用户态实现独立的流，丢包只影响该流，并把 TLS 1.3 握手合并进连接建立（0-RTT/1-RTT），弱网下首字节更快。此外 HTTP/2 的「服务端推送(Server Push)」因命中率低、复杂度高，Chrome 已于 2022 年移除，实践中改用 `<link rel="preload">`、`103 Early Hints` 替代。
