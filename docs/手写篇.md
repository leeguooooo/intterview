...
> 补充(现代做法)：浏览器与 Node 17+ 已内置 `structuredClone(obj)`，原生支持循环引用、Date、RegExp、Map、Set、ArrayBuffer 等，深拷贝纯数据时优先用它。局限是**不能拷贝函数、DOM 节点、Symbol 键、原型链**（遇到会抛错或丢失），这些场景仍需手写递归。`JSON.parse(JSON.stringify(obj))` 是最省事的写法，但会丢失 `undefined`/函数/Symbol，且把 Date 变成字符串、无法处理循环引用——面试时要能说清它的这些坑。
