...
> 补充(现代做法)：`fastclick.js` 早已停止维护，现代项目不再需要它。如今所有主流移动端浏览器在设置了响应式 `viewport`（`width=device-width`）或 `touch-action: manipulation` / `touch-action: none` 时，都会移除 300ms 点击延迟。推荐做法是直接在 CSS 里给可点击元素加 `touch-action: manipulation;`，既消除延迟又保留滚动/缩放手势，无需任何 JS 库。
...
> 补充(现代做法)：同域多标签通讯现在首选 **`BroadcastChannel`** API——比监听 `storage` 事件更直观（无需借道写 `localStorage`，也不会被自己页面触发），比 `SharedWorker` 兼容性好得多（现代浏览器全支持，仅老旧 IE 不支持）。所有同名频道的标签都能收发消息：
> ```js
>     // 任意标签页：创建同名频道
...
>     // 发送（其它标签收到，自己不会收到）
>     channel.postMessage({ id: 100, name: '标题' + Date.now() })
>     // 用完关闭
>     // channel.close()
> ```
> 若需跨标签共享一份长连接/计算，再用 `SharedWorker`；要离线缓存与请求拦截则用 `Service Worker`。
