...
<span class="line"></span></code></pre><div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0;"><div class="line-number"></div><div class="line-number"></div></div></div><p>由于没有旧的 <code>VNode</code> 存在，所以会调用 <code>mount</code> 函数挂载全新的 <code>VNode</code> ，这个小节我们就探讨一下渲染器的 <code>mount</code> 函数是如何把 <code>VNode</code> 渲染成真实 <code>DOM</code> 的，以及其中一些核心的关键点。</p><p><code>mount</code> 函数的作用是把一个 <code>VNode</code> 渲染成真实 <code>DOM</code>，根据不同类型的 <code>VNode</code> 需要采用不同的挂载方式，如下：</p><div class="language-javascript line-numbers-mode" data-highlighter="prismjs" data-ext="js" data-title="js"><pre><code><span class="line">    <span class="token keyword">function</span> <span class="token function">mount</span><span class="token punctuation">(</span><span class="token parameter">vnode<span class="token punctuation">,</span> container</span><span class="token punctuation">)</span> <span class="token punctuation">{</span></span>
...
<span class="line">    <span class="token punctuation">}</span></span>
<span class="line"></span></code></pre><div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0;"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>我们根据 <code>VNode</code> 的 <code>flags</code> 属性值能够区分一个 <code>VNode</code> 对象的类型，不同类型的 <code>VNode</code> 采用不同的挂载函数：</p><p><img src="`+o+`" alt=""></p><p>我们首先来讨论一下 <code>mountElement</code> 函数，它用于挂载普通标签元素。我们在&quot;组件的本质&quot;一章中曾经编写过如下这段代码：</p><div class="language-javascript line-numbers-mode" data-highlighter="prismjs" data-ext="js" data-title="js"><pre><code><span class="line">    <span class="token keyword">function</span> <span class="token function">mountElement</span><span class="token punctuation">(</span><span class="token parameter">vnode<span class="token punctuation">,</span> container</span><span class="token punctuation">)</span> <span class="token punctuation">{</span></span>
...
</code></pre><p>再来看第二个问题：<strong>没有将<code>VNodeData</code> 应用到元素上</strong>，我们知道 <code>VNodeData</code> 作为 <code>VNode</code> 的描述，对于标签元素来说它包含了元素的样式、事件等诸多信息，我们需要将这些信息应用到新创建的真实DOM元素上，假设我们有如下 <code>VNode</code>：</p><p>TIP</p><p>再次强调，本章使用上一章节中所编写的 <code>h</code> 函数。</p><div class="language-javascript line-numbers-mode" data-highlighter="prismjs" data-ext="js" data-title="js"><pre><code><span class="line">    <span class="token keyword">const</span> elementVnode <span class="token operator">=</span> <span class="token function">h</span><span class="token punctuation">(</span></span>
...
</code></pre><p>如上代码可知，我们为 <code>elementVnode</code> 添加了一个子节点，该子节点是一个边长为 <code>50px</code> 的绿色正方形，使用渲染器渲染修改后的 <code>elementVnode</code> 的效果如下：</p><p><img src="`+p+`" alt=""></p><p>接着我们来看最后一个问题：<strong>不能严谨地处理<code>SVG</code> 标签</strong>，在之前的 <code>mountElement</code> 函数中我们使用 <code>document.createElement</code> 函数创建DOM元素，但是对于 <code>SVG</code> 标签，更加严谨的方式是使用 <code>document.createElementNS</code> 函数，修改 <code>mountElement</code> 如下：</p><pre><code>function mountElement(vnode, container) {
...
<span class="line"></span></code></pre><div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0;"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>在这段模板中我们同时使用了 <code>class</code> 属性和绑定的 <code>:class</code> 属性，对于非绑定的 <code>class</code> 属性来说它的值就是我们最终想要的类名列表字符串，但是对于绑定的 <code>:class</code> 属性来说它的值是动态的 <code>javascript</code> 值，所以我们需要设计一下哪些值是被允许的。</p><p>首先数组应该是被允许的：</p><div class="language-javascript line-numbers-mode" data-highlighter="prismjs" data-ext="js" data-title="js"><pre><code><span class="line">    dynamicClass <span class="token operator">=</span> <span class="token punctuation">[</span><span class="token string">&#39;class-b&#39;</span><span class="token punctuation">,</span> <span class="token string">&#39;class-c&#39;</span><span class="token punctuation">]</span></span>
<span class="line"></span></code></pre><div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0;"><div class="line-number"></div></div></div><p>对象也应该是被允许的：</p><div class="language-javascript line-numbers-mode" data-highlighter="prismjs" data-ext="js" data-title="js"><pre><code><span class="line">    dynamicClass <span class="token operator">=</span> <span class="token punctuation">{</span></span>
...
<span class="line">    <span class="token punctuation">}</span></span>
<span class="line"></span></code></pre><div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0;"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>在编译器对模板进行编译时，我们把非绑定和绑定的 <code>class</code> 属性值合并，如下是我们期望编译器对上面模板的编译结果：</p><div class="language-javascript line-numbers-mode" data-highlighter="prismjs" data-ext="js" data-title="js"><pre><code><span class="line">    <span class="token function">h</span><span class="token punctuation">(</span><span class="token string">&#39;div&#39;</span><span class="token punctuation">,</span> <span class="token punctuation">{</span></span>
...
<span class="line"></span></code></pre><div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0;"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>如果 <code>dynamicClass</code> 是数组，那么如上代码等价于：</p><div class="language-javascript line-numbers-mode" data-highlighter="prismjs" data-ext="js" data-title="js"><pre><code><span class="line">    <span class="token function">h</span><span class="token punctuation">(</span><span class="token string">&#39;div&#39;</span><span class="token punctuation">,</span> <span class="token punctuation">{</span></span>
...
<span class="line">    <span class="token punctuation">}</span><span class="token punctuation">)</span></span>
<span class="line"></span></code></pre><div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0;"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>如果 <code>dynamicClass</code> 是对象，那么编译的结果等价于：</p><div class="language-javascript line-numbers-mode" data-highlighter="prismjs" data-ext="js" data-title="js"><pre><code><span class="line">    <span class="token function">h</span><span class="token punctuation">(</span><span class="token string">&#39;div&#39;</span><span class="token punctuation">,</span> <span class="token punctuation">{</span></span>
...
<span class="line"></span></code></pre><div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0;"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>可以看到虽然我们使用 <code>setAttribute</code> 函数将复选框的 <code>checked</code> 属性设置为 <code>false</code>，但是当我们访问 <code>checkboxEl.checked</code> 时得到的依然是 <code>true</code>，这是因为在 <code>setAttribute</code> 函数为元素设置属性时，无论你传递的值是什么类型，它都会将该值转为字符串再设置到元素上，所以如下两句代码是等价的：</p><div class="language-javascript line-numbers-mode" data-highlighter="prismjs" data-ext="js" data-title="js"><pre><code><span class="line">    checkboxEl<span class="token punctuation">.</span><span class="token function">setAttribute</span><span class="token punctuation">(</span><span class="token string">&#39;checked&#39;</span><span class="token punctuation">,</span> <span class="token boolean">false</span><span class="token punctuation">)</span></span>
...
</code></pre><p>如上高亮代码所示，我们首先创建了一个正则表达式 <code>domPropsRE</code>，用来检测那些应该以 <code>Property</code> 的方式添加到 DOM 元素上的属性，其他的属性使用 <code>setAttribute</code> 方法设置。另外我们注意到正则 <code>domPropsRE</code> 除了用来匹配我们前面说过的固定的几个属性之外，它还能匹配那些拥有大写字母的属性，这是为了匹配诸如 <code>innerHTML</code>、<code>textContent</code> 等属性设计的，同时这也顺便实现了一个特性，即拥有大写字母的属性我们都会采用 <code>el[key] = xxx</code> 的方式将其添加到 DOM 元素上。</p><p>如下是渲染上面 <code>input</code> 标签的效果图：</p><p><img src="`+l+`" alt=""></p><p>TIP</p><p>完整代码&amp;在线体验地址：<a href="https://codesandbox.io/s/821421zvp8" target="_blank" rel="noopener noreferrer">https://codesandbox.io/s/821421zvp8</a><a href="https://codesandbox.io/s/821421zvp8" target="_blank" rel="noopener noreferrer"> (opens new window)</a></p><h3 id="事件的处理" tabindex="-1"><a class="header-anchor" href="#事件的处理"><span>事件的处理</span></a></h3><p>现在我们只剩下为 DOM 元素添加事件了，实际上在 <code>mount</code> 阶段为 DOM 元素添加事件很容易，我们只需要在元素对象上调用 <code>addEventListener</code> 方法即可，关键在于我们的 <code>VNodeData</code> 要如何设计。</p><p>通常我们给元素添加事件的规则是<strong>使用<code>v-on</code> 或 <code>@</code> 符号加上事件名字</strong>，例如给元素添加点击事件：</p><div class="language-html line-numbers-mode" data-highlighter="prismjs" data-ext="html" data-title="html"><pre><code><span class="line">    <span class="token tag"><span class="token tag"><span class="token punctuation">&lt;</span>div</span> <span class="token attr-name">@click</span><span class="token attr-value"><span class="token punctuation attr-equals">=</span><span class="token punctuation">&quot;</span>handler<span class="token punctuation">&quot;</span></span><span class="token punctuation">&gt;</span></span><span class="token tag"><span class="token tag"><span class="token punctuation">&lt;/</span>div</span><span class="token punctuation">&gt;</span></span></span>
<span class="line"></span></code></pre><div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0;"><div class="line-number"></div></div></div><p>当然事件名字中不包含 <code>&#39;on&#39;</code> 前缀，即 <code>click</code> 而不是 <code>onclick</code>，我们可以用如下 <code>VNode</code> 对象来描述如上模板：</p><div class="language-javascript line-numbers-mode" data-highlighter="prismjs" data-ext="js" data-title="js"><pre><code><span class="line">    <span class="token keyword">const</span> elementVNode <span class="token operator">=</span> <span class="token function">h</span><span class="token punctuation">(</span><span class="token string">&#39;div&#39;</span><span class="token punctuation">,</span> <span class="token punctuation">{</span></span>
<span class="line">      <span class="token literal-property property">click</span><span class="token operator">:</span> handler</span>
<span class="line">    <span class="token punctuation">}</span><span class="token punctuation">)</span></span>
<span class="line"></span></code></pre><div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0;"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>然而这么做是有问题的，如上代码所示 <code>elementVNode</code> 的 <code>VNodeData</code> 中的 <code>click</code> 属性没办法与其他DOM属性区分，所以渲染器并不知道 <code>click</code> 属性代表的是事件，当然我们可以做出规定，例如我们规定 <code>VNodeData</code> 中的 <code>click</code> 属性是个特殊的属性，它用来存储事件回调函数，但这是很笨的方法，因为 DOM 原生事件很多，这种方案需要我们一一列举所有 DOM 事件并且扩展性很差。所以我们需要考虑如何将事件与属性区分，其实我们就沿用原生 DOM 对象的设计即可，在原生 DOM 对象中所有事件函数的名字都是 <code>&#39;on&#39; + 事件名称</code> 的形式，所以我们可以在 <code>VNodeData</code> 中使用 <code>onclick</code> 代替 <code>click</code>：</p><div class="language-javascript line-numbers-mode" data-highlighter="prismjs" data-ext="js" data-title="js"><pre><code><span class="line">    <span class="token keyword">const</span> elementVNode <span class="token operator">=</span> <span class="token function">h</span><span class="token punctuation">(</span><span class="token string">&#39;div&#39;</span><span class="token punctuation">,</span> <span class="token punctuation">{</span></span>
...
</code></pre><p>如上高亮代码所示，我们通过检查 <code>VNodeData</code> 对象的键名(<code>key</code>)的前两个字符是否是 <code>&#39;on&#39;</code>，来区分其是否是事件，如果是事件则调用 <code>el.addEventListener</code> 将事件回调函数添加到元素上。</p><p>我们可以测试一下我们的代码：</p><div class="language-javascript line-numbers-mode" data-highlighter="prismjs" data-ext="js" data-title="js"><pre><code><span class="line">    <span class="token comment">// 事件回调函数</span></span>
...
</code></pre><p><code>mountText</code> 函数实现起来很简单，由于纯文本类型的 <code>VNode</code> 其 <code>children</code> 属性存储着与之相符的文本字符串，所以只需要调用 <code>document.createTextNode</code> 函数创建一个文本节点即可，然后将其添加到 <code>container</code> 中，如下：</p><div class="language-javascript line-numbers-mode" data-highlighter="prismjs" data-ext="js" data-title="js"><pre><code><span class="line">    <span class="token keyword">function</span> <span class="token function">mountText</span><span class="token punctuation">(</span><span class="token parameter">vnode<span class="token punctuation">,</span> container</span><span class="token punctuation">)</span> <span class="token punctuation">{</span></span>
...
</code></pre><p>使用渲染器渲染如上 <code>elementVnode</code> 的结果如下图所示：</p><p><img src="`+i+`" alt=""></p><p>TIP</p><p>完整代码&amp;在线体验地址：<a href="https://codesandbox.io/s/72zq40y0q6" target="_blank" rel="noopener noreferrer">https://codesandbox.io/s/72zq40y0q6</a><a href="https://codesandbox.io/s/72zq40y0q6" target="_blank" rel="noopener noreferrer"> (opens new window)</a></p><h3 id="挂载-fragment" tabindex="-1"><a class="header-anchor" href="#挂载-fragment"><span>挂载 Fragment</span></a></h3><p>其实挂载 <code>Fragment</code> 和单纯地挂载一个 <code>VNode</code> 的 <code>children</code> 是没什么区别的，在没有 <code>Fragment</code> 时我们要想挂载一个片段，这个片段必须使用包裹元素包裹，如下：</p><div class="language-html line-numbers-mode" data-highlighter="prismjs" data-ext="html" data-title="html"><pre><code><span class="line">    <span class="token comment">&lt;!-- div 就是包裹元素 --&gt;</span></span>
...
</code></pre><p>使用渲染器渲染该 <code>elementVNode</code> 的效果图如下：</p><p><img src="`+u+`" alt=""></p><p>可以发现 <code>Portal</code> 的挂载点是 <code>#portal-box</code>，而非 <code>#app</code>。</p><p>TIP</p><p>完整代码&amp;在线体验地址：<a href="https://codesandbox.io/s/nr16wzln8m" target="_blank" rel="noopener noreferrer">https://codesandbox.io/s/nr16wzln8m</a><a href="https://codesandbox.io/s/nr16wzln8m" target="_blank" rel="noopener noreferrer"> (opens new window)</a></p><h2 id="有状态组件的挂载和原理" tabindex="-1"><a class="header-anchor" href="#有状态组件的挂载和原理"><span>有状态组件的挂载和原理</span></a></h2><p>我们在“组件的本质”一章中讲到过：<strong>组件的产出是<code>VNode</code></strong>，当时我们也大致实现了有状态组件的挂载，其思路是<strong>拿到组件产出的<code>VNode</code>，并将之挂载到正确的 <code>container</code> 中</strong>，思路很简单，我们着手实现。</p><p>回顾一下我们的 <code>mount</code> 函数，如下高亮代码所示：</p><pre><code>function mount(vnode, container, isSVG) {
  const { flags } = vnode
  if (flags &amp; VNodeFlags.ELEMENT) {
    // 挂载普通标签
    mountElement(vnode, container, isSVG)
...
</code></pre><p>当 <code>VNode</code> 的 <code>flags</code> 的值属于组件时(<code>VNodeFlags.COMPONENT</code>)，则会调用 <code>mountComponent</code> 函数挂载该 <code>VNode</code>，但是组件还分为有状态组件和函数式组件，所以在 <code>mountComponent</code> 函数内部，我们需要再次对组件的类型进行区分，并使用不同的挂载方式，如下是 <code>mountComponent</code> 函数的实现：</p><div class="language-javascript line-numbers-mode" data-highlighter="prismjs" data-ext="js" data-title="js"><pre><code><span class="line">    <span class="token keyword">function</span> <span class="token function">mountComponent</span><span class="token punctuation">(</span><span class="token parameter">vnode<span class="token punctuation">,</span> container<span class="token punctuation">,</span> isSVG</span><span class="token punctuation">)</span> <span class="token punctuation">{</span></span>
...
<span class="line"></span></code></pre><div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0;"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>如下是组件 <code>MyComponent</code> 组件的实现：</p><div class="language-javascript line-numbers-mode" data-highlighter="prismjs" data-ext="js" data-title="js"><pre><code><span class="line">    <span class="token keyword">class</span> <span class="token class-name">MyComponent</span> <span class="token punctuation">{</span></span>
...
<span class="line">      <span class="token punctuation">}</span></span>
<span class="line">    <span class="token punctuation">}</span></span>
<span class="line"></span></code></pre><div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0;"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>该组件的 <code>render</code> 函数返回了它要渲染的内容，如下是使用渲染器渲染后的效果：</p><p><img src="`+k+`" alt=""></p><p>TIP</p><p>完整代码&amp;在线体验地址：<a href="https://codesandbox.io/s/2on8xyk01y" target="_blank" rel="noopener noreferrer">https://codesandbox.io/s/2on8xyk01y</a><a href="https://codesandbox.io/s/2on8xyk01y" target="_blank" rel="noopener noreferrer"> (opens new window)</a></p><p>这里再次强调一下，这就是<strong>有状态组件的挂载原理</strong> ，仅此而已。有的同学可能会产生疑惑，比如这里没有体现生命周期呀，也没有体现 <code>data</code>、<code>props</code>、<code>ref</code> 或者 <code>slots</code> 等等，实际上我们早在“组件的本质”一章中就提到过<strong>这些内容是在基本原理的基础上，再次设计的产物，它们为<code>render</code> 函数生成 <code>VNode</code> 的过程中提供数据来源服务</strong>，而<strong>组件产出<code>VNode</code> 才是永恒的核心</strong>，所以本节我们重在讲解原理，至于 <code>data</code>、<code>props</code>、<code>ref</code> 等内容属于组件实例的设计，我们会在后续的章节中统一讲解。</p><h2 id="函数式组件的挂载和原理" tabindex="-1"><a class="header-anchor" href="#函数式组件的挂载和原理"><span>函数式组件的挂载和原理</span></a></h2><p>函数式组件就更加简单了，它就是一个返回 <code>VNode</code> 的函数：</p><div class="language-javascript line-numbers-mode" data-highlighter="prismjs" data-ext="js" data-title="js"><pre><code><span class="line">    <span class="token keyword">function</span> <span class="token function">MyFunctionalComponent</span><span class="token punctuation">(</span><span class="token punctuation">)</span> <span class="token punctuation">{</span></span>
...
<span class="line">      <span class="token punctuation">)</span></span>
<span class="line">    <span class="token punctuation">}</span></span>
<span class="line"></span></code></pre><div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0;"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>在挂载函数式组件的时候，比挂载有状态组件少了一个实例化的过程，如果一个 <code>VNode</code> 描述的是函数式组件，那么其 <code>tag</code> 属性值就是该函数的引用，如下：</p><p>如下是 <code>mountFunctionalComponent</code> 函数的实现：</p><div class="language-javascript line-numbers-mode" data-highlighter="prismjs" data-ext="js" data-title="js"><pre><code><span class="line">    <span class="token keyword">function</span> <span class="token function">mountFunctionalComponent</span><span class="token punctuation">(</span><span class="token parameter">vnode<span class="token punctuation">,</span> container<span class="token punctuation">,</span> isSVG</span><span class="token punctuation">)</span> <span class="token punctuation">{</span></span>
...
<span class="line">    <span class="token punctuation">}</span></span>
<span class="line"></span></code></pre><div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0;"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>我们来测试一下我们的代码：</p><div class="language-javascript line-numbers-mode" data-highlighter="prismjs" data-ext="js" data-title="js"><pre><code><span class="line">    <span class="token keyword">const</span> compVnode <span class="token operator">=</span> <span class="token function">h</span><span class="token punctuation">(</span>MyFunctionalComponent<span class="token punctuation">)</span></span>
<span class="line">    <span class="token function">render</span><span class="token punctuation">(</span>compVnode<span class="token punctuation">,</span> document<span class="token punctuation">.</span><span class="token function">getElementById</span><span class="token punctuation">(</span><span class="token string">&#39;app&#39;</span><span class="token punctuation">)</span><span class="token punctuation">)</span></span>
<span class="line"></span></code></pre><div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0;"><div class="line-number"></div><div class="line-number"></div></div></div><p>最终的渲染效果如下：</p><p><img src="`+m+'" alt=""></p><p>TIP</p><p>完整代码&amp;在线体验地址：<a href="https://codesandbox.io/s/02nrpqkvv" target="_blank" rel="noopener noreferrer">https://codesandbox.io/s/02nrpqkvv</a><a href="https://codesandbox.io/s/02nrpqkvv" target="_blank" rel="noopener noreferrer"> (opens new window)</a></p><p>实际上如果对于 <strong>有状态组件</strong> 和 <strong>函数式组件</strong> 具体的区别不太了解的同学看到这里或许会产生疑问，觉得 <strong>有状态组件</strong> 的实例化很多余，实际上实例化是必须的，因为 <strong>有状态组件</strong> 在实例化的过程中会初始化一系列 <strong>有状态组件</strong> 所特有的东西，诸如 <code>data(或state)</code>、<code>computed</code>、<code>watch</code>、生命周期等等。而函数式组件只有 <code>props</code> 和 <code>slots</code>，它要做的工作很少，所以性能上会更好。具体的关于本地数据、<code>props</code> 数据，计算属性，插槽等的设计和实现，我们在后面的章节中统一讲解，这里给大家展示的就是最根本的原理。</p><p>阅读全文</p>',215),h=[g];function b(f,V){return a(),s("div",null,h)}const N=n(v,[["render",b],["__file","Vue-渲染器之挂载.html.vue"]]),_=JSON.parse('{"path":"/Vue-%E6%B8%B2%E6%9F%93%E5%99%A8%E4%B9%8B%E6%8C%82%E8%BD%BD.html","title":"","lang":"zh-CN","frontmatter":{"description":"原文链接: https://interview.poetries.top/principle-docs/vue/07-%E6%B8%B2%E6%9F%93%E5%99%A8%E4%B9%8B%E6%8C%82%E8%BD%BD.html sidebarDepth: 4 TIP 本章主要讲解渲染器将各种类型的 VNode 挂载为真实 DOM 的原理，阅读...","head":[["meta",{"property":"og:url","content":"https://interview.leeguoo.com/Vue-%E6%B8%B2%E6%9F%93%E5%99%A8%E4%B9%8B%E6%8C%82%E8%BD%BD.html"}],["meta",{"property":"og:site_name","content":"前端面试题集锦"}],["meta",{"property":"og:description","content":"原文链接: https://interview.poetries.top/principle-docs/vue/07-%E6%B8%B2%E6%9F%93%E5%99%A8%E4%B9%8B%E6%8C%82%E8%BD%BD.html sidebarDepth: 4 TIP 本章主要讲解渲染器将各种类型的 VNode 挂载为真实 DOM 的原理，阅读..."}],["meta",{"property":"og:type","content":"article"}],["meta",{"property":"og:image","content":"https://interview.leeguoo.com/images/s_poetries_work_uploads_2024_02_1e74bad4264c445b.png"}],["meta",{"property":"og:locale","content":"zh-CN"}],["meta",{"property":"og:updated_time","content":"2024-06-05T18:28:42.000Z"}],["meta",{"property":"article:modified_time","content":"2024-06-05T18:28:42.000Z"}],["script",{"type":"application/ld+json"},"{\\"@context\\":\\"https://schema.org\\",\\"@type\\":\\"Article\\",\\"headline\\":\\"\\",\\"image\\":[\\"https://interview.leeguoo.com/images/s_poetries_work_uploads_2024_02_1e74bad4264c445b.png\\",\\"https://interview.leeguoo.com/images/s_poetries_work_uploads_2024_02_b246f283925e3893.png\\",\\"https://interview.leeguoo.com/images/s_poetries_work_uploads_2024_02_cdd5ce233259e7d2.png\\",\\"https://interview.leeguoo.com/images/s_poetries_work_uploads_2024_02_c307e84482200af6.png\\",\\"https://interview.leeguoo.com/images/s_poetries_work_uploads_2024_02_bf5c631339510653.png\\",\\"https://interview.leeguoo.com/images/s_poetries_work_uploads_2024_02_8a6c8bc46e52701b.png\\",\\"https://interview.leeguoo.com/images/s_poetries_work_uploads_2024_02_10301451530c73bc.png\\",\\"https://interview.leeguoo.com/images/s_poetries_work_uploads_2024_02_a5574e9cd8a098c5.png\\",\\"https://interview.leeguoo.com/images/s_poetries_work_uploads_2024_02_a36595be236a4c42.png\\",\\"https://interview.leeguoo.com/images/s_poetries_work_uploads_2024_02_f49b158a31644537.png\\",\\"https://interview.leeguoo.com/images/s_poetries_work_uploads_2024_02_c7eade36dfee3450.png\\"],\\"dateModified\\":\\"2024-06-05T18:28:42.000Z\\",\\"author\\":[]}"]]},"headers":[{"level":2,"title":"sidebarDepth: 4","slug":"sidebardepth-4","link":"#sidebardepth-4","children":[]},{"level":2,"title":"责任重大的渲染器","slug":"责任重大的渲染器","link":"#责任重大的渲染器","children":[]},{"level":2,"title":"挂载普通标签元素","slug":"挂载普通标签元素","link":"#挂载普通标签元素","children":[{"level":3,"title":"基本原理","slug":"基本原理","link":"#基本原理","children":[]},{"level":3,"title":"class的处理","slug":"class的处理","link":"#class的处理","children":[]},{"level":3,"title":"Attributes 和 DOM Properties","slug":"attributes-和-dom-properties","link":"#attributes-和-dom-properties","children":[]},{"level":3,"title":"事件的处理","slug":"事件的处理","link":"#事件的处理","children":[]}]},{"level":2,"title":"挂载纯文本、Fragment 和 Portal","slug":"挂载纯文本、fragment-和-portal","link":"#挂载纯文本、fragment-和-portal","children":[{"level":3,"title":"挂载文本节点","slug":"挂载文本节点","link":"#挂载文本节点","children":[]},{"level":3,"title":"挂载 Fragment","slug":"挂载-fragment","link":"#挂载-fragment","children":[]},{"level":3,"title":"挂载 Portal","slug":"挂载-portal","link":"#挂载-portal","children":[]}]},{"level":2,"title":"有状态组件的挂载和原理","slug":"有状态组件的挂载和原理","link":"#有状态组件的挂载和原理","children":[]},{"level":2,"title":"函数式组件的挂载和原理","slug":"函数式组件的挂载和原理","link":"#函数式组件的挂载和原理","children":[]}],"git":{"updatedTime":1717612122000,"contributors":[{"name":"guoli","email":"guoli@zhihu.com","commits":2}]},"autoDesc":true,"filePathRelative":"Vue-渲染器之挂载.md","excerpt":"<p>原文链接: <a href=\\"https://interview.poetries.top/principle-docs/vue/07-%E6%B8%B2%E6%9F%93%E5%99%A8%E4%B9%8B%E6%8C%82%E8%BD%BD.html\\" target=\\"_blank\\" rel=\\"noopener noreferrer\\">https://interview.poetries.top/principle-docs/vue/07-%E6%B8%B2%E6%9F%93%E5%99%A8%E4%B9%8B%E6%8C%82%E8%BD%BD.html</a></p>\\n<hr>\\n"}');export{N as comp,_ as data};
