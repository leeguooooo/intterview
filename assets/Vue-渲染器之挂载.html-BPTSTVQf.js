import{_ as n,c as s,o as e,a}from"./app-BuhGo8SK.js";const o="/images/s_poetries_work_uploads_2024_02_1e74bad4264c445b.webp",t="/images/s_poetries_work_uploads_2024_02_b246f283925e3893.webp",c="/images/s_poetries_work_uploads_2024_02_cdd5ce233259e7d2.webp",p="/images/s_poetries_work_uploads_2024_02_c307e84482200af6.webp",l="/images/s_poetries_work_uploads_2024_02_bf5c631339510653.webp",d="/images/s_poetries_work_uploads_2024_02_8a6c8bc46e52701b.webp",i="/images/s_poetries_work_uploads_2024_02_10301451530c73bc.webp",r="/images/s_poetries_work_uploads_2024_02_a5574e9cd8a098c5.webp",u="/images/s_poetries_work_uploads_2024_02_a36595be236a4c42.webp",k="/images/s_poetries_work_uploads_2024_02_f49b158a31644537.webp",m="/images/s_poetries_work_uploads_2024_02_c7eade36dfee3450.webp",v={},g=a(`<h2 id="简版速记" tabindex="-1"><a class="header-anchor" href="#简版速记"><span>简版速记</span></a></h2><ul><li><strong>渲染器两阶段</strong>：<code>mount</code>（首次挂载，无旧 VNode）和 <code>patch</code>（更新，新旧 VNode 对比打补丁）。</li><li><strong>mount 分派</strong>：根据 <code>vnode.flags</code> 位运算区分类型，分别调用 <code>mountElement</code> / <code>mountComponent</code> / <code>mountText</code> / <code>mountFragment</code> / <code>mountPortal</code>。</li><li><strong>mountElement 四步</strong>：① 保存 <code>vnode.el</code> 引用；② 应用 <code>VNodeData</code>（style / class / attr / event）；③ 递归挂载 <code>children</code>（区分单子/多子，调用通用 <code>mount</code> 而非 <code>mountElement</code>）；④ SVG 用 <code>createElementNS</code> 并向下传递 <code>isSVG</code> 参数。</li><li><strong>class 处理</strong>：底层存字符串；应用层支持数组 + 对象，<code>h</code> 函数/编译器负责序列化。</li><li><strong>Attr vs DOM Prop</strong>：<code>value / checked / selected / muted</code> 及含大写字母的属性用 <code>el[key] = ...</code>；其余用 <code>setAttribute</code>；<code>checked</code> 等布尔属性不能用 <code>setAttribute(&#39;checked&#39;, false)</code> 置 <code>false</code>（会被转为字符串 <code>&#39;false&#39;</code>，仍然生效）。</li><li><strong>事件</strong>：<code>VNodeData</code> 中以 <code>&#39;on&#39;</code> 开头的 key（如 <code>onclick</code>）通过 <code>el.addEventListener(key.slice(2), handler)</code> 注册。</li><li><strong>Fragment</strong>：<code>tag</code> 为 <code>null</code>，挂载等同于挂载 <code>children</code>；<code>vnode.el</code> 指向第一个子节点（空片段用空文本节点占位）。</li><li><strong>Portal</strong>：<code>tag</code> 为挂载目标选择器或 DOM 元素；子节点挂载到 <code>target</code> 而非 <code>container</code>；<code>vnode.el</code> 指向插入到 <code>container</code> 中的空文本占位节点（保持事件冒泡位置正确）。</li><li><strong>有状态组件挂载</strong>：<code>new vnode.tag()</code> 实例化 → <code>instance.render()</code> 得 VNode → <code>mount</code> 挂载 → <code>instance.$el = vnode.el = instance.$vnode.el</code>。</li><li><strong>函数式组件挂载</strong>：直接调用 <code>vnode.tag()</code> 得 VNode → <code>mount</code> → <code>vnode.el</code>，无实例化，性能更好；无 data/computed/watch，只有 props 和 slots。</li></ul><hr><h2 id="sidebardepth-4" tabindex="-1"><a class="header-anchor" href="#sidebardepth-4"><span>sidebarDepth: 4</span></a></h2><p>TIP</p><p>本章主要讲解渲染器将各种类型的 <code>VNode</code> 挂载为真实 DOM 的原理，阅读本章内容你将对 <code>Fragment</code> 和 <code>Portal</code> 有更加深入的理解，同时渲染器对有状态组件和函数式组件的挂载实际上也透露了有状态组件和函数式组件的实现原理，这都会包含在本章的内容之中。另外本章的代码将使用上一章所编写的 <code>h</code> 函数，所以请确保你已经阅读了上一章的内容。</p><h2 id="责任重大的渲染器" tabindex="-1"><a class="header-anchor" href="#责任重大的渲染器"><span>责任重大的渲染器</span></a></h2><p>所谓渲染器，简单的说就是将 <code>Virtual DOM</code> 渲染成特定平台下真实 <code>DOM</code> 的工具(就是一个函数，通常叫 <code>render</code>)，渲染器的工作流程分为两个阶段：<code>mount</code> 和 <code>patch</code>，如果旧的 <code>VNode</code> 存在，则会使用新的 <code>VNode</code> 与旧的 <code>VNode</code> 进行对比，试图以最小的资源开销完成 <code>DOM</code> 的更新，这个过程就叫 <code>patch</code>，或“打补丁”。如果旧的 <code>VNode</code> 不存在，则直接将新的 <code>VNode</code> 挂载成全新的 <code>DOM</code>，这个过程叫做 <code>mount</code>。</p><p>通常渲染器接收两个参数，第一个参数是将要被渲染的 <code>VNode</code> 对象，第二个参数是一个用来承载内容的容器(<code>container</code>)，通常也叫挂载点，如下代码所示：</p><pre><code>function render(vnode, container) {
...
</code></pre><p>如上代码可知，我们为 <code>elementVnode</code> 添加了一个子节点，该子节点是一个边长为 <code>50px</code> 的绿色正方形，使用渲染器渲染修改后的 <code>elementVnode</code> 的效果如下：</p><p><img src="`+c+`" alt=""></p><p>接着我们来看最后一个问题：<strong>不能严谨地处理<code>SVG</code> 标签</strong>，在之前的 <code>mountElement</code> 函数中我们使用 <code>document.createElement</code> 函数创建DOM元素，但是对于 <code>SVG</code> 标签，更加严谨的方式是使用 <code>document.createElementNS</code> 函数，修改 <code>mountElement</code> 如下：</p><pre><code>function mountElement(vnode, container) {
...
</code></pre><p>那么这样设计有什么意义呢？这是因为在 <code>patch</code> 阶段对DOM元素进行移动时，应该确保将其放到正确的位置，而不应该始终使用 <code>appendChild</code> 函数，有时需要使用 <code>insertBefore</code> 函数，这时候我们就需要拿到相应的节点引用，这时候 <code>vnode.el</code> 属性是必不可少的，就像上面的代码中即使 <code>Fragment</code> 没有子节点我们依然需要一个占位的空文本节点作为位置的引用。</p><p>TIP</p><p>完整代码&amp;在线体验地址：<a href="https://codesandbox.io/s/109r8nlwk4" target="_blank" rel="noopener noreferrer">https://codesandbox.io/s/109r8nlwk4</a><a href="https://codesandbox.io/s/109r8nlwk4" target="_blank" rel="noopener noreferrer"> (opens new window)</a></p><h3 id="挂载-portal" tabindex="-1"><a class="header-anchor" href="#挂载-portal"><span>挂载 Portal</span></a></h3><blockquote><p>补充(现代做法)：Vue 3 正式 API 中 <code>Portal</code> 已重命名为 <code>&lt;Teleport&gt;</code>，用法为 <code>&lt;Teleport to=&quot;#portal-box&quot;&gt;...&lt;/Teleport&gt;</code>。本节所讲的 <code>mountPortal</code> 实现原理与之完全对应，理解底层实现后使用 <code>&lt;Teleport&gt;</code> 无任何障碍。</p></blockquote><p>实际上 <code>Portal</code> 可以不严谨地认为是<strong>可以被到处挂载的<code>Fragment</code></strong>。类型为 <code>Fragment</code> 的 <code>VNode</code> 其 <code>tag</code> 属性值为 <code>null</code>，而类型是 <code>Portal</code> 的 <code>VNode</code> 其 <code>tag</code> 属性值为挂载点(选择器或真实DOM元素)。实现 <code>Portal</code> 的关键是要将其 <code>VNode</code> 的 <code>children</code> 中所包含的子 <code>VNode</code> 挂载到 <code>tag</code> 属性所指向的挂载点，<code>mountPortal</code> 函数的实现如下：</p><pre><code>function mountPortal(vnode, container) {
...
<span class="line">    <span class="token function">render</span><span class="token punctuation">(</span>compVnode<span class="token punctuation">,</span> document<span class="token punctuation">.</span><span class="token function">getElementById</span><span class="token punctuation">(</span><span class="token string">&#39;app&#39;</span><span class="token punctuation">)</span><span class="token punctuation">)</span></span>
<span class="line"></span></code></pre><div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0;"><div class="line-number"></div><div class="line-number"></div></div></div><p>最终的渲染效果如下：</p><p><img src="`+m+'" alt=""></p><p>TIP</p><p>完整代码&amp;在线体验地址：<a href="https://codesandbox.io/s/02nrpqkvv" target="_blank" rel="noopener noreferrer">https://codesandbox.io/s/02nrpqkvv</a><a href="https://codesandbox.io/s/02nrpqkvv" target="_blank" rel="noopener noreferrer"> (opens new window)</a></p><p>实际上如果对于 <strong>有状态组件</strong> 和 <strong>函数式组件</strong> 具体的区别不太了解的同学看到这里或许会产生疑问，觉得 <strong>有状态组件</strong> 的实例化很多余，实际上实例化是必须的，因为 <strong>有状态组件</strong> 在实例化的过程中会初始化一系列 <strong>有状态组件</strong> 所特有的东西，诸如 <code>data(或state)</code>、<code>computed</code>、<code>watch</code>、生命周期等等。而函数式组件只有 <code>props</code> 和 <code>slots</code>，它要做的工作很少，所以性能上会更好。具体的关于本地数据、<code>props</code> 数据，计算属性，插槽等的设计和实现，我们在后面的章节中统一讲解，这里给大家展示的就是最根本的原理。</p><p>阅读全文</p>',217),h=[g];function b(f,V){return e(),s("div",null,h)}const y=n(v,[["render",b],["__file","Vue-渲染器之挂载.html.vue"]]),_=JSON.parse(`{"path":"/Vue-%E6%B8%B2%E6%9F%93%E5%99%A8%E4%B9%8B%E6%8C%82%E8%BD%BD.html","title":"Vue 渲染器之挂载","lang":"zh-CN","frontmatter":{"title":"Vue 渲染器之挂载","description":"简版速记 渲染器两阶段：mount（首次挂载，无旧 VNode）和 patch（更新，新旧 VNode 对比打补丁）。 mount 分派：根据 vnode.flags 位运算区分类型，分别调用 mountElement / mountComponent / mountText / mountFragment / mountPortal。 mountEl...","head":[["meta",{"property":"og:url","content":"https://interview.leeguoo.com/Vue-%E6%B8%B2%E6%9F%93%E5%99%A8%E4%B9%8B%E6%8C%82%E8%BD%BD.html"}],["meta",{"property":"og:site_name","content":"前端面试题集锦"}],["meta",{"property":"og:title","content":"Vue 渲染器之挂载"}],["meta",{"property":"og:description","content":"简版速记 渲染器两阶段：mount（首次挂载，无旧 VNode）和 patch（更新，新旧 VNode 对比打补丁）。 mount 分派：根据 vnode.flags 位运算区分类型，分别调用 mountElement / mountComponent / mountText / mountFragment / mountPortal。 mountEl..."}],["meta",{"property":"og:type","content":"article"}],["meta",{"property":"og:image","content":"https://interview.leeguoo.com/images/s_poetries_work_uploads_2024_02_1e74bad4264c445b.webp"}],["meta",{"property":"og:locale","content":"zh-CN"}],["meta",{"property":"og:updated_time","content":"2026-07-06T04:15:51.000Z"}],["meta",{"property":"article:modified_time","content":"2026-07-06T04:15:51.000Z"}],["script",{"type":"application/ld+json"},"{\\"@context\\":\\"https://schema.org\\",\\"@type\\":\\"Article\\",\\"headline\\":\\"Vue 渲染器之挂载\\",\\"image\\":[\\"https://interview.leeguoo.com/images/s_poetries_work_uploads_2024_02_1e74bad4264c445b.webp\\",\\"https://interview.leeguoo.com/images/s_poetries_work_uploads_2024_02_b246f283925e3893.webp\\",\\"https://interview.leeguoo.com/images/s_poetries_work_uploads_2024_02_cdd5ce233259e7d2.webp\\",\\"https://interview.leeguoo.com/images/s_poetries_work_uploads_2024_02_c307e84482200af6.webp\\",\\"https://interview.leeguoo.com/images/s_poetries_work_uploads_2024_02_bf5c631339510653.webp\\",\\"https://interview.leeguoo.com/images/s_poetries_work_uploads_2024_02_8a6c8bc46e52701b.webp\\",\\"https://interview.leeguoo.com/images/s_poetries_work_uploads_2024_02_10301451530c73bc.webp\\",\\"https://interview.leeguoo.com/images/s_poetries_work_uploads_2024_02_a5574e9cd8a098c5.webp\\",\\"https://interview.leeguoo.com/images/s_poetries_work_uploads_2024_02_a36595be236a4c42.webp\\",\\"https://interview.leeguoo.com/images/s_poetries_work_uploads_2024_02_f49b158a31644537.webp\\",\\"https://interview.leeguoo.com/images/s_poetries_work_uploads_2024_02_c7eade36dfee3450.webp\\"],\\"dateModified\\":\\"2026-07-06T04:15:51.000Z\\",\\"author\\":[]}"]]},"headers":[{"level":2,"title":"简版速记","slug":"简版速记","link":"#简版速记","children":[]},{"level":2,"title":"sidebarDepth: 4","slug":"sidebardepth-4","link":"#sidebardepth-4","children":[]},{"level":2,"title":"责任重大的渲染器","slug":"责任重大的渲染器","link":"#责任重大的渲染器","children":[]},{"level":2,"title":"挂载普通标签元素","slug":"挂载普通标签元素","link":"#挂载普通标签元素","children":[{"level":3,"title":"基本原理","slug":"基本原理","link":"#基本原理","children":[]},{"level":3,"title":"class的处理","slug":"class的处理","link":"#class的处理","children":[]},{"level":3,"title":"Attributes 和 DOM Properties","slug":"attributes-和-dom-properties","link":"#attributes-和-dom-properties","children":[]},{"level":3,"title":"事件的处理","slug":"事件的处理","link":"#事件的处理","children":[]}]},{"level":2,"title":"挂载纯文本、Fragment 和 Portal","slug":"挂载纯文本、fragment-和-portal","link":"#挂载纯文本、fragment-和-portal","children":[{"level":3,"title":"挂载文本节点","slug":"挂载文本节点","link":"#挂载文本节点","children":[]},{"level":3,"title":"挂载 Fragment","slug":"挂载-fragment","link":"#挂载-fragment","children":[]},{"level":3,"title":"挂载 Portal","slug":"挂载-portal","link":"#挂载-portal","children":[]}]},{"level":2,"title":"有状态组件的挂载和原理","slug":"有状态组件的挂载和原理","link":"#有状态组件的挂载和原理","children":[]},{"level":2,"title":"函数式组件的挂载和原理","slug":"函数式组件的挂载和原理","link":"#函数式组件的挂载和原理","children":[]}],"git":{"updatedTime":1783311351000,"contributors":[{"name":"leeguooooo","email":"guoli@zhihu.com","commits":4},{"name":"guoli","email":"guoli@zhihu.com","commits":2}]},"autoDesc":true,"filePathRelative":"Vue-渲染器之挂载.md","excerpt":"<h2>简版速记</h2>\\n<ul>\\n<li><strong>渲染器两阶段</strong>：<code>mount</code>（首次挂载，无旧 VNode）和 <code>patch</code>（更新，新旧 VNode 对比打补丁）。</li>\\n<li><strong>mount 分派</strong>：根据 <code>vnode.flags</code> 位运算区分类型，分别调用 <code>mountElement</code> / <code>mountComponent</code> / <code>mountText</code> / <code>mountFragment</code> / <code>mountPortal</code>。</li>\\n<li><strong>mountElement 四步</strong>：① 保存 <code>vnode.el</code> 引用；② 应用 <code>VNodeData</code>（style / class / attr / event）；③ 递归挂载 <code>children</code>（区分单子/多子，调用通用 <code>mount</code> 而非 <code>mountElement</code>）；④ SVG 用 <code>createElementNS</code> 并向下传递 <code>isSVG</code> 参数。</li>\\n<li><strong>class 处理</strong>：底层存字符串；应用层支持数组 + 对象，<code>h</code> 函数/编译器负责序列化。</li>\\n<li><strong>Attr vs DOM Prop</strong>：<code>value / checked / selected / muted</code> 及含大写字母的属性用 <code>el[key] = ...</code>；其余用 <code>setAttribute</code>；<code>checked</code> 等布尔属性不能用 <code>setAttribute('checked', false)</code> 置 <code>false</code>（会被转为字符串 <code>'false'</code>，仍然生效）。</li>\\n<li><strong>事件</strong>：<code>VNodeData</code> 中以 <code>'on'</code> 开头的 key（如 <code>onclick</code>）通过 <code>el.addEventListener(key.slice(2), handler)</code> 注册。</li>\\n<li><strong>Fragment</strong>：<code>tag</code> 为 <code>null</code>，挂载等同于挂载 <code>children</code>；<code>vnode.el</code> 指向第一个子节点（空片段用空文本节点占位）。</li>\\n<li><strong>Portal</strong>：<code>tag</code> 为挂载目标选择器或 DOM 元素；子节点挂载到 <code>target</code> 而非 <code>container</code>；<code>vnode.el</code> 指向插入到 <code>container</code> 中的空文本占位节点（保持事件冒泡位置正确）。</li>\\n<li><strong>有状态组件挂载</strong>：<code>new vnode.tag()</code> 实例化 → <code>instance.render()</code> 得 VNode → <code>mount</code> 挂载 → <code>instance.$el = vnode.el = instance.$vnode.el</code>。</li>\\n<li><strong>函数式组件挂载</strong>：直接调用 <code>vnode.tag()</code> 得 VNode → <code>mount</code> → <code>vnode.el</code>，无实例化，性能更好；无 data/computed/watch，只有 props 和 slots。</li>\\n</ul>"}`);export{y as comp,_ as data};
