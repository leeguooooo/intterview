...
</code></pre><p>这一步更新完成之后，新的索引关系可以用下图来表示：</p><p><img src="`+x+'" alt=""></p><p>由于 <code>li-d</code> 节点所对应的真实 DOM 元素已经更新完成且被移动，所以现在真实 DOM 的顺序是：<code>li-d</code>、<code>li-a</code>、<code>li-b</code>、<code>li-c</code>，如下图所示：</p><p><img src="'+N+`" alt=""></p><p>这样，一次比对就完成了，并且位置索引已经更新，我们需要进行下轮的比对，那么什么时候比对才能结束呢？如下代码所示：</p><div class="language-javascript line-numbers-mode" data-highlighter="prismjs" data-ext="js" data-title="js"><pre class="language-javascript"><code><span class="line">    <span class="token keyword">while</span> <span class="token punctuation">(</span>oldStartIdx <span class="token operator">&lt;=</span> oldEndIdx <span class="token operator">&amp;&amp;</span> newStartIdx <span class="token operator">&lt;=</span> newEndIdx<span class="token punctuation">)</span> <span class="token punctuation">{</span></span>
...
<span class="line"></span></code></pre><div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0;"><div class="line-number"></div><div class="line-number"></div></div></div><p>我们通过肉眼可以很容易的发现，这两段文本头部和尾部分别有一段相同的文本：</p><p><img src="`+R+`" alt=""></p><p>所以真正需要进行 <code>Diff</code> 的部分就变成了：</p><div class="language-javascript line-numbers-mode" data-highlighter="prismjs" data-ext="js" data-title="js"><pre class="language-javascript"><code><span class="line">    <span class="token literal-property property">text1</span><span class="token operator">:</span> vue</span>
...
<span class="line"></span></code></pre><div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0;"><div class="line-number"></div><div class="line-number"></div></div></div><p>这代表文本被删除。</p><p>很显然，该预处理过程在上例的情况下能够避免 <code>Diff</code> 算法的执行，从而提高 <code>Diff</code> 效率。当然，换一个角度来看的话，这本身也是 <code>Diff</code> 策略的一部分，不过这显然要更高效。所以我们能否将此预处理步骤应用到 <code>VNode</code> 的 <code>Diff</code> 中呢？当然可以，来看下面的例子：</p><p><img src="`+z+`" alt=""></p><p>如上图所示，新旧 <code>children</code> 拥有相同的前缀节点和后缀节点，对于前缀节点，我们可以建立一个索引，指向新旧 <code>children</code> 中的第一个节点，并逐步向后遍历，直到遇到两个拥有不同 <code>key</code> 值的节点为止，如下代码所示：</p><div class="language-javascript line-numbers-mode" data-highlighter="prismjs" data-ext="js" data-title="js"><pre class="language-javascript"><code><span class="line">    <span class="token comment">// 更新相同的前缀节点</span></span>
...
</code></pre><p>那么这个数组的作用是什么呢？通过上图可以发现，该数组中的每一个元素分别与新 <code>children</code> 中剩余未处理的节点对应，实际上 <code>source</code> 数组将用来存储<strong>新<code>children</code> 中的节点在旧 <code>children</code> 中的位置，后面将会使用它计算出一个最长递增子序列，并用于 DOM 移动</strong>。如下图所示：</p><p><img src="`+K+`" alt=""></p><p>我们可以通过两层 <code>for</code> 循环来完成这个工作，外层循环用于遍历旧 <code>children</code>，内层循环用于遍历新 <code>children</code>：</p><div class="language-javascript line-numbers-mode" data-highlighter="prismjs" data-ext="js" data-title="js"><pre class="language-javascript"><code><span class="line">    <span class="token keyword">const</span> prevStart <span class="token operator">=</span> j</span>
