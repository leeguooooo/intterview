...
</code></pre><p>TIP</p><p>完整代码&amp;在线体验地址：<a href="https://codesandbox.io/s/62x41myyrz" target="_blank" rel="noopener noreferrer">https://codesandbox.io/s/62x41myyrz</a><a href="https://codesandbox.io/s/62x41myyrz" target="_blank" rel="noopener noreferrer"> (opens new window)</a></p><p>现在对于旧的 <code>children</code> 类型来说，我们只剩下最后一种情况没有处理了，就是当旧的 <code>children</code> 类型为多个子节点时，同样的我们来画一张图：</p><p><img src="`+p+`" alt=""></p><p>如上图所示，当旧的 <code>children</code> 类型为多个子节点时，新的 <code>children</code> 类型有三种情况，不同的情况采用不同的操作：</p><ul><li>情况一：有多个旧的子节点，但新的子节点是单个子节点，这时只需要把所有旧的子节点移除，再将新的单个子节点添加到容器元素即可。</li><li>情况二：有多个旧的子节点，但没有新的子节点，这时只需要把所有旧的子节点移除即可。</li><li>情况三：新旧子节点都是多个子节点，这时将进入到至关重要的一步，即核心 <code>diff</code> 算法的用武之地。</li></ul><p>实际上在整个新旧 <code>children</code> 的比对中，只有当新旧子节点都是多个子节点时才有必要进行真正的核心 <code>diff</code>，从而尽可能的复用子节点。</p><p>对于<strong>情况一</strong> 和<strong>情况二</strong> 而言，实现起来相当容易，如下代码所示：</p><pre><code>function patchChildren(
  prevChildFlags,
  nextChildFlags,
  prevChildren,
  nextChildren,
