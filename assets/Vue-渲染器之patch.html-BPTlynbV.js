import{_ as n}from"./s_poetries_work_uploads_2024_02_7cb972d068b2d405-2w8zVw84.js";import{_ as s,c as a,o as e,a as p}from"./app-C0SBBF5d.js";const o={},t=p(`<h2 id="简版速记" tabindex="-1"><a class="header-anchor" href="#简版速记"><span>简版速记</span></a></h2><blockquote><p><strong>patch 核心流程一览（面试速答）</strong></p></blockquote><table><thead><tr><th>场景</th><th>策略</th></tr></thead><tbody><tr><td>新旧 VNode <strong>类型不同</strong></td><td><code>replaceVNode</code>：移除旧 DOM，挂载新 VNode</td></tr><tr><td>同类型<strong>标签元素</strong></td><td><code>patchElement</code>：tag 不同则替换；tag 相同则依次 patchData + patchChildren</td></tr><tr><td><strong>VNodeData 更新</strong></td><td><code>patchData</code>：遍历新数据全量应用；再遍历旧数据移除多余项（style / class / 事件 / DOM Prop / Attr）</td></tr><tr><td><strong>子节点更新</strong></td><td><code>patchChildren</code>：3×3 = 9 种情况；只有<strong>新旧均为多子节点</strong>时才需核心 diff（本章暂简化为全删全加）</td></tr><tr><td><strong>文本节点</strong></td><td><code>patchText</code>：比较 <code>children</code> 文本，不同则设 <code>el.nodeValue</code></td></tr><tr><td><strong>Fragment</strong></td><td><code>patchFragment</code>：直接调 <code>patchChildren</code> + 按子节点类型更新 <code>el</code> 引用</td></tr><tr><td><strong>Portal</strong></td><td><code>patchPortal</code>：先在旧容器中更新子节点，再用 <code>appendChild</code> 将子节点搬运到新容器（移动已存在的 DOM 节点）</td></tr><tr><td><strong>有状态组件—主动更新</strong></td><td><code>instance._update()</code>：<code>_mounted</code> 为真时走 <code>patch(旧VNode, 新VNode)</code>；否则走初次 <code>mount</code></td></tr><tr><td><strong>有状态组件—被动更新</strong></td><td><code>patchComponent</code>：组件类不同则 <code>replaceVNode</code>（触发 <code>unmounted</code>）；相同则更新 <code>$props</code> 再调 <code>_update()</code></td></tr><tr><td><strong>函数式组件更新</strong></td><td>通过 <code>vnode.handle</code>（prev / next / container / update）协调新旧 VNode；<code>patchComponent</code> 更新 handle 后调 <code>handle.update()</code></td></tr></tbody></table><blockquote><p>在上一章中我们讲解并实现了渲染器的挂载逻辑，本质上就是将各种类型的 <code>VNode</code> 渲染成真实DOM的过程。渲染器除了将全新的 <code>VNode</code> 挂载成真实DOM之外，它的另外一个职责是负责对新旧 <code>VNode</code> 进行比对，并以合适的方式更新DOM，也就是我们常说的 <code>patch</code>。本章内容除了让你了解基本的比对逻辑之外，还讲述了在新旧 <code>VNode</code> 比对的过程中应该遵守怎样的原则，让我们开始吧！</p></blockquote><h2 id="基本原则" tabindex="-1"><a class="header-anchor" href="#基本原则"><span>基本原则</span></a></h2><p>通常重渲染(<code>re- render</code>)是由组件的更新开始的，因为在框架的使用层面开发者通过变更数据状态从而引起框架内部对UI的自动更新，但是组件的更新本质上还是对真实DOM的更新，或者说是对标签元素的更新，所以我们就优先来看一下如何更新一个标签元素。</p><p>我们首先回顾一下渲染器的代码，如下：</p><pre><code>function render(vnode, container) {
...
</code></pre><p>如上高亮代码所示，由于我们在 <code>patchComponent</code> 函数内已经更新过了 <code>handle</code> 对象，所以此时我们可以通过 <code>vnode.handle.prev</code> 和 <code>vnode.handle.next</code> 分别拿到旧的组件 <code>VNode</code> 和新的组件 <code>VNode</code>，但大家不要搞混的是：<code>prevVNode</code> 和 <code>nextVNode</code> 是用来描述函数式组件的 <code>VNode</code>，并非函数式组件所产出的 <code>VNode</code>。因为函数式组件所产出的 <code>VNode</code> 存放在用来描述函数式组件的 <code>VNode</code> 的 <code>children</code> 属性中，所以在如上代码中我们通过 <code>prevVNode.children</code> 拿到了组件所产出的旧的 <code>VNode</code> 即 <code>prevTree</code>，接着使用新的 <code>props</code> 重新调用组件函数 <code>nextVNode.tag(props)</code> 得到新产出的 <code>VNode</code> 即 <code>nextTree</code>，有了 <code>prevTree</code> 和 <code>nextTree</code> 之后我们就可以调用 <code>patch</code> 函数执行更新操作了。</p><p>以上就是函数式组件的更新过程。</p><p>TIP</p><p>完整代码&amp;在线体验地址：<a href="https://codesandbox.io/s/5yz63qqx7p" target="_blank" rel="noopener noreferrer">https://codesandbox.io/s/5yz63qqx7p</a><a href="https://codesandbox.io/s/5yz63qqx7p" target="_blank" rel="noopener noreferrer"> (opens new window)</a></p><p>阅读全文</p>`,260),c=[t];function l(d,i){return e(),a("div",null,c)}const k=s(o,[["render",l],["__file","Vue-渲染器之patch.html.vue"]]),v=JSON.parse('{"path":"/Vue-%E6%B8%B2%E6%9F%93%E5%99%A8%E4%B9%8Bpatch.html","title":"Vue 渲染器之patch","lang":"zh-CN","frontmatter":{"title":"Vue 渲染器之patch","description":"简版速记 patch 核心流程一览（面试速答） 在上一章中我们讲解并实现了渲染器的挂载逻辑，本质上就是将各种类型的 VNode 渲染成真实DOM的过程。渲染器除了将全新的 VNode 挂载成真实DOM之外，它的另外一个职责是负责对新旧 VNode 进行比对，并以合适的方式更新DOM，也就是我们常说的 patch。本章内容除了让你了解基本的比对逻辑之外，...","head":[["meta",{"property":"og:url","content":"https://interview.leeguoo.com/Vue-%E6%B8%B2%E6%9F%93%E5%99%A8%E4%B9%8Bpatch.html"}],["meta",{"property":"og:site_name","content":"前端面试题集锦"}],["meta",{"property":"og:title","content":"Vue 渲染器之patch"}],["meta",{"property":"og:description","content":"简版速记 patch 核心流程一览（面试速答） 在上一章中我们讲解并实现了渲染器的挂载逻辑，本质上就是将各种类型的 VNode 渲染成真实DOM的过程。渲染器除了将全新的 VNode 挂载成真实DOM之外，它的另外一个职责是负责对新旧 VNode 进行比对，并以合适的方式更新DOM，也就是我们常说的 patch。本章内容除了让你了解基本的比对逻辑之外，..."}],["meta",{"property":"og:type","content":"article"}],["meta",{"property":"og:image","content":"https://interview.leeguoo.com/images/s_poetries_work_uploads_2024_02_7cb972d068b2d405.webp"}],["meta",{"property":"og:locale","content":"zh-CN"}],["meta",{"property":"og:updated_time","content":"2026-07-06T04:15:51.000Z"}],["meta",{"property":"article:modified_time","content":"2026-07-06T04:15:51.000Z"}],["script",{"type":"application/ld+json"},"{\\"@context\\":\\"https://schema.org\\",\\"@type\\":\\"Article\\",\\"headline\\":\\"Vue 渲染器之patch\\",\\"image\\":[\\"https://interview.leeguoo.com/images/s_poetries_work_uploads_2024_02_7cb972d068b2d405.webp\\"],\\"dateModified\\":\\"2026-07-06T04:15:51.000Z\\",\\"author\\":[]}"]]},"headers":[{"level":2,"title":"简版速记","slug":"简版速记","link":"#简版速记","children":[]},{"level":2,"title":"基本原则","slug":"基本原则","link":"#基本原则","children":[]},{"level":2,"title":"替换 VNode","slug":"替换-vnode","link":"#替换-vnode","children":[]},{"level":2,"title":"更新标签元素","slug":"更新标签元素","link":"#更新标签元素","children":[{"level":3,"title":"更新标签元素的基本原则","slug":"更新标签元素的基本原则","link":"#更新标签元素的基本原则","children":[]},{"level":3,"title":"更新 VNodeData","slug":"更新-vnodedata","link":"#更新-vnodedata","children":[]},{"level":3,"title":"更新子节点","slug":"更新子节点","link":"#更新子节点","children":[]}]},{"level":2,"title":"更新文本节点","slug":"更新文本节点","link":"#更新文本节点","children":[]},{"level":2,"title":"更新 Fragment","slug":"更新-fragment","link":"#更新-fragment","children":[]},{"level":2,"title":"更新 Portal","slug":"更新-portal","link":"#更新-portal","children":[]},{"level":2,"title":"有状态组件的更新","slug":"有状态组件的更新","link":"#有状态组件的更新","children":[{"level":3,"title":"主动更新","slug":"主动更新","link":"#主动更新","children":[]},{"level":3,"title":"初步了解组件的外部状态 props","slug":"初步了解组件的外部状态-props","link":"#初步了解组件的外部状态-props","children":[]},{"level":3,"title":"被动更新","slug":"被动更新","link":"#被动更新","children":[]},{"level":3,"title":"我们需要 shouldUpdateComponent","slug":"我们需要-shouldupdatecomponent","link":"#我们需要-shouldupdatecomponent","children":[]}]},{"level":2,"title":"函数式组件的更新","slug":"函数式组件的更新","link":"#函数式组件的更新","children":[]}],"git":{"updatedTime":1783311351000,"contributors":[{"name":"leeguooooo","email":"guoli@zhihu.com","commits":4},{"name":"guoli","email":"guoli@zhihu.com","commits":1}]},"autoDesc":true,"filePathRelative":"Vue-渲染器之patch.md","excerpt":"<h2>简版速记</h2>\\n<blockquote>\\n<p><strong>patch 核心流程一览（面试速答）</strong></p>\\n</blockquote>\\n<table>\\n<thead>\\n<tr>\\n<th>场景</th>\\n<th>策略</th>\\n</tr>\\n</thead>\\n<tbody>\\n<tr>\\n<td>新旧 VNode <strong>类型不同</strong></td>\\n<td><code>replaceVNode</code>：移除旧 DOM，挂载新 VNode</td>\\n</tr>\\n<tr>\\n<td>同类型<strong>标签元素</strong></td>\\n<td><code>patchElement</code>：tag 不同则替换；tag 相同则依次 patchData + patchChildren</td>\\n</tr>\\n<tr>\\n<td><strong>VNodeData 更新</strong></td>\\n<td><code>patchData</code>：遍历新数据全量应用；再遍历旧数据移除多余项（style / class / 事件 / DOM Prop / Attr）</td>\\n</tr>\\n<tr>\\n<td><strong>子节点更新</strong></td>\\n<td><code>patchChildren</code>：3×3 = 9 种情况；只有<strong>新旧均为多子节点</strong>时才需核心 diff（本章暂简化为全删全加）</td>\\n</tr>\\n<tr>\\n<td><strong>文本节点</strong></td>\\n<td><code>patchText</code>：比较 <code>children</code> 文本，不同则设 <code>el.nodeValue</code></td>\\n</tr>\\n<tr>\\n<td><strong>Fragment</strong></td>\\n<td><code>patchFragment</code>：直接调 <code>patchChildren</code> + 按子节点类型更新 <code>el</code> 引用</td>\\n</tr>\\n<tr>\\n<td><strong>Portal</strong></td>\\n<td><code>patchPortal</code>：先在旧容器中更新子节点，再用 <code>appendChild</code> 将子节点搬运到新容器（移动已存在的 DOM 节点）</td>\\n</tr>\\n<tr>\\n<td><strong>有状态组件—主动更新</strong></td>\\n<td><code>instance._update()</code>：<code>_mounted</code> 为真时走 <code>patch(旧VNode, 新VNode)</code>；否则走初次 <code>mount</code></td>\\n</tr>\\n<tr>\\n<td><strong>有状态组件—被动更新</strong></td>\\n<td><code>patchComponent</code>：组件类不同则 <code>replaceVNode</code>（触发 <code>unmounted</code>）；相同则更新 <code>$props</code> 再调 <code>_update()</code></td>\\n</tr>\\n<tr>\\n<td><strong>函数式组件更新</strong></td>\\n<td>通过 <code>vnode.handle</code>（prev / next / container / update）协调新旧 VNode；<code>patchComponent</code> 更新 handle 后调 <code>handle.update()</code></td>\\n</tr>\\n</tbody>\\n</table>"}');export{k as comp,v as data};
