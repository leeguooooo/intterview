import{_ as n}from"./s_poetries_work_uploads_2024_02_7cb972d068b2d405-2w8zVw84.js";import{_ as s,c as a,o as e,a as p}from"./app-D2IzyuJd.js";const o="/images/s_poetries_work_uploads_2024_02_0fb891a8c939750a.webp",t="/images/s_poetries_work_uploads_2024_02_9d80fe7de09592dc.webp",c="/images/s_poetries_work_uploads_2024_02_1ce999b58fbe87ce.webp",l={},i=p(`<h2 id="简版速记" tabindex="-1"><a class="header-anchor" href="#简版速记"><span>简版速记</span></a></h2><p><strong>patch 核心原则</strong></p><ul><li>类型（flags）不同 → 直接 <code>replaceVNode</code>（移除旧 DOM，mount 新 VNode）</li><li>类型相同 → 按类型分发：<code>patchElement</code> / <code>patchComponent</code> / <code>patchText</code> / <code>patchFragment</code> / <code>patchPortal</code></li></ul><p><strong>patchElement 三步</strong></p><ol><li>tag 不同 → <code>replaceVNode</code></li><li>更新 VNodeData：遍历新 data 调 <code>patchData</code>；再遍历旧 data 对已消失的 key 传 <code>null</code> 给 <code>patchData</code> 移除</li><li>递归 <code>patchChildren</code></li></ol><p><strong>patchChildren 九种情况（3 × 3）</strong></p><table><thead><tr><th>旧 \\ 新</th><th>单节点</th><th>无节点</th><th>多节点</th></tr></thead><tbody><tr><td>单节点</td><td><code>patch</code> 递归</td><td>removeChild</td><td>remove + mount 多个</td></tr><tr><td>无节点</td><td>mount 新</td><td>无操作</td><td>mount 多个</td></tr><tr><td>多节点</td><td>remove 全部 + mount</td><td>remove 全部</td><td><strong>核心 diff</strong>（本章暂用暴力替换）</td></tr></tbody></table><blockquote><p>只有「多→多」才需要核心 diff 算法以复用节点，其余情况暴力增删即可。</p></blockquote><p><strong>patchText</strong>：对比 <code>children</code> 文本内容，不同则 <code>el.nodeValue = nextVNode.children</code></p><p><strong>patchFragment</strong>：直接委托 <code>patchChildren</code>，再按子节点类型更新 <code>nextVNode.el</code> 引用</p><p><strong>patchPortal</strong>：先在旧容器里 <code>patchChildren</code>；若挂载目标（<code>tag</code>）变了，再用 <code>appendChild</code> 把已更新的子节点搬运到新容器</p><p><strong>有状态组件更新</strong></p><ul><li>主动更新：<code>_update()</code> 用 <code>_mounted</code> 标志区分初次挂载（<code>mount</code>）和后续更新（<code>patch</code>）</li><li>被动更新：父组件 <code>patchComponent</code> 先更新子组件 <code>$props</code>，再调 <code>_update()</code></li><li>组件替换（不同 <code>tag</code>）：<code>replaceVNode</code> + 触发旧组件 <code>unmounted</code> 钩子</li></ul><p><strong>函数式组件更新</strong>：在 <code>VNode</code> 上挂 <code>handle</code> 对象（<code>prev/next/container/update</code>），<code>patchComponent</code> 更新 <code>handle.prev/next</code> 后调 <code>handle.update()</code>；<code>update</code> 内部用 <code>handle.prev</code> 是否为 <code>null</code> 区分初次 / 更新</p><blockquote><p>在上一章中我们讲解并实现了渲染器的挂载逻辑，本质上就是将各种类型的 <code>VNode</code> 渲染成真实DOM的过程。渲染器除了将全新的 <code>VNode</code> 挂载成真实DOM之外，它的另外一个职责是负责对新旧 <code>VNode</code> 进行比对，并以合适的方式更新DOM，也就是我们常说的 <code>patch</code>。本章内容除了让你了解基本的比对逻辑之外，还讲述了在新旧 <code>VNode</code> 比对的过程中应该遵守怎样的原则，让我们开始吧！</p></blockquote><h2 id="基本原则" tabindex="-1"><a class="header-anchor" href="#基本原则"><span>基本原则</span></a></h2><p>通常重渲染(<code>re- render</code>)是由组件的更新开始的，因为在框架的使用层面开发者通过变更数据状态从而引起框架内部对UI的自动更新，但是组件的更新本质上还是对真实DOM的更新，或者说是对标签元素的更新，所以我们就优先来看一下如何更新一个标签元素。</p><p>我们首先回顾一下渲染器的代码，如下：</p><pre><code>function render(vnode, container) {
...
<span class="line">    <span class="token punctuation">}</span></span>
<span class="line"></span></code></pre><div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0;"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>如上代码所示，既然 <code>patch</code> 函数的作用是用来比对新旧 <code>VNode</code>，那么 <code>patch</code> 函数必然需要接收新旧 <code>VNode</code> 作为参数，我们使用 <code>prevVNode</code> 形参代表旧的 <code>VNode</code>，使用 <code>nextVNode</code> 形参代表新的 <code>VNode</code>，如上是很清晰的一段比对逻辑，首先我们需要拿到新旧 <code>VNode</code> 的类型(<code>flags</code>)，接着是一连串的 <code>if...else if</code> 语句，其核心原则是：<strong>如果类型不同，则直接调用<code>replaceVNode</code> 函数使用新的 <code>VNode</code> 替换旧的 <code>VNode</code>，否则根据不同的类型调用与之相符的比对函数</strong>，如下图所示：</p><p><img src="`+o+`" alt=""></p><h2 id="替换-vnode" tabindex="-1"><a class="header-anchor" href="#替换-vnode"><span>替换 VNode</span></a></h2><p>我们首先来研究一下如何替换 <code>VNode</code>，即 <code>replaceVNode</code> 函数应该做什么，我们先来复现需要替换 <code>VNode</code> 的场景，如下代码所示：</p><div class="language-javascript line-numbers-mode" data-highlighter="prismjs" data-ext="js" data-title="js"><pre class="language-javascript"><code><span class="line">    <span class="token comment">// 旧的 VNode 是一个 div 标签</span></span>
